...
4. **Perifocal state** (orbital plane; `x̂` → periastron, `ŷ` 90° ahead):
   ...
   (Watch the prefactor: `n·a/(1−e cos E)`, the eccentric-anomaly form. The
   true-anomaly form uses `n·a/√(1−e²)` with `(−sin ν, e+cos ν)` — mixing the
   ν-form prefactor with `sin E / cos E` scales the speed by
   `(1−e cos E)/√(1−e²)` and corrupts every derived period. This exact mixup
   shipped in the pipeline until 2026-08-21.)
