...
| 23 | **Neumann+ 2019** ([`2019ApJ...882...47N`](https://ui.adsabs.harvard.edu/abs/2019ApJ...882...47N)) | §2.3 defines `f_i` as *"the number of atoms of **the stable** isotope per 1 kg"* and then prints `f_i = 10³ x_i N_A / m_{a,i}` *"with the relative mass fraction x_i of the stable isotope, the molar mass of **the radioactive** isotope"*. Counting ²⁷Al atoms needs 26.98, not 26 | Every ²⁶Al heat number inherits a **3.77 % overstatement** if the formula is followed as printed. It moves no verdict in C21 — the `t₀` threshold shifts under 2 % — but it silently propagates into any absolute heat budget. ⚠ **Under review, not confirmed** (2026-09-07): Ruedas 2017 Table 2 gives ²⁶Al an atomic mass of **25.986891863 u** — the radioactive isotope's own — so dividing by the radioactive molar mass may be a convention this reading does not see rather than a slip. Settling it needs a source that prints the atom count directly; not done, and the row stays open rather than being closed either way. Discriminant recorded: `H₀ = 2.041×10⁻⁷ W/kg` (26.98) vs `2.118×10⁻⁷` (26) for ordinary chondrite | C21 (b), 2026-09-07 |
