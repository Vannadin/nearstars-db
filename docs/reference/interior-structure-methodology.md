...
Regenerated by `python3 engine/test_ice_giant.py --refresh` (about 13 minutes for Uranus,
2026-08-28) and printed from the frozen result by `--table`. The full solve is **not** in
`scripts/check.sh`; what the gate runs is one integration at the frozen converged point plus a
fingerprint of the shooting path, so a change that would move the answer fails the gate and
asks for a refresh. Where the minutes go is measured in `engine/speed-context-notes.md`: not
the Fermi integrals but a pressure shoot that cannot converge on a mass staircase.
