...
`core_temperature` [K] · `cmb_temperature` [K] · `cmb_temperature_core` [K] · `cmb_temperature_mantle` [K] · `ice_column_state` [—] ·
...
**Declared-optional** — inputs this recipe declares a default for, in the code and in prose above; absent is a normal state, not a hole (C50 (b), Brief 170 B). `ice_mass_fraction` [—] (absent is recorded as `0.0`, because the recipe normalises it internally) · `differentiated` [—] (default `True`) · `initial_porosity` [—] and `porosity_cap` [Pa] (the code says the default 0 *«means this recipe does not decide, not that porosity is zero»*) · `gas_mass_fraction` [—] (absent means «not a gas body»; absent is recorded as `0.0`, because the recipe normalises it internally) · `envelope_z` [—] (*«Z is a declaration: accretion and evolution set it and this recipe has neither»*) · `ice_allowed` [—] (**derived, not declared** — it is `ice_mass_fraction` read as a statement: an explicit `0.0` means «this body has no ice», absent means «unknown», and only the second opens the ice axis for the inversion. C57, Brief 182 B) · `basal_iron_number` [Fe#, 0–100] (**a declaration, not a derivation**, C100; C103 removes it. Fe# is `100*Fe/(Fe + Mg)` after Samuel+ 2021, so it runs 0–100 — writing an Mg# (0–1) here makes the melting-curve shift a hundred times too small, and *too small looks exactly like nothing happening*. Absent means the curves are untouched: `d_fe` stays `None` and every value is bit-identical. ⚠ **The enrichment reaches the deepest rock sample only** — it is a property of the basal layer, so shallower samples use the unshifted curve by design. The integrator's latent-heat term does not see it at all, and that one is a limit: knowing which step lies inside the layer needs the layer, which is the answer being sought) · `interface_temperature_jumps` [K per boundary] (a declaration: boundary name «inner role/outer role» → jump, roles `core` · `rock` · `ice` · `crust` · `envelope`, material boundaries of the stack only — upper/lower mantle and inner/outer core are not layers and wait for phase 4. Absent means no jumps and every old value is bit-identical. `ice/envelope` is the old `boundary_temperature_jump` under its boundary name and runs the old path; giving both is refused. prereg-interface-jumps.md). · `lithosphere_thickness_km` [km] (a declaration with the three provenance cells — grade · source · counter_evidence_searched. A conductive surface layer: inside it the density is read at T = A/r + B (no internal heating), joined continuously to the adiabat at its base and to `surface_temperature_k` [K] at its top; the integrated temperature itself stays the virtual adiabat, so the surface condition «adiabat's end = T_pot» is unchanged. The base radius needs R, so the solve repeats until |ΔR| ≤ 1 m (at most 8 times, else refused). Absent means no layer and every old value is bit-identical; `surface_temperature_k` is read only when the layer is declared. Refused on the inverting branch for now. prereg-surface-lithosphere.md) · `core_light_elements` [mass fraction per element] (a declaration with the three provenance cells: the core's light elements S · Si · O · C · H as mass fractions, inside the core box of `eos.CORE_BOX_WT`, else refused by name. A body with an observed core radius may give `S: fit` — the sulphur fit against `core_plus_layer_radius_km` then uses this field's O · C, which must equal one of the frozen fixings (Mars: `box_ceiling`); a body without one gives numbers and gets a Huang+ 2023 19 GPa liquid Fe–S–O–C core. Si and H are named but refused until their material frames land (ⓒ2 · ⓒ3); `fit` is sulphur-only. Absent means the composition's own core material and every old value bit-identical. prereg-core-light-elements.md).
...
| `antigorite` (serpentinised rock, the light end of one declared axis) | BM2 | 2640.5 (**derived**: 273.50 u per m = 1 unit / 172 Å³) | 67.27 | 4 (fixed) | 0 to 10 GPa | 0.00133 — **borrowed**: Holland & Powell 1998 Table 5 a° = 4.70×10⁻⁵ K⁻¹ in their α(T) = a°(1 − 10/√T), flattened at 298 K and multiplied by Hilairet's K₀ | 298 K | Hilairet+ 2006 §3 [13] BM2 (V₀ 2926.23 Å³), ρ₀ from the structural formula §2 [6] and the m = 1 volume §4 [15]; thermal term from Holland & Powell 1998 Table 5 (atg), the source Hilairet's own §4 borrows from; c_V 966 J/kg/K from their C_p polynomial at 298 K  |
...
1000 K up, the floor Mazevet+ 2019 §3.1 states for ρ ≳ 1 g/cc; between the ocean table and
that floor it goes to the dense-liquid table (`h2o_liquid_dense`, SeaFreeze `water2`, Brown
2018 — baked 2026-08-30 to the spline's real ceiling, 2.3 GPa at 360 K rising to 30 GPa at
1000 K). What still declines by name: 12–20.6 GPa between the melting curve and that ceiling,
and hot water below 0.1 GPa.
...
| Mars | 0.5318 | 0.5320 | -0.0 % | 0.3545 | 0.36340 | 2.4 % | 0.492 | 0.540 | 45.9 |
...
**The ice giants' moment of inertia, put in front of publication (2026-08-31).** Until this
date `nmoi` was bit-compared only against the frozen anchor — reproduced daily without ever
being asked whether it is the right number; only the radius had a `published` column. No
spacecraft has measured either planet's moment of inertia, so every published C/MR² is a
derived value — an interior model fitted to J₂ and J₄ under an **assumed rotation period**,
and that assumption alone moves the value by −3.3 % (Uranus) / +6.0 % (Neptune) between the
Voyager radio periods and Helled+ 2010's modified ones. The headline column is
**Nettelmann+ 2013** ([`2013P&SS...77..143N`](https://ui.adsabs.harvard.edu/abs/2013P%26SS...77..143N)),
whose footnote 2 prints λ = I/(M_p R_mean²) — the same mean-radius normalization as this
recipe's sphere — at the Voyager periods: **Uranus 0.230(1), Neptune 0.2410(8)**.
**Neuenschwander & Helled 2022**
([`2022MNRAS.512.3124N`](https://ui.adsabs.harvard.edu/abs/2022MNRAS.512.3124N)) stand
beside it as the independent empirical check, their Table 2 solution bands normalized to
the *equatorial* radius — a 1.2–1.6 % convention gap that would manufacture a fake
disagreement if quoted raw (the same trap as the fact sheet's 0.254 above); converted by
(a/R_mean)² the two papers meet within 0.15 %, and the gate recomputes that conversion on
every run as its transcription check.

| body | C/MR² derived | published (N13, P_Voy, R_mean) | Δ | I/(M·R_pub²) | Δ after | vs P_HAS |
|---|---|---|---|---|---|---|
| Uranus | 0.1741 | 0.230(1) | **−24.3 %** | 0.1937 | −15.8 % | −21.7 % |
| Neptune | 0.1799 | 0.2410(8) | **−25.3 %** | 0.2135 | −11.4 % | −29.6 % |

The middle columns take the radius error out: the denominator's R² carries the +5.48 % /
+8.94 % radius excess squared, and renormalizing to the published mean radius leaves
**−15.8 % / −11.4 %** that the radius does not explain — at matched mass and radius this
recipe's mass sits more centrally condensed than the gravity-fitted profiles. Where that
remainder lives (the silicate/ice/envelope split, the envelope drawn too thick, the missing
rotation) is not analysed here; the number is measured and written down, with no tolerance
asserted — the only thing the gate can FAIL on is the two sources disagreeing after the
normalization conversion. Regenerated by `python3 engine/test_ice_giant.py --table`; the
investigation lives in the `PUBLISHED_NMOI` block of `engine/test_ice_giant.py` and in
`engine/icegiant-nmoi-context-notes.md`.
...
| **liquid above 2.3 GPa, below 1000 K** | a column warm enough to melt ice VI/VII but under the hot-water fit's floor | **integrates** on `h2o_liquid_dense` (SeaFreeze `water2`, Brown 2018) up to the spline's ceiling at that temperature — 3.0 GPa at 400 K, 10 GPa at 600 K, 13 GPa at 700 K, 30 GPa at 1000 K — with its own dT/dP\|_S and c_P. Above the ceiling and under 1000 K (12–20.6 GPa, a band ≲ 170 K above the melting curve) it **declines** by name, thrown as too cold since hotter resolves it | analog |
...
| **undifferentiated with ice or gas** | `differentiated: false` and `ice_mass_fraction` or `gas_mass_fraction` > 0 | **declines**, and the reason is structural rather than a missing rule (C7, closed 2026-08-30): water mixed into silicate is a **reaction** — hydrated minerals with their own density, volume change and heat — and what makes a body neither fully mixed nor fully layered is a **transport history**, how far the water got. The literature models that as thermal evolution (Malamud & Prialnik 2015, read in full on 2026-08-30 — F3; 2013 and Prialnik & Merk 2008, abstracts only), not as a composition a hydrostatic solver takes. A mixing rule exists only for the never-wet state — cold ice and rock grains compacted on their own curves, volumes additive (Malamud & Prialnik 2015 §3.1.3, the two-layer model) — and none for the reacted, partially differentiated body; the refusal stands on the latter. This does **not** cover C10: antigorite with enstatite is two solids as grains, and volume additivity between them is the rock–metal rule's own shape | — |
| **a declared differentiation front** (the middle rung, C11) | `differentiation_front` < 1 with ice and a declared `crust_rock_fraction` | **integrates** a fourth layer: above the front, a never-melted primordial crust of ice-ladder and silicate grains mixed by additive volume (Malamud & Prialnik 2015 §3.1.3, the two-layer model of Yasui & Arakawa 2009 — the shape C10 uses for antigorite plus enstatite, not the reaction C7 declined). Two declarations, not one: how deep melting reached, and the crust's rock fraction (the source's outer mantle is ice-enriched by refrozen water, so the front alone does not fix it). A crust step above the melting curve is refused as a self-contradictory declaration; optional `crust_porosity` adds the same paper's eqs. (4)–(6) with Γ = 1 as a bounded term (crust rock raises C/MR², porosity lowers it). Not tuned: Callisto and Titan are reported on a declared grid | analog |
...
| **large rock core in a giant** | heavy elements placed as a compact core whose base passes 13.5 TPa. In a Jupiter-mass giant that is a core above **16.7 M⊕** (measured 2026-08-30; the polytrope envelope allowed 17.7, the "0 M⊕ since the table" of 2026-08-28 was the envelope-base defect fixed with the sub-Neptunes, and the 11.5 of 2026-08-29 was the isothermal-bulk-modulus finite difference poking half a step past the silicate ceiling on the shooting's ceiling trial — clipped in F2, not physics either) | **partly open**: cores below that integrate at grade analog; above it the recipe declines, naming the silicate ceiling. The limit is the envelope's overburden, not the core's own weight, so it depends on the pair rather than on the core mass | analog |
...
| **ammonia in the ice mantle** | `ammonia_mass_fraction` > 0 with a declared temperature (C22 step 1, 2026-09-03; default 0 — the anchors do not move) | **integrates** at the same site as the rock: the ices are mixed first (`mix(water phase, 1 − w; nh3, w)`, Bethkenhagen+ 2013's table, additive volume, ∇_ad by c_P), then the rock. A declaration — the solar-ratio pair value is 0.1159 (Bethkenhagen+ 2017 §V) and is not the default. Five labels ride on any w > 0: methane's larger share is not built (C4); the table's ceiling (237–333 GPa by isotherm) is below the mantle base (820–1016 GPa), where the table refuses by name and no deep-mantle rule is declared (step 2, owner); the convention caveat reaches ∇_ad (density grounded, thermal sign not); ammonia is fluid along the Redmer+ 2011 isentropes (2013 §III A) but molecular-vs-dissociated is unsaid; partial dissociation to N₂+H₂ is marked only in 2017's Fig. 6. Measured: the wrap reproduces the eight-point −2.9 to −3.5 % of `ammonia-table-context-notes.md` §113; the opt-in full solve at 0.1159 died on the trial corridor's cold flank (152 GPa · 545 K) because the table's refusal carried no `too_cold` — step 2's second prerequisite. Less dense, so it **enlarges** the planet | analog |
| **serpentinised rock** | `serpentinisation` in (0, 1] on a differentiated body | **integrates**, the rock layer a volume-additive mixture of the enstatite/PREM silicate and antigorite (Hilairet+ 2006 BM2 to 10 GPa; ρ₀ derived, not printed) — two solids as grains, the rock–metal rule's own shape and not the reaction C7 declined. A declaration: how far the water got is history. antigorite's thermal term is **borrowed** from Holland & Powell 1998 (the source Hilairet's own text borrows from) — a pure-Mg end-member's expansivity, temperature-dependent in the source and flattened here at 298 K (40 % low by 600 K) — so the grade is set by that borrowing and flattening, not by the fit (F2, 2026-08-30; it used to be set by the term's absence). Above 10 GPa the antigorite end declines by name (dehydration). On Callisto, Titan and Enceladus no fraction in [0, 1] reaches the published C/MR² | analog |
| **hot water outside its fit** | a liquid above 50 000 K, or under 1000 K where `water2`'s ceiling has also been passed | **declines** by name at both ends. The floor is Mazevet+ 2019 §3.1's own for ρ ≳ 1 g/cc ("10³ K ≲ T"); under it the dense-liquid table (`water2`) carries the liquid up to its ceiling, and only past that ceiling is the refusal thrown — as too cold, since the fluid opens above. Where a column crosses 1000 K liquid, the switch is a measured −2.5 … −3.3 % density step (Mazevet relative to water2, 2.3–26 GPa). The ceiling is the paper's 50 000 K | — |
...
- **Bollengier, O., Brown, J. M. & Shaw, G. H. 2019**, J. Chem. Phys. 151, 054501
  ...
  Above it, **Brown, J. M. 2018**, Fluid Phase Equilibria 463, 18
  ...
  function representations of thermodynamic surfaces: H₂O at high pressure and temperature
  as an example*, distributed as SeaFreeze's `water2`; **baked 2026-08-30** into
  `water2_table.py` for the dense liquid between the ocean table and the hot-water fit, to the
  spline's real ceiling (not its 100 GPa knot box). Its range as a third party states it:
  AQUA §2.3.5, "liquid and supercritical H₂O from 1 GPa to 100 GPa and up to 10⁴ K". Brown's
  own fit-and-residual discussion has not been read (paywalled). *No arXiv preprints.*
- **Haldemann, J., Alibert, Y., Mordasini, C. & Benz, W. 2020**, A&A 643, A105
  ...
  State for Planetary Models* — seven regions from 0.1 Pa to 400 TPa with the seam
  methodology written out (Method 1 at the Gibbs crossing; Method 2 interpolating
  derivatives, "thermodynamic consistency will not be guaranteed"). Read for its §2.3.5, which
  delegates this recipe's band to Brown 2018; **on the shelf, not in the engine** — the
  candidate for the day the ladder runs out of range. *Open access.*
...
- **Holland, T. J. B. & Powell, R. 1998**, J. Metamorphic Geol. 16, 309
  ([`1998JMetG..16..309H`](https://ui.adsabs.harvard.edu/abs/1998JMetG..16..309H),
  doi [10.1111/j.1525-1314.1998.00140.x](https://doi.org/10.1111/j.1525-1314.1998.00140.x)).
  *An internally consistent thermodynamic data set for phases of petrological interest.*
  **Cached** as `docs/phase3/_papers/1998JMetG..16..309H.pdf` (fetched by the owner). Table 5's
  antigorite (atg, Mg₄₈Si₃₄O₈₅(OH)₆₂): a° = 4.70×10⁻⁵ K⁻¹ in V(1,T) = V°[1 + a°(T−298) −
  20a°(√T − √298)], κ₂₉₈ = 525 kbar, the C_p polynomial — the thermal term antigorite carries
  here, borrowed exactly where Hilairet+ 2006 §4 borrow theirs (F2). Verified by bibcode and
  title; first page reads "J. metamorphic Geol., 1998, 16, 309–343".
...
- **Nettelmann, N., Helled, R., Fortney, J. J. & Redmer, R. 2013**, P&SS 77, 143
  ([`2013P&SS...77..143N`](https://ui.adsabs.harvard.edu/abs/2013P%26SS...77..143N), arXiv
  **[1207.2309](https://arxiv.org/abs/1207.2309)**). **Cached** in
  `docs/phase3/_papers/1207.2309.md`. The ice giants' published C/MR²: three-layer LM-R
  models fitted to the Jacobson gravity data under both rotation hypotheses — at the
  Voyager periods λ = 0.230(1) (Uranus) and 0.2410(8) (Neptune), at the Helled+ 2010
  modified periods 0.2224(1) and 0.2555(2) — with footnote 2 printing the normalization
  λ = I/(M_p R_mean²), which is what lets these values stand against this recipe's `nmoi`
  without a conversion.
- **Neuenschwander, B. A. & Helled, R. 2022**, MNRAS 512, 3124
  ([`2022MNRAS.512.3124N`](https://ui.adsabs.harvard.edu/abs/2022MNRAS.512.3124N), arXiv
  **[2203.02233](https://arxiv.org/abs/2203.02233)**). **Cached** in
  `docs/phase3/_papers/2203.02233.md`. The independent empirical check on the ice giants'
  C/MR²: piecewise-polytropic ToF models, Table 2's solution bands per rotation period and
  wind depth, normalized to the equatorial radius (§3.6) — the gate converts them by
  (a/R_mean)² with Table 1's equatorial radii and checks they meet Nettelmann+ 2013 within
  0.5 % (measured 0.10–0.15 %).
...
- **Malamud, U. & Prialnik, D. 2015**, Icarus 246, 21
  ...
  actually gets (C7), and two of C9's exclusions. **Read in full** (PDF in the cache,
  2026-08-30, F3): its §3.3 EOS is an equilibrium closed form with printed coefficients —
  ψ_w = 0.45 exp(−β_w(T/T_m)√P), ψ_d = 0.4 exp(−β_d P) Γ(T_max), two-layer volume additivity —
  fitted to laboratory compaction to 764 MPa, ice I, no rock melt; the differentiation front
  is an output of the evolution run, not a parameter. Verified by bibcode and title;
  `engine/malamud-readthrough-context-notes.md`.
