...
| ice giant | `body_class` is `ice_giant` | **declines**, and the distance is now measured rather than gestured at: the ice mantle sits entirely above the water ladder's 1800 K ceiling (100 GPa at 5000–7000 K; centres at 5700 K for Uranus and 5500 K for Neptune, Scheibe+ 2019), so what is needed is the fluid/superionic branch rather than a high-pressure solid. Of the three components only water has a transcribable form (Mazevet+ 2019); ammonia and methane exist as tables, and the cost of letting water stand in for all three is not quantified anywhere cited. And the equation of state would not be sufficient: built on that same EOS, adiabatic models miss both planets' cooling times in opposite directions, because "neither planet is fully adiabatic in the deeper interior" — the difference is a thermal boundary layer, set by a heat flux this recipe does not carry | — |
