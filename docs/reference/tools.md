...
- `scripts/check_methodology_coverage.py` — check.sh gate 12: a methodology doc must be registered in all three places, so this compares entry *sets*, not file existence — the English index, the Korean mirror (a mirror listing fewer recipes than its source passes the mirror gate), and the GitHub wiki's `Methodology-Library` / `-ko` portal, which lives in `nearstars-db.wiki.git` and is otherwise outside every gate here. Also fails on a `*-methodology.md` file no index row points at. The wiki half shallow-clones; unreachable network warns and passes, `--no-wiki` skips it
