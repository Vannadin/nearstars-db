...
| Paper cache: is this paper held? (bibcode **and** arXiv name, sub-folders) | `scripts/refs/check_paper_held.py <bibcode …>` · `--scan <doc.md …>` | [11](#11-dev-helpers) | On demand, never in `check.sh`: the cache names files by bibcode *or* by arXiv id and keeps some in sub-folders, so a bibcode-prefix glob manufactured six false "not held" on 2026-09-03/04 (RM22, Nettelmann 2011, Reiners & Christensen, Garraffo, Yadav & Thorngren, Zhang & Rogers). Two defects of its own on install day (2026-09-04): old-style arXiv ids (`astro-ph/0103383` → `astro-ph_…`, 52 cached names) were missed — the tool built to stop false absences made one for Burrows+ 2001, caught only because the ABSENT line prints what it checked (that format stays) — and "held" meant "a file exists": three cached names are arXiv abstract pages, so a held paper now needs a body (pdf, `ltx_document` html, or md > 2 KB) and the verdict is otherwise **ABSTRACT-ONLY**, distinct from ABSENT. A third the same day: bibcodes with `&` are cached with `&` → `_` (four names) — so doubt that the tool knows every cache naming rule, and when a paper you can see on disk comes back ABSENT, compare its "checked …" list with `ls` first. The arXiv id is **read** from ADS's `identifier` field (needs `ADS_API_TOKEN`; without it the tool exits rather than guess). Kept out of the gate because a token-less environment would fail it and because its negative is a cache state, not a repository defect. `--scan` over the two dynamo docs: 7/10 held, 4 of them under arXiv names. |
...
- `phase3/stability-sim/scripts/plot_interactive.py` — **interactive** 4-panel viewer (Plotly, self-contained HTML): same panels/data as plot_moons.py but click-legend to toggle a body across all panels, hover for exact values, box-zoom/pan to separate packed inner bodies. Follows the reader's `prefers-color-scheme` (live, no toggle — the rest of the site does the same, so asking again with a button was redundant); plasma palette (matches the other viewers). `--dir <results-dir> [--label <system>] [--center <body>]`.
...
- `phase3/stability-sim/scripts/validate_orbits.py` — validation-suite driver: expands each manifest system into its `planets|moons × leapfrog|accurate` cells, runs the missing ones (existing results are skipped — the accurate cells cost hours), renders each, and writes `docs/phase4/orbit-viewers/<slug>/index.html` + the index across systems (the whole orbit-viewer surface — the old `build_viewers.py` gallery re-ran the same leapfrog integrator only to sample it densely enough to animate, so that run is now the leapfrog cell itself — snapshots are 10,000 uniform, which resolves every hierarchy's secular cycles and still leaves the animation ~200 frames in its `anim_years` window). Derives the accurate horizon and the folded moon mass per system. The gallery index always lists every manifest system with stored results, so `--systems` narrows what runs without shrinking the site. Cells are independent and REBOUND is single-threaded per run, so `--jobs N` runs N concurrently (one core each, logging to `<cell-dir>/run.log`). `--systems`, `--cells`, `--force`, `--jobs`, `--pages-only`, `--dry-run`.
- `phase3/stability-sim/scripts/hades_rescue_scan.py` — **orbit-rescue scanner**: steps one element of a hypothetical moon (α Cen's Hades) over a list, runs each candidate for 10⁵ yr in the validation-cell configuration, and tables the verdict against the nearest low-order resonances with the moons on either side. Completed candidates are skipped, so an interrupted scan resumes for free. `--grid MIN:MAX:STEP` (semi-major axis in km), `--vary FIELD:v1,v2` (`a|e|i|ma`), `--combo f=v,f=v`, `--jobs N`. Results + conclusions in `results/hades_rescue/README.md`.
- `phase3/stability-sim/scripts/plot_rescue_scan.py` — survival-landscape figure for the above: max eccentricity per candidate over the resonance comb that explains it (`plot_moons.py` draws one run; this draws a whole scan). Writes both site palettes into the scan dir. `[--theme dark|light|both]`.
- `phase3/stability-sim/scripts/verify_j2c.py` — acceptance test for the compiled J2 force (`j2force.c` + `j2c.py`, default since the stride fix): builds the α Cen moon system twice, once with the C force and once with the Python callback, and compares every particle's position and velocity **bitwise** over a leapfrog run and an ias15+MEGNO run (the latter exercises the particles-array realloc that `init_megno` causes). Also reports the speedup (27.6× / 9.2× measured). Run it after touching either file; `STAB_J2_C=0` forces the Python path.
...
- `scripts/gate_targeted.py <base-sha> <head-sha>` — derive which physics tests the gate's `--targeted` lane must run, from the commit range alone. Builds the `engine/` import graph with the AST, takes the **transitive** closure of dependents of every changed module, and prints three lines: changed-path count, the changed code paths the mapping cannot explain (non-empty ⇒ the caller falls back to full), and the derived items (`test_*.py`, `tools/<tool>.py`, `run:bodies/<b>.yaml`). Any changed `engine/**.py` or body file adds all three answer runs unconditionally. Called by check.sh; not meant to be run by hand except to inspect a range
...
**Related — Dante size study viewer.** `phase4/viewers/dante-size-study.html` is a live copy of the frozen moon viewer, made for the 2026-08 Dante size review. It adds a radius slider (200–900 km, density fixed) that recomputes mass, surface gravity, tidal output (∝R⁵), surface flux (∝R³), the area-averaged radiating temperature — the hard constraint no heat partition can dodge — and the apparent diameter from Pandora, tinting the body from rock to incandescent as it heats. Values are synced to the design of record (the frozen original is deliberately NOT synced): Chaos e 0.10→0.02, Polyphemus 120→120.873 M⊕. Note the retrograde moons' (i, Ω) are an equivalent representation of the design values, not an error.
