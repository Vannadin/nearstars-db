...
**Declared-optional** — inputs this recipe declares a default for; absent is a normal state, not a hole. `composition_intent` [—] (absent → the preset **`earth_like`**, and the run says so: `composition_preset_used` is 1) · `composition` [—] (the function parameter carries the same default). ⚠ **`composition` moved out of Needs deliberately** (C65 follow-up, 2026-09-11): C45 (f) counts *a contract `Needs` filled by a caller's literal*, and this pair was one — the doc called it required while `def assign(..., composition="earth_like")` supplied it. A default that is declared is not a hole; **calling it required while defaulting it is what made it invisible**. ⚠ *This line is what the adapter actually looks up* — it read `composition_intent` while the contract named only `composition`, so the lookup and the contract disagreed and the default was invisible to the checker that counts them (C65 follow-up, C45 (f)).
