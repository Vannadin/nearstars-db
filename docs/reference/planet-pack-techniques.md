...
**NearStars rule:** the star's **base photospheric color** is the grounded
value from
[`stellar-photospheric-color-methodology.md`](stellar-photospheric-color-methodology.md)
(its three-regime rule: FGK/WD → blackbody at Teff, M dwarf → Pickles real-SED
pale warm orange, L/T/Y → blackbody deep dim red) — the single color path in the
repo. Every other star color (`sunspotColor`, `rimColor`, `*SunlightColor`,
`sunLensFlareColor`, `emitColor*`) is a **rendering derivation** of that one base
hex, not a second computation. Keep `rimPower` inversely related to Teff.
Generate, don't copy magic numbers. (Emitter: `emit_kopernicus_cfg.py` →
`star_fields.py`.)
...
- Colors: `sunlightColor`, `scaledSunlightColor`, `IVASunColor`,
  `sunLensFlareColor`, `ambientLightColor` — all derived from the star's grounded
  base hex (see [`stellar-photospheric-color-methodology.md`](stellar-photospheric-color-methodology.md);
  warm/orange for M/K, near-white for G/F, blue-white for A, **deep dim red** for
  L/T/Y brown dwarfs — *not* the "magenta" render intuition — and blue for a
  neutron star).
...
- **Brown / L-dwarf**: dim `luminosity` (~1–2), dark `ambientLightColor`,
  **deep-dim-red** `emitColor0/1` (intrinsic ember glow; the grounded base hex
  at ~1000–1300 K per
  [`stellar-photospheric-color-methodology.md`](stellar-photospheric-color-methodology.md)
  §6.3 — *not* the packs' magenta render choice), suppressed corona
...
distinct mechanisms exist across the packs; pick per case. (These are disk
*rendering techniques*; the disk **color** is derived separately per
[`debris-disk-color-methodology.md`](debris-disk-color-methodology.md) — feed its
tint into whichever mechanism you pick, don't invent a disk color here.)
...
> Schema/technique only. The atmosphere/ocean **colors** are derived, not
> guessed here: sky/haze tint via
> [`atmosphere-reflected-color-methodology.md`](atmosphere-reflected-color-methodology.md)
> and surface/ocean tint via
> [`surface-color-albedo-methodology.md`](surface-color-albedo-methodology.md).
> Feed those hexes into the fields below.
