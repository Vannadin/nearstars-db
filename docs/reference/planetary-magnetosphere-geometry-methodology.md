...
ends: `M_A` 0 gives 1, a sphere, and `M_A` 1 gives 150. (A linear form was tried first and
fails at the small end — [derivation record](solar-system-radiation-belts.md#retractions-and-first-attempts-that-were-wrong).)
...
Compression is `2^α` = 1.4142 in both, α 0.5 by analogy (the cylindrical threshold; no
sub-Alfvénic magnetopause has a fitted one). A b I owns no field, so no pause is emitted for
it at all; its `M_A` ~ 0 is the sphere limit of the same rule.
...
- **`*_deform` is the engine's non-dipolar knob, not decoration** (corrected 2026-08-04).
  ...
  modding docs describe it only as "deform the surface using a sum of sine waves", but
  ROKerbalism uses it as physics.
...
**Which plan a body gets is decided by α.** Fitting the generalized stock form to a body's
own softened Shue surface only succeeds near the cylindrical threshold, so `pause_waist` /
`pause_smooth` are recorded for exactly the bodies that can use them:
...
`pause_smooth` is **a convention, not a derived value** on the induced branch (it does not
generalise — see the [derivation record](solar-system-radiation-belts.md#retractions-and-first-attempts-that-were-wrong)) — the residual against the
...
**Every body keeps a single α** — a separate night-side value was trialled and rolled back
([derivation record](solar-system-radiation-belts.md#night-side-alpha-trialled-and-rolled-back)).
...
a bulbous head on a narrowing stem. Saturn's 0.736 puts it at −0.472, so its tail widens
without bound: a funnel. Same equation, opposite sign.

The driver is magnetodisc inflation, and it works through the denominator. `α` measures
flaring *relative to the nose*, and a disc that has already pushed the nose out leaves less
room for the flanks to grow past it. Measured against the vacuum-dipole Chapman–Ferraro
prediction at each planet's own wind pressure, the inflation ratios are Jupiter **1.58**,
Saturn **1.35**, Earth **1.05** — the inverse ordering of their α. Jupiter's Io torus is
