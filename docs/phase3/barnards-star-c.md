...
The orbit eccentricity is e = 0.08 ± 0.06 from the Basant 2025
β-distribution prior — the highest eccentricity nominal value of the
four planets, though still consistent with circular at 1.3σ and with
the long-term-stability requirement of e < 0.02. The orbital period
ratio with b (4.124 / 3.154 = 1.31) is close to but not exactly the
4:3 mean-motion resonance; Basant 2025 SPOCK stability tests find
the 4-planet configuration stable over 10⁹ orbits without explicit
resonance locking.
...
non-detection), and radius / composition are inferred from
mass-radius scaling. The atmospheric escape calculation from France
2020 — 87 Earth-atm Gyr⁻¹ at HZ (0.1 AU) — implies cumulative loss at
c's 0.0274 AU is ~13× higher per unit area, again precluding
significant primary atmosphere retention. The cfg adopts the same
bare-rock interpretation used for b and d, with c distinguished
visually by its slightly lower insolation (cooler surface tint) and
the highest absolute mass of the four (slightly larger angular
diameter in the sky relative to its smaller siblings).
...
| `eccentricity` | 0.08 | medium | Basant 2025 β-prior fit; highest nominal e of the four, but stability favors e < 0.02 |
...
| `radius_rearth` | 0.74 | medium | Tie-break: non-transiting; mass-radius for 0.34 M⊕ Earth-analog rocky → 0.69–0.79 R⊕; interesting-first picks 0.74 |
| `surface_gravity_g_earth` | 0.61 | medium | derived = 0.335 / 0.74² |
| `density_g_cc` | 4.6 | medium | Tie-break: slightly lower density than smaller siblings (mass-radius slope) |
| `insolation_s_earth` | 4.74 | high | derived from L = 0.00356 L☉ and a = 0.0274 AU |
| `equilibrium_temp_k` (A=0, full redistribution) | 400 | high | Basant 2025 Table 3 |
| `equilibrium_temp_k` (A=0, no redistribution) | 476 | high | derived dayside formula |
| `equilibrium_temp_k` (A=0.1) | 390 | high | derived |
...
| `star_apparent_angular_diameter_deg` | 3.63 | high | derived: 2 R★ / a × (180/π) |
...
c is tidally locked at 1:1 (synchronous). Walterová 2020 timescales
for a 0.34 M⊕ rocky planet at 0.0274 AU around a 0.16 M☉ M dwarf
remain well under 10⁵ years, much shorter than the 10 Gyr system age.
...
with long-term stability. The β-prior fit at e = 0.08 ± 0.06 is the
recommended value, but stability constraints (SPOCK 10⁹-orbit test)
favor e < 0.02. The libration amplitude at e = 0.08 would be
non-trivial (~5°), potentially producing measurable diurnal
insolation variation; at the recommended stability-constrained e <
0.02 the libration is negligible.
...
exactly 4/3). The 10-year stellar activity cycle modulates
...
- **Basant R. et al. 2025** — *Four Sub-Earth Planets Orbiting
  ...
  Msini = 0.335 ± 0.030 M⊕, a = 0.0274 AU, T_eq = 400 K (A=0, full
  redistribution).
...
- **Direct radius measurement**: as with all four planets, no
  ...
  cfg's `radius_rearth = 0.74` placeholder.
