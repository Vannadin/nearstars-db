...
above the formal confirmation threshold.
...
inner HZ edge for Barnard's M4 V host is at P ≈ 10 d (a ≈ 0.115 AU);
e at 6.74 d sits just inside that boundary, the closest of the four
planets to the HZ but still firmly in the runaway-greenhouse
...
The orbit eccentricity is e = 0.04 ± 0.04 (Basant 2025 β-prior),
...
places tidal locking timescales below 10⁵ yr (Walterová 2020),
guaranteeing synchronous spin-orbit configuration. The orbital
period ratio with c (6.739 / 4.124 = 1.634) is close to but not
exactly the 5:3 mean-motion resonance.
...
|---|---|---|---|
| `tidally_locked` | true | high | 6.74-d orbit → tidal lock < 10⁵ yr (Walterová 2020) |
| `obliquity_deg` | 0 | high | tidal damping |
| `eccentricity` | 0.04 | medium | Basant 2025 β-prior fit; consistent with circular |
| `sidereal_period_days` | 6.7392 ± 0.0028 | high | Basant 2025 |
| `semi_major_axis_au` | 0.0381 ± 0.0005 | high | Basant 2025 |
| `argument_of_periapsis_deg` | −27.5 | medium | Basant 2025 (low ecc → weak constraint) |
| `epoch_jd` | 2460245.3 | high | Basant 2025 t_peri |
| `mass_mearth` | 0.193 ± 0.033 | high | Basant 2025 Msini |
| `radius_rearth` | 0.64 | medium | Tie-break: non-transiting; mass-radius for 0.19 M⊕ Earth-analog rocky → 0.58–0.69 R⊕; interesting-first picks 0.64 |
| `surface_gravity_g_earth` | 0.47 | medium | derived = 0.193 / 0.64² |
| `density_g_cc` | 4.0 | medium | Tie-break: lowest density of the four (mass-radius slope); could be slightly volatile-enriched |
| `insolation_s_earth` | 2.45 | high | derived from L = 0.00356 L☉ and a = 0.0381 AU |
| `equilibrium_temp_k` (A=0, full redistribution) | 340 | high | Basant 2025 Table 3 |
| `equilibrium_temp_k` (A=0, no redistribution) | 405 | high | derived dayside formula |
| `equilibrium_temp_k` (A=0.3) | 311 | high | derived |
...
| `atmosphere_scale_height_km` | 11 | medium | derived: kT/μg with T = 340 K, μ = 44, g = 4.6 m/s² |
...
| `surface_radiation_dose_msv_yr` | 3000 | medium | Atri 2020 scaling: thin atmospheric shielding (~10 g/cm²) + 2.45 S⊕ XUV × France 2020 duty cycle |
...
| `star_apparent_angular_diameter_deg` | 2.61 | high | derived: 2 R★ / a × (180/π) |
...
cfg's 0.64 R⊕ choice is midpoint. The density 4.0 g/cc is the lowest
of the four planets — consistent with either (a) slight volatile
enrichment (a few wt% H₂O / CO₂ in the mantle) or (b) simply the
mass-radius slope for small rocky planets. A direct radius measurement
would distinguish, but no transit detection exists.
...
cfg's 0.01 bar surface pressure is ~10⁻⁴ of Venus's 92 bar.
...
e is tidally locked at 1:1 (synchronous). Tidal-locking timescales
for a 0.19 M⊕ rocky planet at 0.0381 AU around a 0.16 M☉ M dwarf
are well under 10⁵ years per Walterová 2020 — far shorter than the
10 Gyr age. Obliquity is damped to 0°.

Basant 2025 best-fit e = 0.04 ± 0.04 is consistent with circular;
SPOCK stability requires e < 0.02 for long-term 4-planet residency.
Libration amplitude at the recommended low eccentricity is < 1° and
does not produce measurable diurnal modulation.
...
~10-20 d. Basant 2025 explicitly rules out planets larger than ~0.5 M⊕
in the 10–42 d window with 99% confidence; future MAROON-X / ESPRESSO
data could push this limit lower.
...
papers (Comet Barnard e 1881, 1885, 1887). Preserved in
`docs/phase3/_bib/barnards-star-e.yaml` with `status: skipped`.
