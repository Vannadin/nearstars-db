...
| `disk_tint_rgb_hex` | `#fbeacb` (pale cream, blue-leaning reflectance under red M1V light) | medium | CORRECTED: the disk is NOT achromatic — it is MEASURED BLUE in reflectance (Krist 2005, B/I ~ 1.6; Fitzgerald 2007 blue in NIR; tiny porous blowout grains a ~0.1 µm, Graham 2007). Mie synthesis: that strong blue scattering shifts the red M1V starlight bluer/lighter than a neutral grey would, giving a pale cream absolute color (the famous "AU Mic blue" is the disk-vs-star reflectance, not the absolute hue) |
