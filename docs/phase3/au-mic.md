...
| `disk_tint_rgb_hex` | `#e3f0ff` (blue; vivid `#acd6ff`) | medium | CORRECTED: the disk is NOT achromatic — it is MEASURED BLUE (Krist 2005, B/I ~ 1.6; Fitzgerald 2007 blue in NIR; tiny porous blowout grains a ~0.1 µm, Graham 2007). Mie reflectance synthesis reproduces it (B/I 1.74 ≈ the measured 1.6); the cfg tint is the intrinsic blue dust color (the renderer applies the M1V star light). Vivid pack: `#acd6ff` |
...
2023 ([arXiv:2306.00077](https://arxiv.org/abs/2306.00077)) ran a TESS Sectors 1+27 flare census and
...
- **The edge-on debris disk** — by far the most photogenic feature
  ...
  `#e3f0ff` is BLUE — AU Mic's disk is measured blue in scattered
  light (Krist 2005, B/I ~ 1.6; tiny porous grains), reproduced by
  the Mie reflectance synthesis (B/I 1.74). The dust temperature of
  ~50 K is too cold for the disk's own thermal emission to be visible
  in the optical band, so this is pure blue-sloped scattering. The
  vivid cfg pack saturates it to `#acd6ff`; the renderer applies the
  red M1V star light on top.
