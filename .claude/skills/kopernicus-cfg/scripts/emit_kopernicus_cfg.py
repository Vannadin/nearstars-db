...
Two layers, two output files:
  * stars.cfg       — full star bodies: Properties + ScaledVersion
                      ...
                      from curated Teff/L/R (see star_render.py).
...
# DONE(v3): star bodies — Properties + ScaledVersion{Light, Material, Coronas} from Teff/L/R (star_render.py)
...
import star_render  # noqa: E402  (sibling module — Teff/L → ScaledVersion field synthesis)
...
def render_star_body_block(sr: StarRender) -> str:
    ...
    light_fields = star_render.light_block(sr.teff_k, sr.luminosity_lsun)
    light_inner = "\n".join(f"{j}    {k} = {v}" for k, v in light_fields.items())
    curves = star_render.intensity_curves_text(sr.luminosity_lsun, indent=j + "    ")
    light_node = f"{j}Light\n{j}{{\n{light_inner}\n{curves}\n{j}}}"
    material = star_render.material_block(sr.teff_k, sr.spectype, f"{tex}_Sunspots.dds")
...
def render_stars_combined(star_renders: list[StarRender]) -> str:
    lines = [
        ...
        "// Colors derived from Teff (blackbody→sRGB); luminosity = 1360 × L/Lsun (Sol convention).",
        "",
        MM_HEADER,
        "{",
    ]
