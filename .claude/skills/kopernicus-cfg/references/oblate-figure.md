...
- **Volume (resolved):** `a`/`b`/`c` are ratios on the PQS base radius and are all ≥ 1, so a base radius = the mean radius would **inflate** the body by `a·b·c` (Dante ×1.22). To conserve volume, set the **Kopernicus body `radius` (the PQS base sphere) = the polar radius** (= mean radius × `c_physical`); the ≥ 1 ratios then reproduce the true volume-preserving figure (it bulges one axis and contracts the others, Σ deviations ≈ 0). ⚠️ Do not confuse this with **Principia's `reference_radius`**, which is the *equatorial* radius used only to scale J2/C22 (gravity side); there the body's volume is set separately by `mean_radius` (the Jupiter/Saturn pattern: mean 69911 + reference 71492).
- **Real-pack precedent (RSS-scale, confirms the polar-base choice).** Sol (RSS-Reborn) and RSS-Origin make their giants oblate by **baking the shape into a ParallaxContinued scaled mesh** (`.bin` cache), NOT VertexHeightOblateAdvanced. Both set the PQS base `radius` to the **polar radius** and bulge the equator outward — RSS-Origin Jupiter `radius = 66854000` (= exact polar), Saturn `54364000` (= polar); Sol Jupiter `66316000` (1000-atm, just below polar). Their Principia J2 uses the **equatorial** `reference_radius` (Sol: Jupiter 71492 km, Saturn 60330 km) — the mean/reference split. So our two choices (Principia ref = equatorial; visual base = polar) match RSS practice. ⚠️ They gate oblate behind a toggle because **oblate giants clash with EVE/Scatterer up close** (Sol's own note) — relevant for Polyphemus (atmosphere giant): consider a toggle / verify the cloud+oblate interaction. For stock scale, the runtime `CustomEllipsoid` (polar base) is simpler than baking a mesh.
