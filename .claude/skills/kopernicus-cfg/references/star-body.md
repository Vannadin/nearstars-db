...
(→ `dist/…/Kopernicus/stars.cfg`), with the color/field math in
[`scripts/star_render.py`](../scripts/star_render.py):

- All `Light`/`Material` colors are derived from `Teff` (blackbody → sRGB,
  same method as `scripts/phase3/disk_color_mie.py`): M/K warm, G white,
  A/B blue-white.
