...
> 인류는 고대로부터 지금까지 하늘을 우러러 보아왔습니다. 닿을 수 없는 곳의 빛을 붙들어 숫자로, 표로, 논문으로 기록해온 수많은 선구자들이 있었습니다. 이 프로젝트는 그분들께서 평생을 다해 쌓아올린 관측과 계산을 바탕으로 하고 있습니다. 저는 저 깊은 하늘 너머를 탐구한 수많은 연구자들을 존경합니다. 이 위대한 거인들의 어깨에 올라설 수 있음에 감사드리며, 사용한 모든 자료는 출처를 명시하였음을 밝힙니다.
>
> 저는 스스로를 천문학자로도, 개발자로도 규정하지 않습니다. 그저 영화를 전공했고 SF를 좋아한 한 명의 게이머일 뿐입니다. 저는 AI를 사용해 제작한 이 "slop"들을 직접 개발했다고 생각하지 않으며, 다만 실력 좋은 개발자를 옆에 두고 불완전한 지식으로나마 별들로 향하는 길을 기획하는 프로듀서일 뿐이라고 여기고 있습니다.
>
...
항성계 소개, 방법론, 뷰어 등의 개요는 [문서 사이트](https://vannadin.github.io/nearstars-db/wiki/)에서 볼 수 있습니다(영어/한국어). 이 저장소에서 생성되므로 데이터와 함께 버전 관리되고 링크 검사도 받습니다. (GitHub 위키는 이제 이곳을 가리키는 현관일 뿐입니다.)
