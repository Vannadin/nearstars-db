...
### 중첩 및 강화
- **자율성 계열:** [[feedback_autonomy]](일 진행) + [[feedback_agent_token_saving]](무거운 읽기 위임) + [[feedback_star_addition]](사전위임 규칙)은 일관 자율성 철학 형성.
- **논문 규율:** [[feedback_derived_value_grounding]] + [[feedback_ads_paper_discipline]] 둘 다 동결-캐시, web 재검색 회피 강제.
- **큐레이션 깊이:** [[feedback_planet_curation]](두 단계) + [[feedback_skip_metallicity]](영향 주도 우선순위) 함께 자원 배치 형성.
- **흥미-우선 루프:** [[feedback_interesting_first_cascade]](값 연쇄) ↔ [[feedback_gameplay_variety]](풍성 편향) ↔ [[feedback_phase4_facet_choices]](사용자 옵션 주도).

### 잠재 충돌 또는 명료화 필요
1. **서브에이전트 비용 vs. 값-검증:** [[feedback_agent_token_saving]]은 동결 캐시로 서브에이전트 신뢰. [[feedback_audit_cost_discipline]]은 발견당 팬아웃 경고. 호환(동결 캐시 + 리포트별 배치 사용)이지만, 전환점(에이전트 vs. 메인 스레드 check.sh)은 향후 세션에 더 명확할 수 있음.

2. **흥미-우선 이론 override:** [[feedback_interesting_first_cascade]]는 "관측/이론 여전히 승리"지만 [[feedback_gameplay_variety]]는 "반드시-존재-금지 아니면 풍성 편향". 둘 다 참: 흥미-우선은 *지지*된 reading 선택, 미지지 아님. 이론-배제는 유일 hard constraint; 경계선/marginal 경우는 게임플레이로 쏠림.

3. **파생값의 메타데이터:** [[project_nearstars_db_principle]]은 파생에 기본값 금지; [[feedback_derived_value_grounding]]은 방법 인용. 충돌 아니지만 더 명확할 수 있음: 큐레이션 항목은 출처 인용(sources_extra); 파생은 재정당화 없이 변환.

### 정식화 권장사항
- **높은 우선순위(곧 CLAUDE.md 또는 CONVENTIONS.md 정식화):**
  - 섹션 1.1–1.9(의사소통 및 언어) 광범위 적용 가능 + 안정적
  - 섹션 2.1–2.5(자율성 및 워크플로우) portable 및 well-tested
  - 섹션 3.1–3.3(근거 및 논문) portable 및 반복 오류 감소

- **중간 우선순위(프로젝트 README 또는 nearstars 특정 문서):**
  - 섹션 4.1–4.6(NearStars 큐레이션). 좋지만 NearStars 특정; `NEARSTARS-CONVENTIONS.md` 또는 스킬 SKILL.md 파일로 승격 가능.

- **낮은 우선순위(메모로 유지, 필요 시 참조):**
  - 섹션 5(데이터베이스 및 파이프라인). 아키텍처적이며 거의 변경 없음; 메모 참조(및 빌더 코드 주석)로 충분할 수 있음.
  - 섹션 6(롤백 패턴). 향후 세션 경고; 재제안 방지 지속 메모로 가치 있음.

---

**초안 완료:** 2026-07-01
**통합 범위:** 21개 feedback_*.md 파일 + 5개 project_*.md 파일 + 2개 롤백 패턴
**단어 수:** 약 3,500단어
