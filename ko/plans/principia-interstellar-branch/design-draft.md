...
subsystem 소속은 바디 인덱스의 속성이므로, 디스패처 루프(`:1514-1548`)는 **subsystem 블록 단위의 쌍 범위**를 순회합니다. 즉 same-vs-cross는 **쌍마다의 검사가 아니라 루프 수준의 분기**라 핫패스 비용이 없습니다. 편평도/geopotential 분기(`:1381-1407`)도 같은 `Δq`를 소비하므로 저절로 함께 고쳐집니다.
