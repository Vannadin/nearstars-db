...
| ④ | **ModuleManager & ConfigNode** (패치 DSL, `:FOR` vs `:NEEDS`, 연산자, `:HAS`, ConfigNode) | [`modulemanager-confignode.md`](modulemanager-confignode.md) | **live** |
...
**2026-06-30 선택 범위:** **플러그인 작성 코어(①②③⑤)** — NearStars C# 플러그인을 막고 있는 진짜 갭 — 에 **④**(같은 날 MM 파서 소스로 근거화)까지. ⑥은 이미 커버됨.
...
- **KB 미확인 갭** (M/L, witness 대기) — `part.AddForceAtPosition`, `WaitForSeconds`/`WaitForFixedUpdate`, `[KSPScenario]` 등록 경로, MM 인덱스 apply-semantics + MM 변수(④ §8–9). 각 기둥 문서의 *Gaps* 절 참조. *(`VesselModule.OnSave`와 `[KSPScenario]` 인자는 이제 닫힘 — H.)*
