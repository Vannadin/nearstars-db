...
4. **근일점 기준 상태** (궤도면. `x̂` → 근일점, `ŷ` 90° 전방).
   ...
   n   = 2π / P
   vx_p = −(n·a/(1−e cos E)) sin E          vy_p = (n·a/(1−e cos E)) · √(1−e²) cos E
   ```
   단위. `a`는 AU, `P`는 년 → 속도는 AU/yr. 4.74047을 곱하면 km/s.
   (계수 주의. `n·a/(1−e cos E)`가 편심이각(E) 형태의 올바른 계수다. 진근점이각(ν)
   형태는 `n·a/√(1−e²)`에 `(−sin ν, e+cos ν)`를 쓴다 — ν형 계수에 `sin E / cos E`를
   섞으면 속도 크기가 `(1−e cos E)/√(1−e²)` 배 틀어지고, 거기서 나온 주기가 전부
   오염된다. 정확히 이 혼동이 2026-08-21까지 파이프라인에 실려 있었다.)
...
> **주의 (2026-08-21):** 이 예시는 *방법* 설명을 위해 Akeson 2021 입력셋을 그대로
> 유지한다. 현재 배포 DB는 상호 정합된 다른 세트를 채택했다 (π = 747.17 mas,
> a = 17.57″, P = 79.91 yr, M = 1.1055/0.9373 M☉ — Kervella 2016 / Pourbaix &
> Boffin 2016). 이 표의 숫자를 DB로 복사하지 말 것. (여담: 과거 오염된 DB 질량쌍
> 1.1055/0.9092는 Kervella의 M_A와 이 예시의 M_B가 뒤섞인 키메라였다.)
...
5. Perifocal:
     ...
     n·a/(1−e·cos E) = 2.4775/0.8350 = 2.9670 AU/yr,  with √(1−e²) = 0.8061
     x_p = 16.510 · cos(1.9526) = −6.155 AU
     y_p = 16.510 · sin(1.9526) = +15.321 AU
     vx_p = −2.9670 · sin(1.2881) = −2.849 AU/yr
     vy_p = (2.9670·0.8061) · cos(1.2881) = +0.6674 AU/yr
...
7. Sky-frame relative (B − A), Hilditch 컨벤션:
     ...
     vN = (−0.3390)(−2.849) + (−0.8023)(0.6674) = +0.966 − 0.535 = +0.430 AU/yr
     vE = (−0.8718)(−2.849) + (0.0714)(0.6674)  = +2.484 + 0.048 = +2.532 AU/yr
     vW = (0.3538)(−2.849) + (−0.5928)(0.6674)  = −1.008 − 0.396 = −1.404 AU/yr
