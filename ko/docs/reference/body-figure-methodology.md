...
유효하고 항성의 중심응집(NMoI ~0.05–0.08)에선 음수 J₂를 뱉으므로, 항성은 이 공식에 넣지 않습니다. 미미한 값으로 기록만 하고
emit하지 않습니다(느린 자전체는 모두 ~10⁻⁶ 임계 아래입니다).
...
중력과 시각 양쪽을 구동해 자동으로 일관됩니다. **하드 의존성**(노드를 쓰는 천체는 플러그인 없이는 형상이 안 나옴)이고
...
6. Emit 임계: ~1×10⁻⁶ 아래의 C₂₂(및 J₂)는 건너뜀(값은 기록하되 행은 omit) — 동역학적으로 무의미합니다.
