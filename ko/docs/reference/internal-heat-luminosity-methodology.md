...
이 값은 **NearStars에서 반복해 등장하는 도출값**이다. Alpha Centauri A b (Polyphemus)(α Cen A b), ε Ind A b, 그리고
...
Nimmo 해석 핵(시험 파일에만)이 Table 4 를 성분별로 10 % 안에서 재현하고 근은 인쇄값 4155 대비 4152 K 에 앉습니다. 프로파일은 CMB 에서
안쪽으로 RK4 로 적분하고 앵커가 있습니다. 중심압이 같은 천체에 대한 `interior.solve()` 자신의 `core_pressure` 와 맞아야 하고(지구
358.47 vs 358.46 GPa, ±0.1 %, 입력을 `interior.solve()` 출력에서 그대로 받아 전사 상수의 반올림이 못 들어옴; 안쪽 2 % 반경 0.03 % 는 적분하지 않음), 선언된 3760 K 에서 `core_state` 와 같은 내핵을 찾습니다(ICB 351.3 vs 351.4
GPa). 엔진 자신의 지구에서 풀린 T_c 는 3978 K 이고 거기서는 핵이 **전부 액체**입니다 — 고리를 닫으면 지구 내핵이 사라집니다
...
NearStars에서의 실무적 함의는 이렇다. **늙고 저질량인 거대행성은 냉각 광도가 희미해 복사 지배**(A b,
...
1. **암석 행성: 방사성 + 영년 냉각이 지배하지만, 결실은 `T_eff`가 아니라 내부다.** 내부 플럭스는 장수명
   ...
   Jaupart & Galson 1980; Davies 2013)다. 방사성 몫은 분모에 따라 다르다. Korenaga 2008
   ([`2008RvGeo..46.2007K`](https://ui.adsabs.harvard.edu/abs/2008RvGeo..46.2007K), 캐시됨)은 **벌크 지구 Urey 비 ≈ 0.35**(방사성/표면 총 열류)와
   **대류 Urey 비 0.23 ± 0.15**(방사성/대류 맨틀 예산 — §3.1: 대륙 열생산 7.5 ± 2.5 TW 를 BSE 열생산 16 ± 3 TW 에서
   빼면 대류 맨틀 8.5 ± 5.5 TW)를 따로 인쇄하며, 지오섬 소비처에 필요한 것은 후자다. 이는
...
3. **거대행성(0.3 ≲ M ≲ 13 M_J): Kelvin–Helmholtz 냉각 트랙(§2).** `L_int(M, age)`를
   Burrows/Baraffe/Fortney/Saumon-Marley 격자에서 읽어 온다. 늙고 + 저질량이면 → 희미 → 복사 지배(A b
   §7)이고, 젊으면 → 자발광(§3)이다.
...
**A b = α Cen A b(늙고 + 저질량 = 희미함).** **늙고(~5 Gyr), ~0.38 M_J(Saturn급)**인 온대 거대행성이다.
...
2013; Sclater+ 1980)로, 평균 표면 플럭스 **~0.087 W/m²**이다. 그중 방사성(U/Th/⁴⁰K 붕괴) 대 영년(잔여 강착열 +
핵/맨틀 냉각)의 비는 어느 예산을 분모로 삼느냐에 달린다(Korenaga 2008). **벌크 지구 ≈ 0.35**, **대류 맨틀
0.23 ± 0.15** — "현재 대류 열류의 약 20 % 만 방사성 원소에서 오고 나머지는 영년 냉각이 떠받친다". Korenaga 의
논지는 대류 비가 관례보다 *낮다* 는 것이라 범위와 이견이 정직한 내용이다. Nimmo & Primack 2020 의 "맨틀 안에서
약 15 TW" 는 Korenaga 의 8.5 ± 5.5 TW 맨 위에 걸린다 — 조정할 것이 아니라 실어 갈 불일치. (총량은 Pollack 1993 의
44 TW 를 Jaupart 2007 이 46 TW 로 고친 것으로 위 46–47 TW 와 맞고, 증가분은 전부 대륙. 그 논문의 20 ± 3 TW 는
McDonough & Sun 1995 의 값을 대조용으로 인용한 것이니 집어 오지 말 것.) 그 플럭스를 §1에 대입하면 `T_int = (0.087/σ)^(1/4) ≈ 35 K`로, 지구의
...
## 8b. 선언된 영역이 손실 법칙을 고른다

암석 천체의 맨틀은 자기가 가진 뚜껑을 통해 열을 버린다. 두 영역은 한 법칙의 두 설정이 아니라
**입력이 다른 서로 다른 법칙**이다. 그래서 천체가 선언한 `tectonic_regime` 이 어느 쪽을 돌릴지
고르고, 그 선택은 선언에서 따라 나오는 결과이지 수를 보고 추론한 것이 아니다.
...
**셋째 줄의 거절이 이 절의 요점이다.** 뚜껑 두께가 선언 안 된 정체뚜껑 천체도 기본값으로 때우면
mobile 쪽 법칙으로 돌릴 수 있고, 그러면 수가 하나 나온다 — **자기 영역과 모순되는 법칙으로 식은
수**이고, 출력 어디에도 그 사실이 안 적힌다. 레시피는 대신 이름을 대며 거절하고, 고쳐야 할 것은
빠진 선언 쪽이다.

⚠ **`mobile` 이 Nimmo 를 쓰는 것은 천체에 대한 판정이 아니라 엔진에 대한 서술이다.** 선언을 더 받지
않고 돌아가는 법칙이 그것일 뿐이고, mobile 천체에 대해 두 법칙을 비교해 본 적은 없다. 그러니 이
줄을 「mobile 은 그렇다고 판정했다」로 읽으면 안 된다.

⚠ **뚜껑 법칙의 한계: `n = 1` 의 식 (3) 은 두 표면온도를 구별하지 못한다.** `θ ∝ ΔT` 이고 `Ra_i ∝ ΔT` 인데 `Nu = a·θ^(−1−β)·Ra^β`, `β = 1/3` 이라 flux 는 `ΔT⁰` 로 간다 — 216 K 와 293 K 에서 **열여섯 자리까지 같은 수**로 측정됐다. **그러니 화성이 선언한 표면온도는 이 법칙의 답에 들어가지 않는다**. Nimmo 쪽은 실제로 반응한다. 그래도 두 법칙 다 선언을 요구한다 — 상쇄는 이 적합의 성질이지 법칙의 약속이 아니다.

⚠ **Foley 식 (3) 은 표면이 아니라 뚜껑 바닥을 지나는 flux 다.** 면적은 `4π(R_p − δ)²` 이고, 그
환산은 읽는 사람 머릿속이 아니라 코드 출력에 있어야 한다. 뚜껑 두께가 350 → 500 km 로 가면
뚜껑 바닥의 flux 는 그대로인데 표면 값은 **약 −9.6 %** 움직인다.

⚠ **등급 천장.** 맨틀이 가진 방사성 예산의 몫은 **선언**이다(70 %, 지구 부록 값을 모든 암석체에
옮긴 것). 어떤 선택 간선도 자기가 딛고 선 선언보다 높은 등급을 가질 수 없다. 법칙을 고르는 행위가
고르는 대상의 등급을 올려 주지 않는다.
