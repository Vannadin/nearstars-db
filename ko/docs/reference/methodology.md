...
#### `_principia_notes` 메타 블록

`binary_orbits.json` 의 최상위 `_principia_notes` 키 (언더스코어 접두사이므로 validator 가 스킵). 빌드에서 사용하는 에포크 상수 (`solar_system_epoch_jd`, `game_epoch_sol_jd`) 와 `schema_version` 문자열 ("YYYY-MM-DD-components-orbits") 을 기록합니다. `schema_version` 은 컴포넌트/궤도 shape 가 바뀔 때마다 갱신됩니다. 하위 도구 (예. `scripts/pipeline/build_systems.py`) 가 sanity check 용으로 읽을 수 있지만 권위 있는 값은 아닙니다 — 코드에 있는 상수의 사본일 뿐입니다.
