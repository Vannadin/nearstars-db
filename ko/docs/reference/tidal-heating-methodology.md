...
**Hades — 남겨둘 만한 반론 격파 둘(2026-07-28, 보드에서 옮겨옴).** 그럴듯하게 들리지만 이
...
1. *"내부열은 높지만 작은 강체 위성은 k₂/Q가 낮아 에너지가 용융 대신 텍토닉으로 흐른다"*는
   ...
   없다.
2. *"초기 조건에서 이심률을 낮추면 된다."* 훨씬 큰 이웃이 `e`를 강제하는 구조에서는(Hades:
   질량 770배의 Pandora, 주기비 2.22) 어떤 초기 `e`를 넣어도 안정성 시뮬이 같은 `e_max`를
