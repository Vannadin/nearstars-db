...
**집합**을 싣는다 — 인쇄된 열류와 총 플럭스가 양립하는 체제 전부, 배제된 것, 그리고 인쇄된 열류가
아무 말도 못 하는 것. **후보가 둘 이상인 동안 단일 체제 이름은 내지 않는다.**
**등급** — analog. `resurfacing_rate` 는 내지 않는다(인쇄된 식이 없다).
...
그들에게 그토록 결정적인 것이다.
> ⚠ **`~0.08 W/m²` 는 출처가 없다 — 2026-09-07 표시, 일부러 고치지 않았다.** 인용이 붙은 적이
> 없다. 이 문서 첫 커밋(`98774764`, 2026-06-23)에 그대로 들어왔고, 전 이력에 `git log -S` 를
> 걸어도 인용된 판본이 없다. 보유 추정 둘 — Korenaga 2008 의 16–20 TW = 0.031–0.039 W/m², 이
> 레포 `radiogenic.py` 기본 세트의 0.0415 W/m² — 의 **2.1배**이면서, 지구 **측정 총 표면열류**
> 0.0921 W/m² 의 **87 %** 다. 성분이 아니라 총합 쪽에 앉아 있으니 "방사성 가열만으로" 라는
> 라벨이 틀렸을 수 있다.
>
> ⚠ **그리고 이 문서가 같은 수를 다른 라벨로도 쓴다.** §5 의 Io 항목이 2 W/m² 를 *"지구의
> ~0.08 W/m² 보다 한 자릿수 높다"* 고 적는다 — 한정 없이 지구의 열류, 즉 총합이다. **같은 수가
> 한 번은 성분으로, 한 번은 총합으로 쓰인다.** §5 의 비교는 총합(0.0921)에 대해서만 성립하고,
> 틀린 쪽은 이 문단의 라벨이다.
>
> ⚠ 이 값은 **C34 가 고르는 후보 넷 중 하나**이기도 하다. 출처 없이 두 라벨로 쓰이는 수가 그
> 선택에 들어가 있다. 여기서 고치지 않는다 — 출처를 모르는 수를 추측으로 바꿔선 안 된다.
> [`engine/interior-core.md`](../../../engine/interior-core.md) 의 C47 참고.
...
- 배출 쪽 출처 몇 개는 arXiv 이전이거나 유료 장벽 뒤에 있어, 전문이 아니라 ADS bibcode와
  ...
  > **Kankanamge & Moore 2019는 2026-09-01에 확보·정독했고**(영문판의 서베이 ⑧b 블록),
  > 그때는 "관계식 + 알려진 천체" 기준을 수송 반쪽이 충족한다고 읽었다 — §6이 이오를
  > 내부온도 1471 K·리소스피어 12.6 km로 풀고 갈릴레오 관측과 맞는다고 하니까.
  >
  > **정정(2026-09-02, 브리프 35+37 — 위 독해는 당시의 기록으로 보존).** 그 주장은
  > 인쇄된 계가 지지하지 못하는 것으로 판명됐다. §4의 식들을 축자 배선해 기계 잔차까지
  > 풀면 Table 5의 어떤 자연 독법에서도 이오의 리소스피어가 논문 자신의 12.6 km에서
  > **8–42× 빗나가고**, 정확-근 역산(감사 재현, 유일성 확인)은 α ≈ 9×10⁻⁷ K⁻¹ —
  > 논문이 스스로 지명한 출처(Schubert et al. 2001; 박사논문과 Spencer+ 2021 Table 1이
  > 전달)의 3×10⁻⁵의 1/34 — 를 요구한다. 기제는 **모형이 애초에 무차원**이라는 것이다
  > (박사논문 Table 2.1이 α·k·T₀ 전부 1.0; 리드 두께 검증도 맨틀 깊이의 분율).
  > Table 5는 집필 시점의 재차원화이고 논문 자신의 §2와 모순된다(§2 "T₀ (= 1)" 대
  > Table 5 "1400 K"; 식 (2)는 자기 기준 ~8을 양쪽 독법에서 여섯 자리 빗나감). 축은
  > `engine/tidal_transport.py`에 배선돼 있고 `validation: failed-io-reproduction`
  > 라벨이 상시 붙는다. 기제 전문과 재개 조건(Khurana+ 2011의 유도 신호가 함의하는
  > ≥50 km·≥20 % 용융 전지구층을 위한 모형, 또는 이오의 리드를 보정이 아니라 예측하는
  > 논문)은 `engine/tidal-interior-context-notes.md` §7에 있다.
