...
**현재의 canonical한 단계 정의는
[pipeline-contract.md](pipeline-contract.md)에 있습니다** — Phase 1 (카탈로그
큐레이션) → Phase 2 (논문 인용 측정치) → Phase 3 (cfg-ready 합성) → Phase 4
(오너 아트 디렉션 게이트) → emit (결정론적 cfg 작성, 프로젝트 말미로 이연),
그리고 경계별 산출물, join key, 게이트가 그 문서에 정리되어 있습니다.

> 역사적 참고. 이 섹션은 원래 리빌드 이전 초안(첫 번째 시스템의 별 스켈레톤
> / 행성 / 비주얼 / 마무리)에서 온 다른 Phase 1–4 번호 체계를 담고 있었습니다.
> 그 번호 체계는 폐기되었으며, 이를 사용하는 문서가 있다면 DB-파이프라인
> 리빌드 이전 것입니다.
