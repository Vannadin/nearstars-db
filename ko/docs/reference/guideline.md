...
**목표.** KSP 1.12.x에 근접 항성계를 Sol 에코시스템 컨벤션에 따라 추가하며, Sol과 RSS 양쪽 모두와 호환성을 유지합니다.
...
이 모드는 새로운 항성계를 추가하며, Sol이나 RSS의 기존 천체는 수정하거나 교체하지 않습니다. Sol-Configs 또는 RealSolarSystem 중 하나가 의존성을 충족하며, 두 가지를 동시에 사용할 수 없습니다.
...
참고. RSS-Origin v1.x는 Sol을 지원하지 않습니다. Sol 지원이 포함된 v2는 개발 중이지만 아직 릴리스되지 않았습니다. 이 모드는 이중 호환 문제를 독립적으로 해결해야 합니다.
