...
- `phase3/stability-sim/scripts/build_viewers.py` — 배치 드라이버. 매니페스트를 읽어 각 시스템을 Principia 방식으로 재실행(skip-if-fresh)하고 정적 PNG를 두 팔레트로 + 3D 애니메이션을 렌더한 뒤 이중언어 갤러리(카드는 `prefers-color-scheme`에 따라 `<picture>`로 교체) `docs/phase4/orbit-viewers/index.html`를 씀. `--systems`·`--force`·`--quick`·`--gallery-only`.
