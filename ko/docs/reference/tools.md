...
- `scripts/refs/emit_atmosphere_color.py` — 임의 대기 조성(분자 몰분율, 예 `"CO2:0.95,N2:0.05"`) → 온도별 LTE 플라스마 색. 원자분율 변환 + 분자 밴드 자동선택 + 엔진 실행(스펙트럼 레벨 혼합 후 CIE). 혼합 대기 색을 얻는 올바른 방법(색이 아니라 스펙트럼을 합침). 발광 원소는 H/He/C/N/O, 나머지는 드롭+재정규화. `--html`로 스와치.
