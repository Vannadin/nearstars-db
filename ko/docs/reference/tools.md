...
- `scripts/refs/stellar_photospheric_color.py` — `docs/reference/stellar-photospheric-color-methodology.md`를 뒷받침하는 재생성 가능 도구. Teff·스펙트럼형 → 가시 sRGB 광구 색조를 공용 `cie_color.py` 엔진으로 계산. FGK·백색왜성은 Teff의 Planck 흑체를 쓰고, M왜성은 실측 Pickles 1998 SED(TiO/VO/H2O 밴드 → 벽돌빛 적색이 아니라 옅은 따뜻한 주황)를 씀. L·T·Y왜성은 BT-Settl / Morley+ 2014 모델 스펙트럼을 씀(구름 + 알칼리 공명선 날개 → T형은 적색이 아니라 마젠타). `colour_for_teff()` 가 400–40000 K 사다리를 천체 자신의 온도에서 보간하며, 분광형으로 구간을 나누지 않음. 스펙트럼은 VizieR / SVO 이론 서버에서 받아 `scripts/refs/.cache/{pickles,bt-settl,morley14}/`(gitignored)에 캐시. 파이프라인 자체검증은 G2V → ~#fff4f2, 1400 K → 마젠타.
