...
**Principia**가 태양계용으로 싣고 있는 실제 편평도 / 구면조화 중력 데이터입니다. 우리 천체(예: Alpha Centauri A b (Polyphemus))를 같은 방식으로 작성할 수 있도록 원본을 그대로 옮겨 두었습니다. 이 문서는 [`principia-cfg-reference.md`](principia-cfg-reference.md)가 다루는 cfg *스키마*에 대응하는 *데이터* 짝입니다.
...
> **거대행성들의 패턴에 주목.** 짝수 존 항이 강하고(J₂ ≫ J₄ ≫ J₆), 홀수 존 항은 무시할 만하며, tesseral(order > 0) 항은 없습니다 — 빠르게 도는 유체 천체는 매끄럽고 축대칭이기 때문입니다. J₄는 음수입니다(불룩한 부분이 순수 J₂보다 더 빨리 줄어듭니다). 이것이 바로 A b 같은 가스 거대행성이 따라야 할 형태입니다.
...
- **가스 거대행성 / 빠르게 자유 자전하는 천체**(A b) → 존 **J₂**(선택적으로 **J₄** ≈ J₂의 −4 %, **J₆**), tesseral 없음. 값이 큽니다. 목성 0.0147, 토성 0.0163. A b(~10시간 자전, 0.47 g/cc, 토성의 0.69보다 낮음) → **J₂ ≈ 0.018–0.025**. 형식: 스칼라 `j2`(선택적으로 `geopotential_row`로 J₄/J₆ 추가), `reference_radius` = 적도 반경.
