...
주기율표 원소별 불꽃 반응 / 저온 플라즈마 색상 사전. [Helmenstine 2017 차트](https://sciencenotes.org/flame-test-colors-and-procedure-chemistry/)는 장식용 그라디언트 때문에 정확한 hex 가 안 보여서, 화학 문헌 + [NIST 원자 스펙트럼 DB](https://physics.nist.gov/asd) 를 1차 출처로 쓰고 차트는 시각 sanity check 로만 활용합니다.
...
| 102 | **No** | Nobelium | — | 반감기 부족 | — | Longest-lived isotope t½ ≈ 58 minutes |
| 103 | **Lr** | Lawrencium | — | 반감기 부족 | — | Longest-lived isotope t½ ≈ 11 hours |

## 근거 자료

- [NIST 원자 스펙트럼 DB](https://physics.nist.gov/asd) — 모든 `cie_computed` hex 의 뒤에 있는 방출선 파장·상대 강도
- [Helmenstine 불꽃 반응 차트 (sciencenotes.org)](https://sciencenotes.org/flame-test-colors-and-procedure-chemistry/) — 시각 sanity check 용 (참조한 판은 2017년판)
- [db/refs/element_plasma_colors.yaml](../../../db/refs/element_plasma_colors.yaml) — 정본 데이터셋. 원소·영역마다 자체 `source:` 인용 문자열을 담고 있습니다 (NIST 라인 리스트, 화성 EDL 은 arXiv 2007.04869, 오로라·재진입 분광 문헌)
