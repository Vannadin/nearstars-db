...
**꼬리 길이 `L`은 엔진 인공물이고, 아직 정하지 않았습니다.** `L`은 유계 cfg 부피가 닫히는
지점입니다. Shue에는 대응물이 없습니다. 파라미터가 `r₀`와 `α` 둘뿐이고, 어떤 α도 유한한 끝점을
주지 않습니다(α < 0.5는 0으로 점근, α = 0.5는 반지름 `2 r₀`의 원통, α > 0.5는 발산. Winslow 2013이
같은 문턱을 명시합니다). 그래서 `L`은 도출할 수 없고, 시도했다 실패한 여섯 기준을 아래 표에 남깁니다.

지금 실려 있는 값은 **상속된 미근거값**입니다. `pause_extension`은 각 천체의 기존 꼬리를 보존하도록
역산했을 뿐이고, 외행성 넷의 그 기존 꼬리는 임의의 `rad`를 스톡 `ext`로 나눈 값이었습니다. 수성
(32 R_M)과 지구(200 R_E)는 스톡값 그대로입니다.

측정된 양이 둘 있는데, `L`과 혼동하면 안 됩니다.
...
30 R_E를 측정한 바로 그 지점에서 폭이 0이 됩니다. `L`은 측정 범위 밖에 있어야 하고, 멀수록 측정된
폭을 잘 재현합니다(지구에서 `L` 200은 −180에서 12.1 R_E, 실측 30에 크게 못 미치고, `L` 1500은
29.4입니다).
