...
**이 갈래는 Shue 형식으로 표현할 수 없습니다.** 기존 기계를 재사용하고 싶어지므로 분명히 적어
둡니다. Shue 계열에서는 `r(θ) = r0 (2/(1+cos θ))^α`라 **α 하나**가 주간면 벌어짐과 꼬리를 동시에
지배합니다. 그런데 유도 자기권은 그 둘이 분리돼 있습니다. 노즈→명암경계선 벌어짐은 *작고*
(금성 1.14/1.055 = 1.08) 꼬리는 *깁니다*(5~11 R_V). 꼬리가 주간면의 벌어짐이 아니라 끌려온
자기력선의 드레이핑과 질량적재로 만들어지기 때문입니다. 명암경계선에 맞춰 α를 잡으면 0.113이
나오는데, 그 α로는 11 R_V에 θ = 179.9964°에서야 닿습니다. 사실상 구이고 마지막 각도에서 급히
닫힙니다. 반대로 꼬리에 맞추면 주간면이 부풀어 오릅니다. 여기서는 Kerbalism의 눌린 구가 오히려
더 맞습니다. `compression`과 `extension`이 서로 독립이기 때문입니다. 그러므로 cfg 값은 도출하되,
유도 경계에는 Shue 기준선을 그리지 않습니다.
