...
않는 형상은 애초에 쓸 수가 없습니다. 측정된 원뿔에 폐곡선 인수를 곱합니다.

    ρ(d) = (ρ_term + s·d) · (1 − (d/X)^m)        d = −x ≥ 0,  d = X 에서 ρ = 0

`X` = **25 R_p**, `m` = **20** 입니다. 이 문서 다른 곳에서 Shue 형식을 닫는 `ε` 항과 같은 수법이고,
뭉툭해지지 않도록 의도한 구성입니다. `m`이 크면 인수가 측정 구간 내내 1에 붙어 있고(15 R_p 안쪽 오차
**0.00%**, 최외곽 측정점에서 **1.15%**), 지수를 ½ 대신 1로 두면 끝 접선이 유한해져 반구형 벽에 부딪는
대신 뾰족하게 잦아듭니다. 닫히는 지점*에서의* 형태는 어떤 물리적 주장도 담지 않습니다. 요구되는 것은
닫힘 그 자체뿐입니다.

`X`를 넉넉한 40이 아니라 금성의 가장 먼 확인 통과(20 R_V) 바로 밖인 25 R_p로 둔 것은 의도적입니다.
40이면 최외곽 측정점 오차가 0.78%로 좋아지지만, 확인된 범위의 두 배까지 경계를 주장하게 됩니다.
포기하는 1.15%는 0.04 R_V이고, 통과 위치의 불확실성보다 훨씬 작습니다. Edberg는 이 원거리 판정들에
~1시간 구간을 배정하며 10 R_V 밖 IMB 통과는 세 개뿐입니다.

`pause_extension`은 같은 `X`에서 따라 나옵니다. 금성 **0.0456**, 화성 **0.0588** 입니다.
