...
**야간면 α 는 별도 손잡이이고, 지구가 그것을 씁니다.** `α` = 0.5 가 정확히 원기둥 꼬리의
임계입니다. 폭이 `u^(1−2α)`(`u = π − θ`)로 가므로 0.5 아래면 꼬리가 스스로 닫히고 위면 한없이
벌어집니다. 이 임계는 추상이 아닙니다. Slavin 1985 가 지구 원거리 꼬리를 *원기둥*으로 측정합니다 —
`B_L` 9.2 nT 와 지름 60 R_E 가 120 에서 225 R_E 까지 일정하고, 그것이 α = 0.5 의 거동입니다.

그래서 지구는 `shue_alpha_night` = **0.52** 를 답니다(오너 아트 결정 2026-08-16). 임계 바로
위라 꼬리가 거의 원기둥으로 뻗다가 스스로 닫히고, `ε` 연화항에 기대지 않아도 됩니다. 대가는
측정됩니다. Slavin 이 x = −120 R_E 에서 재는 반경 30 에 대해 주간면 α 0.58 은 28.1(−6.4%),
0.52 는 21.9(−26.9%)를 줍니다. 0.48 안은 −40.1% 라 기각했습니다.
