...
**형상 함수는 주간면 원 + 야간면 원뿔입니다.** 이 경계에는 문헌 표준 형태가 있고, 그것은 Shue도
원뿔 단면(conic)도 아닙니다.

    주간면 (x ≥ 0):  노즈와 명암경계선에 맞춘, (x_c, 0) 중심 반지름 R의 원
    야간면 (x < 0):  직선  ρ = ρ_term + s·(−x)

Martinecz 2009([`2009JGRA..114.0B30M`](https://ui.adsabs.harvard.edu/abs/2009JGRA..114.0B30M))의 유도 자기권 경계(IMB) 모델이 바로 이것입니다.
"IMB 형상은 주간면의 원과 야간면의 직선으로 표현했다"는 그들의 서술 그대로이고, Edberg
...
Orbiter·BepiColombo·Parker Solar Probe 통과 자료로 검증해 **손볼 것 없이 최소 20 R_V까지 여전히
유효**함을 확인했습니다(재적합하면 기울기 0.101 → 0.097, 절편 1.13 → 1.10 R_V로, 20 R_V에서 경계가
0.1 R_V 움직이는 정도입니다). 이 행성들에서 원뿔 단면이 쓰이는 것은 *맞지만*, **bow shock**에
쓰이고 IMB에는 쓰이지 않습니다.

두 대안은 모두 실패합니다. 여기서 둘 다 먼저 시도해 봤으니 왜 실패하는지 남겨 둘 값이 있습니다.

**Shue로는 안 됩니다 — 파라미터 둘로 제약 셋.** `u = π − θ`로 두면 꼬리 폭이 `ρ ∝ u^(1−2α)`로
거동하므로 α = 0.5가 정확히 원통형 꼬리의 문턱입니다. 그 아래면 꼬리가 닫히고, 그 위면 한없이
벌어집니다. 그런데 같은 α가 명암경계선 폭 `ρ(90°) = r₀·2^α`도 함께 정합니다. 손잡이 하나가 두 일을
맡는 셈입니다. 금성에서는 이것이 치명적입니다. α 0.10은 실측 명암경계선 폭 1.13을 재현하고는 꼬리를
곧바로 닫아 버리고, α 0.58은 −20 R_V에서의 실측 폭은 맞히지만(3.16 대 3.15) 명암경계선을 40%
넘깁니다(1.58 대 1.13). 유도 경계는 **허리가 좁고 꼬리는 꾸준히 넓어지는** 모양인데, Shue의 α
하나로는 어떤 값에서도 이 둘을 갈라놓을 수 없습니다.

**원뿔로도 안 됩니다.** `r(θ) = L/(1 + e·cos θ)`를 초점 (X₀,0,0) 기준으로 노즈와 명암경계선에
맞추면 곡률 때문에 꼬리가 너무 넓어집니다. 포물선 극한 `e` = 1에서도 x = −20에서 5.05 R_V로,
실측 3.15를 크게 넘습니다. 꼬리를 닫으려고 `e`를 1 아래로 내리면 타원의 단반축이 꼬리 중간의
불룩함이 되어 명암경계선 폭의 1.5~1.8배까지 벌어지는데, 그 위치는 통과 자료가 하나도 없는 타원
중심입니다. 금성에서는 아예 파탄입니다. 노즈 1.0545, 명암경계선 1.1405, 꼬리 11 R_V로 풀어낸
타원은 **θ = 96°에서 0.9745 R_V까지 내려가 행성 표면 아래로 파고듭니다.**

채택값입니다.

| | 노즈 | 명암경계선 | 야간면 벌어짐 | 주간면 원 |
|---|---|---|---|---|
| 금성 | 1.055 R_V | 1.130 R_V | `s` 0.101 → **5.77°** | R 1.1327, 중심 x −0.0777 |
| 화성 | 1.285 R_M | 1.470 R_M | `s` 0.1314 → **7.49°** | R 1.4833, 중심 x −0.1983 |

금성은 전부 측정값입니다. 노즈와 명암경계선은 Brace 1980의 PVO 이오노포즈 고도에서, 벌어짐은
Edberg가 검증한 Martinecz의 적합 직선에서 왔습니다. 화성은 주간면만 Vignes
2000([`2000GeoRL..27...49V`](https://ui.adsabs.harvard.edu/abs/2000GeoRL..27...49V), Table 2, N = 488)에서 가져오고 야간면은 **가져오지 않습니다.**
그 논문 초록 자체가 "야간면 MPB 위치는 변동이 매우 크다"고 적고 있고, Němec
2020([`2020JGRA..12528509N`](https://ui.adsabs.harvard.edu/abs/2020JGRA..12528509N))은 MAVEN 기반 모델조차 "명암경계선 너머에서는 신뢰할 수 없다고
본다"고 합니다. 그래서 화성의 벌어짐은 금성의 벌어짐을 명암경계선 반지름으로 스케일한 값이고,
근거는 두 유도 자기꼬리 구조에 유의한 차이가 없다는 Phobos-2 / Pioneer Venus
비교([`2001AGUSM..SM32D06K`](https://ui.adsabs.harvard.edu/abs/2001AGUSM..SM32D06K))입니다. 이는 **유추이고 화성 측정치가 아니며**, 그렇게
라벨해야 합니다. 대신 요구하지 않은 검증 하나를 통과합니다. 그 각도에서 화성의 주간면 원은
명암경계선에 기울기 0.135로 도달하고 원뿔 쪽은 0.131이라, 구성 과정에서 아무도 맞춰 주지 않았는데도
3% 안에서 매끄럽게 이어집니다.

원은 표면을 파고드는 실패도 구성 자체로 제거합니다. 행성 중심에서의 거리가 노즈에서 명암경계선
폭까지 단조롭게 가므로, 노즈가 표면 위에 있는 한 그 안쪽으로 잘려 들어갈 수 없습니다.

**정직하게 남기는 한계 하나 — 이 경계는 닫히지 않습니다.** 5.8° 원뿔은 무한히 뻗고, 관측된 영역
안에서는 자료가 실제로 그렇게 말합니다. Kerbalism은 유계 부피를 요구하므로 엔진의
`pause_extension`이 **가장 먼 확인 통과 지점**에서 꼬리를 닫습니다. 금성은 20 R_V(Edberg 2024),
화성은 같은 유추를 따라 행성 반지름 기준 같은 거리입니다. 반면 오버레이는 관측 한계까지만 그리고
거기서 그냥 끊습니다. 닫힌 척 꾸미지 않습니다.

2-α Shue 구성은 측정된 경계도 발표된 α도 없는 천체를 위한 대체 수단으로만 렌더러에 남겨 두며, 그릴
때는 우리 것이라고 라벨에 표시합니다.
