...
### 어떤 경계가 생기고, 어떤 곡선이 그것을 그리는가

두 결정이 대개 같이 움직이는 탓에 자주 섞입니다. **무엇이 경계를 떠받치는가**(유도 대 고유)와
**어떤 곡선족으로 그리는가**(일반화 스톡 대 Shue)입니다. 이 둘은 독립이고, 한쪽에서 다른 쪽을
읽어내려다 잘못 판정한 적이 있습니다. 순서는 이렇습니다.

**0단계 — 장애물이 무엇인가.** 유도 경계를 만드는 것은 **전도성 전리층**이므로 대기가 있어야
합니다. 대기 없는 천체에는 이오노포즈라는 선택지 자체가 없습니다. 장애물은 자기장뿐이고 바닥은
표면 `1 R_p` 하나입니다.

**1단계 — 어느 경계가 이기는가.**

- **대기가 있으면** 판정식은 `R_mp(B_eq) > r_IMB`입니다(Egan 2019,
  [`2019MNRAS.488.2108E`](https://ui.adsabs.harvard.edu/abs/2019MNRAS.488.2108E)). `B_eq > 0`이 **아닙니다.** 넘으면 고유, 못 넘으면 유도이고,
  그 아래에서는 약한 쌍극자가 없느니만 못합니다.
- **대기가 없으면** 같은 판정식이 표면을 상대로 읽힙니다. `R_mp(B_eq) > 1 R_p`. 넘으면 고유,
  못 넘으면 **아무것도 생기지 않고** 항성풍이 지면에 닿습니다. 달의 경우입니다.

|  | 대기 있음 | 대기 없음 |
|---|---|---|
| 자기장이 바닥을 넘음 | 지구·목성(고유) | 수성·가니메데(고유) |
| 못 넘음 | **금성·화성·프록시마 센타우리 b**(유도) | 아무것도 안 생김(달) |

**수성이 금성·화성과 갈리는 지점이 여기**이고, 자기장 세기가 아닙니다. 수성의 쌍극자는 지구의
1/100 수준으로, 상대적으로 보면 화성에 남은 지각 잔류자기보다도 약합니다. 그런데도 수성은 고유
갈래이고 화성은 아닙니다. 수성은 대기가 없어 쌍극자가 내줄 이오노포즈 자체가 없고, 실측 standoff
1.45 R_M(Winslow 2013)이 마주하는 유일한 바닥을 넘습니다. 금성과 화성은 대기가 있어 이오노포즈가
~1.05~1.2 R_p에 서고, 그들의 자기장은 거기 못 미칩니다. 프록시마 센타우리 b도 같은 판정에서 유도
쪽입니다. N₂ 대기가 있고 자기장은 항성풍을 세워 막기에 약합니다(Dong 2017). 그래서 이 천체의 형상
비를 Shue가 아니라 화성 실측에서 가져옵니다.

**2단계 — α를 고른다**(고유 갈래에만 해당). 적합된 α가 있으면 그것, 없으면 유추임을 밝힌 값,
흐름이 아음속이면 `0.5`에 `M_A`로 단축한 꼬리입니다. 어느 천체가 무엇을 택했는지는 아래 유도 갈래
절의 표에 있습니다.

**3단계 — 형상을 인코딩한다.** 1단계에서 따라 나오지 **않는** 단계가 이것입니다.

- **유도** → 일반화 스톡. Shue가 그 경계를 기하적으로 못 그리기 때문입니다.
- **α = 0.5** → 역시 일반화 스톡인데, 이유가 정반대입니다. 그 α에서는 Shue를 **정확히** 재현하고
  닫힌 해로 `pause_waist` = 0입니다(`fit_generalized_pause.py`).
- **그 밖의 α** → 플러그인이 허용하는 곳은 Shue-native, 아니면 스톡 2계수 근사.

그러므로 인코딩은 물리의 대리물이 아닙니다. 금성과 가니메데가 둘 다 `pause_smooth`를 들고 있는데,
한쪽은 Shue가 실패해서이고 다른 쪽은 Shue가 정확해서입니다.
...
Kerbalism 보드에서의 실질적 귀결. 규칙은 "대기 없는 천체는 껍질 하나"가 **아닙니다.** 그건 사례
하나에서 과잉 일반화한 것입니다. 규칙은 **무엇이 그 틈을 파는지 이름을 댈 수 있을 때만 틈을 렌더한다**
입니다. 지구 슬롯은 이름이 있습니다(hiss, 플라스마권이 필요하므로 대기 없는 천체에는 없음). 토성·천왕성의
틈도 이름이 있습니다(특정 `L`에서 고리와 위성이 쓸어냄). 가니메데의 단일 껍질도 이름이 있는데, 틈 논거가
아니라 ~2 R_G 표준오프 때문입니다. 쓸어내는 위성을 가진 대기 없는 천체라면 여전히 파인 틈이 생기고,
서로 다른 두 공급원 집단을 가진 천체라면 여전히 껍질이 둘입니다. 대기가 없어서 잃는 것은 **hiss가 쓸어낸
슬롯** 하나뿐입니다.

파는 것이 없고 집단이 하나라면 **껍질 하나를 씁니다.** 포획 영역 전체를 한 번에 적합한 뒤
`gradient`를 하한 `*_radius / d_max`에 둡니다(Part C 그래디언트 레시피). 여기서 그 하한은 단지 합법
최소가 아닙니다. 평탄부가 껍질 내부 최심점 한 점으로 줄어드는 값이라, 껍질이 한 반경에서 봉우리를
치고 양쪽으로 선형으로 내려가는 토러스로 렌더됩니다. 하한 위로 올리면 꼭대기가 평평해지고, 아래로
내리면 벨트가 적어 둔 `radiation_*`에 어디서도 도달하지 못합니다. 껍질 하나에 그 gradient면 관측된
형태가 이미 나옵니다. 그러므로 두 번째 껍질은 이름 있는 틈이나 이름 있는 두 번째 집단으로 스스로를
정당화해야 하고, 지구 cfg와의 대칭성으로는 안 됩니다.

**두 껍질이 정말 필요할 때는** 겹치게 둡니다. 경계만 맞대면 안 됩니다. 각 껍질의 선량은 **자기** 경계에서 0부터 오르므로, 맞댄 두 껍질은
만나는 그 자리에서 둘 다 0입니다. 프록시마 센타우리 d의 첫 게이트 배치(`L` 1.0–2.5 대 2.5–5.0)가
양쪽 1000과 5000 사이에서 2.5 R_p에 5 rad/h로 떨어졌습니다. 어떤 공급원·손실 균형도 거기에 두지 않는
천 배짜리 홈입니다. 엔진이 껍질들을 **더하므로** 껍질 절반쯤 겹치면 그 구간이 양쪽에서 채워집니다.

**슬롯이 없을 때 봉우리가 어디에 놓이는가.** 지구 외대의 비율(~0.45 `R_mp`)을 목표로 삼지 마십시오.
지구 외대는 **아래쪽**이 슬롯으로 막혀 있고, 슬롯을 없애는 그 없는 hiss가 내부 영역의 주된 손실도 같이
없앱니다. 그래서 주입돼 안으로 확산하는 집단이 더 깊이까지 살아남고, 들어가면서 계속 에너지를 얻습니다
(제1 불변량 보존). 결국 대기 없는 천체에서 봉우리는 지구의 비율보다 **안쪽**에 놓이며, 적합한 단일
껍질이 지시받지 않고도 거기에 둡니다. 프록시마 센타우리 d의 봉우리는 0.35 `R_mp`로, 몸체에서 떨어져
있으면서 0.45보다는 한참 안쪽입니다.

사례는 프록시마 센타우리 d입니다. `L` 1.0–5.0에 적합한 껍질 하나(IoU 0.986), gradient 하한 1.005,
봉우리 5000 rad/h @ 2.65 R_p = 0.35 `R_mp`.
...
`R_mp(B_eq) > r_IMB`입니다. 그 아래면 유도 갈래로 다루십시오.

**이 부등식이 비교하는 면은 이온권계면이 아니라 유도 자기권 경계입니다**(2026-09-06 교정). Egan 자신은
전이를 *"자기 standoff 가 비자화 유도 자기권 경계(847 km)를 넘는 곳"* 에 두고, 그 논문 본문에 *ionopause* 라는
단어는 나오지 않습니다. 화성에서 둘은 다른 면이고 IMB 가 위입니다. Vignes 2000 은 이온권계면 신호를
*"고도 300 km"* 로 보고하며, 자기 파일업 경계를 그것과 활머리충격파 사이의 **세 번째** 경계로 이름 붙입니다.
수로는 화성 이온권계면 `1.089 R_p` 대 IMB `1.250 R_p` 이고, 두 번째를 위해 쓰인 부등식에 첫 번째를 넣으면
**전이 자기장을 1.13~1.69배 과소평가**합니다. 하필 이 갈래의 요지가 "약한 장이 없느니만 못할 수 있다"는
것이라, 판정이 반대로 갈릴 수 있습니다.

⚠ 이름이 흔들리기 시작한 곳은 여기가 아닙니다. Egan 은 MPB 와 IMB 를 정의 절에서 나눠 놓고, 정작 인용할 때는
제목이 자기 파일업 경계인 Trotignon 2006 을 *"비자화 유도 자기권 경계"* 로 부릅니다. 원문을 읽는 것만으로는
원문 자신이 흔드는 이름을 붙잡을 수 없습니다.

**이 경계는 상수가 아니라 항성풍의 함수로 발표돼 있습니다.** Ramstad 2018
([`2017JGRA..122.7279R`](https://ui.adsabs.harvard.edu/abs/2017JGRA..122.7279R))이 IMB 를 `n_sw`·`v_sw` 가 모는 이중원뿔로 적합했고, 공칭 태양풍에서
지하점이 `1.2417 R_p` 입니다. Egan 의 상수와 반지름 기준 0.66 % 안에서 만나므로, **둘 중 하나를 고르는 것이
아니라 한 값입니다.** 그 논문이 다루는 110배 압력 범위에서도 지하점은 1.42~1.16 R_p 사이만 움직입니다.
⚠ 인쇄된 단위 둘이 틀렸습니다. 구현 전에 [`paper-defects.md`](paper-defects.md) #20·#21 을 보십시오.

⚠ **`1.05–1.2 R_p` 는 이 문서에 남고 이 부등식에서만 빠집니다.** 그것은 *이온권계면* 고도 범위이고 다른 양이며,
그 자격으로는 근거가 충분합니다. 금성 측정 셋이 1.0545·1.1157·1.1652 R_p 로 환산되고 화성 이온권계면 1.089 R_p
도 같은 구간 안에 듭니다. Ramstad 의 이온권 경계 공칭 `1.195 R_M` 이 그 상단 바로 아래 앉는데, 이는 두 천체의
이온권 경계가 어디쯤인지에 대한 관찰이지 IB 와 이온권계면을 같은 면으로 취급해도 된다는 허가가 아닙니다.

⚠ **이 모형은 구성상 화성 전용입니다.** 변수가 `n_sw`·`v_sw` 둘뿐이고 중성 대기는 변수가 아니라 적합 상수 안에
흡수돼 있으며, 논문이 그렇게 명시합니다 — 대기 변동의 효과는 *"평균되어 상쇄된다고 가정"* 합니다. 로스터 천체에
쓰면 화성 대기를 빌려 쓰는 것이고, Egan 의 `B_max ∝ P_sw^(1/2)` 같은 **다른 행성으로 옮기는 규칙을 이 논문은
주지 않습니다.**
...
### 피크를 가지는 껍질 — 지구식 배분은 일반화되지 않는다

위 레시피가 주는 것은 세기의 **크기**와 외/내 **비**인데, 그 비를 가져온 두 앵커가 모두 피크를
**깊은** 껍질에 둡니다. 지구 내대가 10.4 rad/h이고 바깥은 2.2, 목성도 내대가 제 원반을 압도합니다.
이걸 곧이곧대로 읽으면 어떤 천체에서든 "멀수록 안전하고 낮으면 죽는다"가 됩니다. 일반화되지
않습니다. 이유는 자기장이 아니라 공급원 쪽에 보입니다.

지구 내대는 **CRAND** 벨트입니다. 우주선이 **대기**를 때리고, 튕겨 나온 알베도 중성자가 붕괴해
양성자로 포획됩니다(Lenchek 1961, [`1961JGR....66.4027L`](https://ui.adsabs.harvard.edu/abs/1961JGR....66.4027L)). 대기를 걷어내면 그 공급원이
사라지는 것은 아니지만, **크기를 모르는 상태**가 됩니다. 표토도 우주선 표적이고 그 알베도 중성자는
일상적으로 측정됩니다. 궤도 중성자 분광으로 대기 없는 표면을 지도화하는 일 자체가 그것으로 됩니다
(수성: Lawrence 2013, [`2013Sci...339..292L`](https://ui.adsabs.harvard.edu/abs/2013Sci...339..292L)). 없는 것은 그 공급원이 실제로 포획 벨트를
**만든** 사례입니다. 확인할 수 있는 대기 없는 자화 천체 둘이 모두 무관한 이유로 실패합니다. 수성은
자기권이 너무 작고 요동쳐서 애초에 포획을 못 하고, 가니메데는 ~2 R_G 표준오프라 어느 쪽이든 껍질 하나
자리밖에 없습니다(Williams 1998, [`1998JGR...10317523W`](https://ui.adsabs.harvard.edu/abs/1998JGR...10317523W)). 그러니 대기 없는 천체의 깊은 CRAND
벨트는 **없는 것이 아니라 크기를 모르는 것**으로 다룹니다. 한편 깊은 껍질을 우선적으로 채워 줄 다른
것은 없습니다. 항성풍과 확산이 먹이는 집단은
**바깥** 경계에서 주입돼 들어오면서 에너지를 얻으므로 피크가 중간 `L`에 놓입니다. 지구 외대부터가
`L` 4–5, 즉 `R_mp`의 ~0.45 지점에서 피크입니다(Schulz & Lanzerotti 1974,
[`1974pdrb.book.....S`](https://ui.adsabs.harvard.edu/abs/1974pdrb.book.....S)).

지금 확보된 계외 관측도 강한 장 천체에 대해 같은 말을 합니다. Kao 2023
([`2023Natur.619..272K`](https://ui.adsabs.harvard.edu/abs/2023Natur.619..272K), Nature 619, 272)이 초저온왜성 LSR J1835+3259의 방사선대를 8.4 GHz에서
**해상**했습니다. 쌍엽 축대칭 구조이고 논문이 명시적으로 "목성 방사선대와 형태가 비슷하다"고
적으며, 두 엽의 간격이 최대 18 반경입니다. 몸체에서 ~9 반경 떨어져 선 토러스가 1년 넘는 세 관측에
걸쳐 안정적으로 있었고, 전자 에너지는 ~15 MeV입니다. Climent 2023
([`2023Sci...381.1120C`](https://ui.adsabs.harvard.edu/abs/2023Sci...381.1120C), Science 381, 1120)이 갈색왜성에서 같은 그림을 보고합니다. 어느 쪽도
지구 형태가 아닙니다.

**규칙.** 외/내 비를 적힌 그대로 쓰는 것은 깊은 껍질에 CRAND 공급원이 있는 경우, 즉 우주선이 때릴
대기가 있는 경우로 한정합니다. **대기가 없는** 천체에서는 뒤집습니다. 보간으로 얻은 세기를
**외대**에 주고, 깊은 껍질이 비를 받습니다. 크기는 그대로입니다. 이 규칙이 정하는 것은 피크가
**어디**에 놓이냐이지 얼마나 크냐가 아니며, 결과는 관측된 형태 그대로 안쪽이 조용한 토러스이지
지면으로 내리꽂는 단조 경사가 아닙니다.

**작업 사례 — Proxima d (16 G SPI 극자기장, 항성풍/플레어 공급, 토러스 없음).**
B_eq = 800 µT → 앵커 보간이 ~5×10³ rad/h를 주고, 위의 무대기 규칙에 따라 그 크기는 **외대**가,
0.2 비인 ~1×10³은 깊은 껍질이 가집니다(d는 맨바위라 내대를 실어 줄 CRAND 공급원이 없습니다).
...
`fit_belts.py`로 껍질 하나에 다시 적합해(`L` 1.0–5.0, IoU 0.986) 보드에 게이트했습니다.
...
- **Lawrence et al. 2013**, Science 339, 292 ([`2013Sci...339..292L`](https://ui.adsabs.harvard.edu/abs/2013Sci...339..292L)); **Williams et al. 1998**,
  JGR 103, 17523 ([`1998JGR...10317523W`](https://ui.adsabs.harvard.edu/abs/1998JGR...10317523W)). 대기 없는 천체의 CRAND 벨트가 없는 것이 아니라
  크기를 모르는 것인 이유(궤도 중성자 분광이 되는 것 자체가 표토가 알베도 중성자를 내기 때문입니다),
  그리고 가니메데의 단일 껍질이 슬롯 논거가 아니라 표준오프 결과인 이유입니다.
- **Kao et al. 2023**, Nature 619, 272 ([`2023Natur.619..272K`](https://ui.adsabs.harvard.edu/abs/2023Natur.619..272K)); **Climent et al. 2023**,
  Science 381, 1120 ([`2023Sci...381.1120C`](https://ui.adsabs.harvard.edu/abs/2023Sci...381.1120C)). 해상된 유일한 계외 방사선대와 갈색왜성 쪽
  대응 사례입니다. 몸체에서 ~9 반경 떨어져 선 쌍엽 목성형 토러스, ~15 MeV 전자, 1년 넘게 안정.
  대기 없는 강한 장 천체에서 피크를 외대에 두는 근거입니다.
