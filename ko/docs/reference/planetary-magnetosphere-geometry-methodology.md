...
**형상 함수는 Shue가 아니라 원뿔(conic)을 씁니다.** 여기서는 Shue 계열이 맞는 도구가 아니고, 무엇이
맞는지는 문헌에 이미 있습니다. Shue는 주간면 벌어짐과 꼬리를 α 하나에 묶으므로, 금성의 실측
벌어짐(1.14/1.055 = 1.08)을 넣으면 꼬리 11 R_V에 θ = 179.9964°에서야 닿는 구가 됩니다. 그것을
때워 쓰는 것(ε로 꼬리를 닫고, 명암경계선 뒤에서 α를 바꾸기)도 동작하지만 손잡이를 만들어 내는
일입니다. 그런데 **이 천체들의 경계는 실제로 원뿔 단면으로 적합됩니다.**

    r(θ) = L / (1 + e·cos θ)        초점은 (X₀, 0, 0)

Vignes가 화성 MPB에 쓴 바로 그 형식이고, 행성 장애물 일반에 쓰이는 표준입니다(Slavin & Holzer
1981, [`1981JGR....8611401S`](https://ui.adsabs.harvard.edu/abs/1981JGR....8611401S)). 이 갈래에서 Shue 계열보다 나은 이유가 셋입니다.

1. **스스로 닫힙니다.** `e < 1`이면 타원이라 꼬리가 `r(180°) = L/(1−e)`에서 닫히고 추가 항이
   필요 없습니다. 엔진도 유계 부피를 요구하는데, 여기서는 그 닫힘이 적합된 이심률에서 나옵니다.
   억지로 얹는 것이 아닙니다.
2. **`X₀`·`e`·`L`이 분리해야 할 것을 분리합니다.** 노즈·명암경계선 폭·꼬리 길이라는 제약 세 개가
   파라미터 세 개에 각각 얹힙니다. 유도 갈래에 필요한 자유도이고, α 하나에는 없는 자유도입니다.
3. **데이터가 적합된 그 형식 그대로**라 재매개화 과정에서 왜곡될 여지가 없습니다.

채택값. **화성**은 Vignes 2000의 발표 원뿔을 그대로 씁니다(`X₀` 0.78, `e` 0.90, `L` 0.96).
**금성**은 발표된 원뿔이 없으므로 실측 제약 세 개(노즈 1.0545, 명암경계선 폭 1.1405, 꼬리 11 R_V)로
풀어 `X₀` 0.7071, `e` 0.9424, `L` 0.6748을 얻었습니다. 세 제약을 정확히 만족합니다.

**이 결정이 강제하는 정정.** 이전 개정본은 2-α 형태에서 꼬리 중간이 불룩해지는 것을 피하려고
`α_night` 0.34를 채택했습니다. 큰 값이 꼬리 폭을 명암경계선의 1.1~1.4배로 넓힌다는 이유였습니다.
그 기준이 틀렸습니다. Vignes 자신의 원뿔은 x ≈ −5 R_M에서 **2.20 R_M, 명암경계선 폭의 약 1.5배**
까지 벌어졌다가 닫힙니다. 하류에서의 벌어짐은 적합된 경계의 실제 특징이고(금성에서 Pioneer
Venus가 보고한 "고도에 따라 벌어짐"도 같은 이야기입니다. Saunders 1986,
[`1986JGR....91.5589S`](https://ui.adsabs.harvard.edu/abs/1986JGR....91.5589S)) 억눌러야 할 인공물이 아닙니다. 원뿔은 이것을 튜닝 없이 재현합니다.

**정직하게 남기는 한계 하나.** 타원의 원거리 꼬리 닫힘은 측정이 아니라 외삽입니다. Vignes는
`X'` ≈ −2 R_M 너머에서 MPB가 "약하고 점진적인 전이"로 나타난다고 적었고, 그들의 통과 자료는
주간면과 명암경계선에 몰려 있습니다. 노즈와 플랭크는 측정이고 꼬리 모양은 적합의 연장입니다.

2-α Shue 구성은 원뿔 적합도 발표된 α도 없는 천체를 위한 대체 수단으로 렌더러에 남겨 두며, 그릴
