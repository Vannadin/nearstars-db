...
**꼬리 길이는 측정된 곳만 고치고, 없는 곳은 두었습니다.** 꼬리는 **벌어짐이 멈추는 지점**까지로
잡습니다. 그 뒤로는 일정 반지름 원통이라 형상이 더 변하지 않고, 거기서 자르는 비용이 없습니다. 그
거리가 측정된 천체는 둘이고, 둘 다 그 값으로 설정했습니다.
...
두 논문 모두 그 지점의 꼬리 폭을 함께 주는데, α로 세운 면이 맞춥니다. x = −3 R_M에서 Winslow는
거의 원통형 꼬리 반지름 ~2.7 R_M을 보고하고 우리 면은 **2.71**이며, x = −120 R_E에서 Slavin은
지름 60 ± 5 R_E를 보고하고 우리 면은 반지름 **28.1**입니다. 이 폭들은 맞춘 값이 아니라 α에서
그냥 나온 값입니다.

목성·토성·천왕성·해왕성은 물려받은 임시값을 그대로 둡니다(목성 것은 계산하면 1512 R_J). **외삽할
근거가 없기 때문입니다.** 두 앵커가 노즈 배수로 2 R_ss와 10~12 R_ss로 5~6배 어긋나는데 α가 설명하는
몫은 3.6배뿐이고, 문헌도 간극을 메우지 못합니다. Winslow는 "the factors determining the location
where tail flaring ceases are not well understood"라고 적으며 재결합으로 플라스마 시트가 끊기는
거리를 지목합니다. 그것은 자속 수송 문제이지 관계식을 세울 압력 균형이 아닙니다. 이전 개정판은
지구로 보정한 12 × 노즈 규칙을 제안했는데, 수성 앵커가 그것을 반증합니다. 그 규칙이 내세운 목성
"독립 검증"도 양을 잘못 잡은 것이었습니다 — Kurth 1981의 ">700 R_J"는 짧은 조우의 하한이고,
Lepping 1983([`1983JGR....88.8801L`](https://ui.adsabs.harvard.edu/abs/1983JGR....88.8801L))은 목성 꼬리가 **최소 9000 R_J**에 이른다고
기록합니다. Voyager 2가 ~4.5 AU까지 검출했고(Kurth 1982,
...
**SOI보다 긴 꼬리는 틀린 게 아니라 불활성입니다.** `Radiation.Compute`는 부모 사슬만 거슬러 오르고
(`body = body.referenceBody`) SOI 검사가 없으므로, 비행체가 어떤 천체의 영향권을 벗어나면 그 천체가
사슬에서 빠지고 필드는 평가되지 않습니다. 따라서 SOI 밖의 값은 비용도 주장도 없습니다.
