...
맞닿게 두는 것이 아니라 겹쳐야 합니다. 여기가 함정입니다. 각 껍질의 선량은 **자기** 경계에서 0으로
시작해 올라갑니다(`gradient·(−SDF)/radius`). 그래서 경계만 맞댄 두 껍질은 만나는 그 지점에서 둘 다
0이고, 프로파일이 이음매에서 붕괴합니다. 프록시마 센타우리 d의 첫 게이트 배치(`L` 1.0–2.5 대
2.5–5.0)가 양쪽 1000과 5000 사이에서 2.5 R_p에 5 rad/h로 떨어졌습니다. 어떤 공급원·손실 균형도
거기에 두지 않는 천 배짜리 홈입니다. 엔진이 두 껍질의 기여를 **더하므로**, 겹쳐 두면 그 구간이
양쪽에서 채워지고 봉우리가 하나인 매끄러운 프로파일이 나옵니다. 규칙 둘이 따라옵니다.

- **껍질 하나의 절반쯤 겹칠 것.** 그러면 각자의 경사가 상대 안쪽에 충분히 들어갑니다.
- **`gradient`는 각자의 하한** `*_radius / d_max`**에 둘 것**(Part C 그래디언트 레시피). 하한값은
  평탄부가 껍질 내부 최심점 한 점으로 줄어드는 값, 즉 한 반경에서 봉우리를 치고 양쪽으로 선형으로
  내려가는 프로파일입니다. 지금 모형화하려는 토러스 형상이 바로 그것입니다. 하한 위로 올리면 평탄한
  꼭대기가 생기고, 아래로 내리면 벨트가 적어 둔 `radiation_*`에 어디서도 도달하지 못합니다.

사례는 프록시마 센타우리 d로, 내대 `L` 1.0–3.5에 외대 `L` 2.0–5.0, gradient 1.157과 1.496입니다.
합쳐진 봉우리가 0.46 `R_mp`에 놓이고 내부 홈이 없습니다.
...
`fit_belts.py`로 다시 적합해(`L` 1.0–3.5과 2.0–5.0 겹침, IoU 0.988 / 0.977) 보드에 게이트했습니다.
