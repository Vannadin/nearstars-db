...
**이것으로 해결되지 않는 하나.** `pause_extension`은 각 천체의 기존 꼬리 길이를 그대로 보존하기
위해서만 다시 계산했고, 그 꼬리 길이는 근거 없이 물려받은 임시값입니다(목성 것은 계산하면 1512 R_J
가 됩니다). 꼬리 길이는 별개의 문제로 미뤄 두었습니다. 판정 기준은 lobe 압력이 주변 항성풍 정압까지
떨어지는 지점이고, 지구에서는 Slavin 1985([`1985JGR....9010875S`](https://ui.adsabs.harvard.edu/abs/1985JGR....9010875S))가 벌어짐이 멈추는
곳을 120 ± 10 R_E, 원거리 중성선을 100–140 R_E로 잡습니다 — 지금 우리가 쓰는 200 R_E와 어긋납니다.
