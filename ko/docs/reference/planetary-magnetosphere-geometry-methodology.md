...
**그 형식은 구현하지 않습니다.** 자기화된 천체용과 유도 천체용으로 형상 계열을 둘씩 게임에 들고
가는 비용이 그만한 값을 하지 않는다고 판단했습니다. 대신 배포되는 Kerbalism pause 함수를, 값이 0이면
스톡을 그대로 재현하는 필드 둘로 제자리에서 일반화합니다(오너 결정, 2026-08-14). 필드와 그 도출은
Part C의 ⚗ 절에 있고, 값은 이렇습니다.

| | radius | compression | extension | smooth | waist |
|---|---|---|---|---|---|
| 금성 | 1.14 | 1.0197 | 0.0567 | 0.5 | 0 |
| 화성 | 1.47 | 1.1063 | 0.0737 | 0.5 | 0 |

구속 조건은 **경계가 명암경계선 뒤에서 불룩해지지 않아야 한다**는 것입니다. 폭이
`√(radius² − px²)`라 `radius`를 넘을 수 없으므로, `radius`를 실측 명암경계선 폭으로 두면 후방
불룩함이 구조적으로 불가능해지고 어깨 뒤쪽 단면은 단조롭게 줄어듭니다. 그러면 노즈와 명암경계선이
정확히 떨어지고 꼬리는 20 R_p에서 닫히며 불룩함은 0.000%입니다. 대가는 후류 폭입니다. 실측 원뿔과
비교해 2 R_p에서 15%, 5에서 33%, 10에서 54% 좁습니다.

이것을 채택하기 전에 대안을 하나씩 다 측정했습니다. 다시 시도하지 않도록 남깁니다.

| 후보 | 노즈 | 명암경계선 | 후류 | 판정 |
|---|---|---|---|---|
| 스톡 무수정 | −15% | +47% | −47% | 측정값 둘이 모두 틀림 |
| Shue, α = log₂(comp) | 0.00% | +0.9% | −83% … −100% | 꼬리가 행성 뒤 2 R_p에서 사라짐 |
| Shue, α ≈ 0.44 (최악오차 최소) | 0.00% | +23% | ±31% | `pause_alpha`와 `pause_compression`이 서로 다른 주간면을 기술하게 됨 |
| 연화 Shue, α와 L 적합 | 0.00% | +32.5% | −32.5% | 이 계열의 최선. `L ≫ r₀`이면 ε ≪ 0.5라 명암경계선이 `r₀·2^α`로 주저앉음 |
| 초점 기준 원뿔 단면 | — | — | +60% | 타원으로 닫으면 0.9745 R_V까지 내려가 **표면 아래로 파고듦** |
| 원 + 원뿔 + 폐곡선 캡 | 0.00% | 0.00% | 0.00% | 정확하지만 자기만의 형상 계열을 요구 |
| **일반화한 스톡** | **0.00%** | **0.00%** | −15% … −54% | 채택 |

Shue가 구조적으로 이보다 잘할 수 없는 이유입니다. `u = π − θ`로 두면 꼬리 폭이 `ρ ∝ u^(1−2α)`라
α = 0.5가 정확히 원통형 꼬리의 문턱인데, *같은* α가 명암경계선 폭을 `r₀·2^α`로 함께 정합니다. 손잡이
하나가 두 일을 맡는 셈이고, 유도 경계는 좁은 허리와 넓어지는 꼬리를 동시에 요구합니다.

화성은 주간면만 Vignes 2000([`2000GeoRL..27...49V`](https://ui.adsabs.harvard.edu/abs/2000GeoRL..27...49V), Table 2, N = 488)에서 가져오고
야간면에서는 아무것도 가져오지 않습니다. 그 논문 초록 자체가 "야간면 MPB 위치는… 변동이 매우 크다"고
적고, Němec 2020([`2020JGRA..12528509N`](https://ui.adsabs.harvard.edu/abs/2020JGRA..12528509N))은 MAVEN 시대 모델조차 "명암경계선 너머에서는
신뢰할 수 없다고 본다"고 합니다.
...
**일반화한 스톡 pause** (`pause_waist`, `pause_smooth`). **권장** 경로이고, 유도 천체가 지금 쓰는
방식입니다. 형상 계열을 하나 더 붙이는 대신, 배포된 함수의 결함 둘을 고칩니다. 배포된 함수는
이렇습니다.
...
노즈(1.055 / 1.285)와 명암경계선(1.13 / 1.47)이 정확히 떨어지고, 꼬리는 20 R_p에서 닫히며 불룩함은
