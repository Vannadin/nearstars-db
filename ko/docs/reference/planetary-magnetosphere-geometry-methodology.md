...
| 흐름이 아음속(sub-Alfvénic) | **알펜 날개**(아래) | 가니메데, A b III |

### 네 번째 형상 계열 — 알펜 날개

아음속 장애물은 bow shock 도 쓸린 꼬리도 만들지 않습니다. 그래서 위 세 계열이 하나도 안 맞습니다.
...
**기울기.** 날개는 배경 자기장에서 `arctan M_A` 만큼 기웁니다. 파동이 `B₀` 를 따라 `v_A` 로 가는
동안 흐름이 `v₀` 로 하류로 실어 나르기 때문입니다(Neubauer 1980,
[`1980JGR....85.1171N`](https://ui.adsabs.harvard.edu/abs/1980JGR....85.1171N). Saur 리뷰
[1908.06446](https://arxiv.org/abs/1908.06446) §4.2 가 이 형태로 인용). 가니메데는 25.6°, 두 날개가 128.8° 로
벌어집니다.

**관 반경 = 장애물 반경.** 플럭스 보존에서 바로 나옵니다. 반경 `R_obst` 인 장애물이 `B₀πR_obst²` 를
배제하고 바깥 자기장은 균일하니, 그 플럭스를 나르는 관도 같은 반경입니다. 같은 리뷰 Figure 3 캡션도
그려진 날개를 "the size of the source" 라고 씁니다. Saur 2013 식 57 의 유효 반경 `R_eff = √3 R_obst`
는 **다른 양**입니다 — 표면이 아니라 포인팅 플럭스의 크기입니다.

**직선 구간.** 날개는 모천체 자기력선을 따르고, 그 선은 자기 접선에서 대략 `ℓ²/2R_curv` 만큼
벗어납니다. 이걸 관 반경 하나와 같게 두면 `ℓ = √(2 R_tube R_curv)` 이고, 쌍극자 적도에서
`R_curv = r/3` 입니다. 곡선 경로에서 실측한 이탈 지점을 가니메데 18%, A b III·A b I 3% 이내로
재현합니다. 날개가 직선인 건 전체 주행의 5% 뿐입니다.

**허공에서 끝나지 않습니다.** 그 뒤로 관은 자기력선을 타고 휘며 `1/√B` 로 좁아져, 모천체 전리층에
오로라 발자국으로 착지합니다 — 가니메데 기준 68 km(아래 바닥값 적용 전). 하류 변위는 평행이동이
아니라 모천체 자전축 둘레의 **방위각 회전**입니다. 평행이동으로 넣으면 끝이 전리층을 78 R_moon
벗어나 허공에 뜹니다. 누적 처짐은 가니메데 12.5° 인데, Bonfond 2009
([`2009JGRA..114.7224B`](https://ui.adsabs.harvard.edu/abs/2009JGRA..114.7224B)) 는 측정된 lead angle 이 상호작용
모델 검증에 신뢰할 만한 양이 아니라고 경고합니다.

**착지.** 날개가 끝나는 전도체는 면이 아니라 두께를 가진 층이라, 전리층 스케일 높이
`H = kT/(μ m_u g)` 가 두 가지를 정합니다 — 관 반경의 바닥값(관이 자기가 녹아드는 층보다 가늘 수는
없습니다)과 이음매 필렛입니다. 상층 전리층 조성인 원자 H, 1000 K 로 잡으면 목성 335 km, 폴리페무스
889 km. 바닥값이 무는 지점은 가니메데가 주행의 91%(68 → 335 km), A b I 가 99%, A b III 는 발자국이
이미 층보다 넓어 안 뭅니다. 더 좁게 잡을 근거는 없었습니다 — 68 km 는 거기 이온 관성길이의 94배이고,
자기력선이 표면 법선에서 7.6° 기울어 들어가 발자국 타원이 0.9% 늘어나는 정도입니다.

장애물 자체는 **구** 그대로입니다. 구속은 어디서나 자기압이고 상류에만 램압이 더해지는데 그 비가
`2M_A²` 이며, standoff 가 `p^(-1/6)` 이므로 주야 비대칭이 `(1 + 2M_A²)^(1/6)` — `M_A` 0.5 에서 8%,
그보다 느려지면 더 작아집니다.

**이제 모든 α를 전문으로 검증했습니다**(논문은 `docs/phase3/_papers/`에 캐시해 두었습니다). 그리고
천체마다 `pause_compression`을 `2^α`로 두어 `log₂(compression)`이 α를 정확히 되돌려주게 했고,
`pause_radius`는 노즈 × `2^α`입니다.

| 천체 | α | 노즈 | 측면 = 노즈·2^α | 출처 |
|---|---|---|---|---|
| 수성 | 0.500 | 1.45 R_M | 2.051 | Winslow 2013 ([`2013JGRA..118.2213W`](https://ui.adsabs.harvard.edu/abs/2013JGRA..118.2213W)) — `R_ss` 1.45 R_M, 평균 `P_Ram` 14.3 nPa에서 벌어짐 0.5 |
| 지구 | 0.580 | 10 R_E | 14.948 | Shue 1998 ([`1998JGR...10317691S`](https://ui.adsabs.harvard.edu/abs/1998JGR...10317691S)) 식 (11): `α = (0.58 − 0.007 Bz)[1 + 0.024 ln Dp]` |
| 목성 | 0.423 | 63 R_J | 84.465 | Rutala 2025 ([2502.09186](https://arxiv.org/abs/2502.09186)) S97* Table 2: `α = 0.28 + 1.08 p_SW`, `r_SS = 38.0 p_SW^−0.25` |
| 토성 | 0.736 | 24 R_S | 39.968 | Kanani 2010 ([`2010JGRA..115.6207K`](https://ui.adsabs.harvard.edu/abs/2010JGRA..115.6207K)) 식 (12): `α = 0.73 ± 0.07 + (0.4 ± 0.5) Dp` |
| 천왕성 | 0.580 | 18 R_U | 26.907 | **유추** — 지구의 α, 근거는 아래 |
| 해왕성 | 0.580 | 26.5 R_N | 39.614 | **유추** — 지구의 α, 근거는 아래 |

토성은 제대로 된 교차검증이 되는 경우입니다. Kanani의 `a₄` = 0.4 ± 0.5는 0과 구별되지 않으므로 α는
사실상 압력과 무관한 0.73인데, 앞선 Arridge 2006 피팅
([`2006JGRA..11111227A`](https://ui.adsabs.harvard.edu/abs/2006JGRA..11111227A))은 압력 항의 부호가 *반대*인 `α = 0.77 − 1.5 Dp`입니다. 그런데도
우리가 채택한 노즈 24 R_S에서 두 식은 각각 **0.7356과 0.7358**로 떨어집니다. 계수도 다르고 통과
자료 집합도 다른 두 피팅이 0.0002까지 일치하는 셈입니다. 주 출처는 Kanani로 잡았습니다. 통과 수가
64개 대 191개이고, 자기권 플라스마 압력을 포함하며, RMS가 더 큰 것은 회귀 악화가 아니라 자료 집합의
차이이기 때문입니다(논문 자체가 A06의 계수를 새 통과 자료에 적용하면 RMS 3.82로, 새 모델의 3.603
보다 크다고 밝힙니다).

**천왕성과 해왕성이 지구의 α를 유추로 가져오는 까닭.** 두 행성 모두 Shue 형식의 피팅이 없고, 피팅된
네 기준점은 외삽을 지지하지 않습니다. 동압 순으로 늘어놓으면 14.3 nPa → 0.500, 1 → 0.580,
0.132 → **0.423**, 0.0146 → 0.736입니다. 목성이 순서를 깨뜨립니다 — 중간 압력에서 α가 가장 낮습니다 —
α가 항성풍만큼이나 내부 플라스마 적재에 좌우되고, 목성의 Io 토러스가 벌어짐을 억누르는 magnetodisc를
부풀리기 때문입니다. 그러니 외삽할 α(압력) 법칙이 애초에 없습니다. 대신 논증할 수 있는 것은 *적재*
쪽입니다. Voyager 2가 관측한 자기권 중 가장 텅 빈 것이 얼음거대행성 둘이었습니다. 천왕성에서는
"천왕성 위성들은 유의미한 플라스마 공급원으로 보이지 않으며" 밀도 최대가 2 cm⁻³ 근처였고(Bridge
1986, [`1986Sci...233...89B`](https://ui.adsabs.harvard.edu/abs/1986Sci...233...89B)), 해왕성에서는 최대 밀도가 1.4 cm⁻³로 "Voyager가 어떤
자기권에서 관측한 것보다도 작은" 값이었습니다(Belcher 1989, [`1989Sci...246.1478B`](https://ui.adsabs.harvard.edu/abs/1989Sci...246.1478B)). Io급이나
Enceladus급 공급원이 없으니 적재 수준은 거대행성보다 지구에 가깝고, 따라서 지구의 α가 변론 가능한
대역입니다. 어디까지나 **피팅이 아니라 유추**이며, 등장하는 자리마다 그렇게 표기합니다.

**꼬리 길이는 `L` = 150 × 노즈, 명시된 관례입니다.** `L`은 유계 cfg 부피가 닫히는 지점입니다.
Shue에는 대응물이 없습니다. 파라미터가 `r₀`와 `α` 둘뿐이고 어떤 α도 유한한 끝점을 주지 않습니다
(α < 0.5는 0으로 점근, α = 0.5는 반지름 `2 r₀`의 원통, α > 0.5는 발산. Winslow 2013이 같은 문턱을
명시합니다). 그리고 자기꼬리의 물리적 끝은 어느 행성에서도 측정된 적이 없습니다. `L`을 도출하려던
여섯 시도는 아래 기각 표에 있습니다.

그래서 관례로 정하되 조건 하나를 겁니다. **어떤 실측과도 모순되지 않을 것.** 발표된 꼬리 도달
범위는 전부 탐사 범위가 만든 하한이고, 그중 가장 먼 것이 목성의 최소 9000 R_J입니다(Lepping 1983,
[`1983JGR....88.8801L`](https://ui.adsabs.harvard.edu/abs/1983JGR....88.8801L)). 노즈 63 R_J의 142.9배이므로, 올림해서 **`L` = 150 × 노즈**로
둡니다. 기록된 하한 대부분을 넘습니다. 목성 143×, 금성
19×(Edberg 2024), 수성 2.1×입니다. **다만 지구는 예외입니다.** 여기 쓰던 지구 값은 ISEE-3의
225 R_E였는데, 실제 기록은 **~3,100 R_E**입니다(Intriligator 1979,
[`1979GeoRL...6..585I`](https://ui.adsabs.harvard.edu/abs/1979GeoRL...6..585I), Pioneer 7 의 꼬리 연관 플라스마. "지자기꼬리가 확장돼 있다는
가장 먼 관측 증거"). 그 사이에 자력계 500 R_E(Mariani 1969,
[`1969JGR....74.5633M`](https://ui.adsabs.harvard.edu/abs/1969JGR....74.5633M))와 플라스마 탐침 ~1,000 R_E(Wolfe 1967,
[`1967JGR....72.4577W`](https://ui.adsabs.harvard.edu/abs/1967JGR....72.4577W))가 있습니다. 즉 `L` = 150 × 노즈는 지구에서 1,500 R_E이고
**결국 어떤 실측과는 모순됩니다.** 두 배 차이이고, 자기장 검출이 아니라 플라스마 공동 검출에
대해서입니다. 그럼에도 관례는 유지합니다(오너 결정 2026-08-16). 정직한 진술은 이렇습니다 —
모든 *자기* 검출은 넘어서지만, 지구의 *플라스마* 검출 하나에는 못 미칩니다.

| 바디 | 노즈 | `L` | AU 환산 |
|---|---|---|---|
| 수성 | 1.45 R_M | 217.5 | 0.004 |
| 금성 | 1.055 R_V | 158.2 | 0.006 |
| 지구 | 10 R_E | 1500 | 0.064 |
| 화성 | 1.285 R_M | 192.8 | 0.004 |
| 목성 | 63 R_J | 9450 | **4.516** |
| 토성 | 24 R_S | 3600 | 1.450 |
| 천왕성 | 18 R_U | 2700 | 0.458 |
| 해왕성 | 26.5 R_N | 3975 | 0.654 |

비율이 모든 바디에서 같으므로 특정 천체가 튀지 않습니다. 순서는 그냥 자기권 크기 순서입니다. 목성의
4.516 AU는 태양에서 9.72 AU 지점에 닿아 토성 궤도 9.58 AU를 넘는데, 이것이 실제로 관측된
현상입니다. Voyager 2가 하류 ~4.5 AU에서 목성 꼬리를 검출했고(Kurth 1982,
[`1982JGR....8710373K`](https://ui.adsabs.harvard.edu/abs/1982JGR....8710373K)) 토성 자신이 그 안에 잠겼습니다(Desch 1983,
[`1983JGR....88.6904D`](https://ui.adsabs.harvard.edu/abs/1983JGR....88.6904D)).

가니메데에는 적용하지 않습니다. 장애물이 항성풍이 아니라 목성 플라스마 흐름 속에 있는 알프벤 날개
영역이기 때문입니다. NearStars 바디에는 2026-08-16 재게이트부터 **적용합니다**. 폴리페무스 `L` 5300 R_p,
Proxima b 231 R_p, Proxima c 1791 R_c입니다. 그쪽 소스는 여전히 phase 4 보드이고, 이제 이 값들을 담고 있습니다.

**`L` 도출 시도 여덟 번이 전부 실패했고, 여덟 번째가 실패할 수밖에 없음을 확정했습니다.**
꼬리는 흐려지지 않습니다. 로브장이 압력균형으로 고정돼 하류로 약해지지 않고, 로브는
Kelvin-Helmholtz 안정이며, 대비 `B_lobe / B_IMF` 는 `sqrt(2) * M_A` 로 꼬리를 따라 상수입니다.
유일한 실제 기제는 위상적 절단(목성에서 관측)이고, 그것이 주는 길이는 계산했으나 채택하지
않았습니다. 자속 예산·heliotail 결과와 기각된 모든 기준을 포함한 전체 기록은 도출 기록
([solar-system-radiation-belts.md](solar-system-radiation-belts.md#why-the-tail-has-no-derivable-length))에 있습니다.

**`L`이 아닌 것.** 측정된 거리가 둘 있는데 혼동하면 안 됩니다.

| 바디 | 벌어짐이 멈추는 곳 | 출처 |
|---|---|---|
| 수성 | 2.9 R_M (2 R_ss) | Winslow 2013 — "the downstream flaring of Mercury's tail ceases by ~2 R_ss" |
| 지구 | 120 R_E (12 R_ss) | Slavin 1985 — 벌어짐 종료 \|X\| = 120 ± 10 R_E |

이것은 **형상**이 변하기를 멈추는 지점이지 꼬리가 끝나는 곳이 아닙니다. 그 뒤로 경계는 일정 반지름
원통으로 이어집니다. `L`을 그 거리로 두는 것은 시도했고 틀렸습니다. 지구에서는 Slavin이 반지름
30 R_E를 측정한 바로 그 지점에서 폭이 0이 됩니다. `L`은 측정 범위 밖에 있어야 하고, 150 × 노즈
관례가 정확히 그것을 보장합니다.

두 앵커는 α로 세운 **형상**은 확인해 줍니다. 맞춘 값이 아닌데도 x = −3 R_M에서 Winslow의 거의
원통형 반지름 ~2.7 R_M에 우리 면이 **2.71**이고, x = −120 R_E에서 Slavin의 지름 60 ± 5 R_E에
우리 면이 반지름 **28.1**입니다.

발표된 꼬리 도달 범위는 전부 탐사 범위가 만든 **하한**이지 끝점이 아닙니다. 가장 큰 것이 목성으로
최소 9000 R_J = 143 `r₀` 입니다(Lepping 1983,
[`1983JGR....88.8801L`](https://ui.adsabs.harvard.edu/abs/1983JGR....88.8801L). Voyager 2가 ~4.5 AU까지 검출했고(Kurth 1982,
[`1982JGR....8710373K`](https://ui.adsabs.harvard.edu/abs/1982JGR....8710373K)) 토성 자신이 그 안에 잠겼습니다. Desch 1983,
[`1983JGR....88.6904D`](https://ui.adsabs.harvard.edu/abs/1983JGR....88.6904D)). 어느 실측과도 모순되지 않으려면 `L`을 143 `r₀` 밖에 두어야
합니다. `L = 150 r₀`이 채택된 관례입니다(오너 결정 2026-08-14). `r₀`에만 의존하므로
검증되지 않은 관계식을 물려받지 않습니다.

금성과 화성이 마지막 줄에 들어가는 까닭은, Shue의 단일 α로는 어떤 값을 넣어도 좁은 허리와 벌어지는
꼬리를 동시에 잡을 수 없기 때문입니다. 실측한 실패 양상은 아래 표에 있습니다. 두 천체의 필드와 그
도출은 Part C의 ⚗ 절에 있고, 값은 이렇습니다.
...
정확히 떨어지고 꼬리는 158 / 193 R_p(150×노즈 관행)에서 닫히며 불룩함은 0.000%입니다. 대가는 후류 폭입니다. 실측 원뿔과
비교해 2 R_p에서 15%, 5에서 33%, 10에서 54% 좁습니다.

이것을 채택하기 전에 대안을 하나씩 다 측정했습니다. 다시 시도하지 않도록 남깁니다.

모든 대안 형상족을 채택 전에 실측했습니다. 비교표는 도출 기록
([solar-system-radiation-belts.md](solar-system-radiation-belts.md#induced-boundary-shape-families-measured-and-rejected))에 있습니다.
...
이 임계를 겨냥해 야간면 α 를 따로 두는 안을 2026-08-16 에 시험했다가 롤백했습니다. 경위는 도출 기록
