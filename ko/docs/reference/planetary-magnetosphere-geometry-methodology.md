...
**일곱 번째 시도, 2026-08-16: 경계가 흐려지지 않으므로 흐려지는 지점도 없습니다.**
안팎 대비가 흐려지는 자리에서 꼬리를 끊고, 약장 천체는 강장 천체보다 상대적으로 짧은 꼬리를 갖게
하자는 발상이었습니다. 계산 셋이 이를 기각합니다.

로브 자기장은 압력균형으로 정해집니다. `B_lobe = sqrt(2 mu0 P_ext)` 이고, 그 궤도 거리의 주변
항성풍에만 의존하며 **행성 자기장과는 전혀 무관합니다**. 그래서 하류로 가도 약해지지 않습니다.
지점마다 같은 외부 압력에 맞춰 다시 균형을 잡을 뿐입니다.

로브는 Kelvin-Helmholtz 안정입니다. 로브장이 흐름과 나란하므로 불안정 조건
`dv^2 > (1/mu0)(1/rho_1 + 1/rho_2)(B_1^2 + B_2^2)` 이 요구하는 임계 전단이 지구 원거리 꼬리에서
1540~1880 km/s인데, 자기외피 흐름은 ~400 km/s입니다. 경계층이 난류로 두꺼워지지 않으니 혼합층
기준이 꼬리 반경을 결코 다 먹지 못합니다(Miura 1987, [`1987JGR....92.3195M`](https://ui.adsabs.harvard.edu/abs/1987JGR....92.3195M);
Walker 1981, [`1981P&SS...29.1119W`](https://ui.adsabs.harvard.edu/abs/1981P%26SS...29.1119W)).

대비 자체도 스케일에 무관합니다. 두 식을 나누면 `B_lobe / B_IMF = sqrt(2) * M_A` 로, 두 자기장이
같은 방식으로 감소하므로 **꼬리를 따라 상수**입니다. 지구 13.5배, 목성 14.7배, 폴리페무스 13.5배.
꼬리 내부는 끝까지 주변보다 한 자릿수 세게 남습니다.

부산물 하나는 남길 만합니다. `sqrt(2) * M_A` 는 자기장 모델 없이 계산되는 값싼 대비 진단값입니다.
프록시마 b가 **4.5배**로, 다른 천체의 13~15배와 대조됩니다. 항성풍이 거의 알펜 속도라(M_A ~ 3)
그렇습니다. b의 꼬리는 짧은 구조는 아니지만 실제로 희미한 구조입니다.

이 자리에서 함께 철회합니다. 그 시도의 중간 단계에서 발표된 꼬리 도달거리로부터 전단층 성장률
`S = R_T / L` 을 역산하고, 그 분산(지구 0.13, 목성 0.014)을 꼬리 길이의 물리적 차이로 읽었습니다.
아닙니다. 모든 `L` 이 탐사 범위 한계이므로 모든 `S` 는 상한이고, 그 분산이 재는 것은 각 임무가
얼마나 멀리 갔는가이지 각 꼬리가 얼마나 긴가가 아닙니다.

**진짜 종료 기제는 존재하고, 확산이 아니라 위상적입니다**(에이전트 3인 문헌 조사, 2026-08-16).
꼬리는 흐려져 사라지는 게 아니라 *잘립니다*. Kurth 1982([`1982JGR....8710373K`](https://ui.adsabs.harvard.edu/abs/1982JGR....8710373K))가 원거리
목성 꼬리의 "pinch-off 또는 disconnection"을 추측했고, Goldstein 1985([`1985JGR....90.8223G`](https://ui.adsabs.harvard.edu/abs/1985JGR....90.8223G))가
확인했습니다. 그 사건 중 Voyager 1 태양풍 자료에 행성간 섹터 통과가 나타났고, "Kurth et al.
(1982)이 예측한 대로다. 이는 꼬리가 목성에서 끊어졌다는 그들의 추측을 지지한다"고 적습니다.
혜성 disconnection event의 행성판입니다. 같은 논문이 6000~7500 R_J 원거리 꼬리의 자기장 스펙트럼이
`f^−5/3`으로 주변 태양풍과 구분되지 않음을 보고합니다.

여기서 계산 가능한 길이가 나옵니다. 붙어 있는 꼬리는 행성이 전류시트를 통과하는 간격만큼만 자랄 수
있으므로 `L = v_wind × T_sector` 이고, `T_sector` 는 두-섹터 패턴(항성 자전/2)과 공전의 회합주기입니다.
태양계 행성은 전부 **3 AU** 근처에 떨어집니다(섹터주기 12.7~13.5일, 풍속 400 km/s). 원거리 검출이
실제로 모여 있는 자리이기도 합니다 — 목성 4.5 AU, 혜성 Hyakutake 3.8 AU. NearStars 값은 폴리페무스
2.58 AU, 프록시마 b 12.83, c 35.5, d 4.89입니다.

**계산했고 의도적으로 채택하지 않았습니다**(오너 결정 2026-08-16). 이유가 둘입니다. 자이언트에서는
`150 × 노즈`와 일치하지만(목성 6157 대 9450 R_J, 폴리페무스 5398 대 5300) 소형 천체에서는 두세
자릿수 벌어집니다. 노즈 비례가 아니라 절대거리이기 때문입니다. 금성이 158에서 76,868 R_V가 되고,
그 `extension` 1.5e-5 는 눈에 보이게 닫히지 않는 원기둥입니다. 그리고 이 기제는 벽이 아니라
*듀티 사이클*입니다. 잘린 꼬리는 계속 흘러가고, 그래서 혜성 153P가 하류 6.5 AU에서 검출됐으며
(Jones 2022, [2006.00500](https://arxiv.org/abs/2006.00500)) Jones 는 일부 이온꼬리가 "태양권 가장자리까지 알아볼 수 있는
구조로 살아남을 수 있다"고 주장합니다.

**자속 예산도 꼬리를 끝내지 못합니다.** "유한한 열린 자속이 다 재결합되는 곳이 끝"이라는 전제가
실측으로 무너집니다. Slavin 1985([`1985JGR....9010875S`](https://ui.adsabs.harvard.edu/abs/1985JGR....9010875S))는 |X| = 120 ± 10 R_E 에서 벌어짐이
멈추고 `B_L` 9.2 nT 와 지름 60 R_E 가 **225 R_E까지 일정**하다고 보고합니다. 로브 자속이 빠져나가는
게 아니라 극관 전량 그대로 평평합니다. Milan 2004([`2004JGRA..109.7210M`](https://ui.adsabs.harvard.edu/abs/2004JGRA..109.7210M))가 그 모델을
그래도 만들었는데, 길이가 "몇 시간 만에 ~400에서 4000 R_E 사이로 거의 10배 변할 수 있다"고 하며
"그 너머로 훨씬 더 긴 끊긴 꼬리와 후류가 존재할 수 있다"고 덧붙입니다. 세 시간에 3,600 R_E를
움직이는 값은 경계가 아닙니다. 다만 쓸모 있는 내부 표지는 남습니다 — Dungey 연결 꼬리와 끊긴 후류의
경계로 지구 기준 400~4000 R_E이고, 우리 1500이 그 안에 들어갑니다.

**이 문제를 실제로 푼 유일한 분야가 heliotail 연구인데, 그 해법은 우리에게 전용되지 않습니다.**
Izmodenov & Alexashov 2003([astro-ph/0308211](https://arxiv.org/abs/astro-ph/0308211))이 우리와 똑같은 말로 난점을 적습니다 —
"heliotail에서는 heliopause를 태양권 경계로 가정할 수 없다 … 태양풍이 하류 공간 전체를 채운다."
그리고 숫자는 오직 성간 중성수소와의 전하교환을 불러와야 나옵니다. 밀도·접선속도 불연속이 **~3,000
AU**에서 사라지고, 플라스마가 **20,000~40,000 AU**에서 성간값에 수렴합니다. 가져갈 문장은 이겁니다 —
"상류 방향과 달리, 태양계 경계는 heliotail에서 **확산적 성격**을 가진다." 행성 자기꼬리는 완전
전리된 자기외피 속에 있어 그런 중성 흡수원이 없고, 그래서 행성에 대응하는 숫자가 존재하지 않습니다.
