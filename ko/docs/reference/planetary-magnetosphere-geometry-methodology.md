...
꼬리가 벌어지는 정도를 정합니다.** 금성은 `α_day` 0.113에 `α_night` 0.5면 꼬리가 ~7 R_V까지
1.2~1.5 R_V 폭을 유지하다 11에서 닫힙니다. Pioneer Venus가 보고한 "고도에 따라 벌어지는" 꼬리와
맞습니다(Saunders 1986, [`1986JGR....91.5589S`](https://ui.adsabs.harvard.edu/abs/1986JGR....91.5589S)).

**`α_night`에 대한 정직한 표시.** 이 값은 고정되지 않았습니다. `r0`·`α_day`·`L`은 측정에서
오지만, `α_night`은 "고도에 따라 벌어진다"는 정성 서술을 재현하려고 고른 값입니다. 꼬리 폭 대
거리 수치를 아직 확보하지 못했기 때문입니다. 도출량이 아니라 기본값(0.5)이 명시된 형상 손잡이로
다루고, 꼬리 반경 프로파일이 생기면 그때 고정하십시오.
