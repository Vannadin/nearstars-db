...
이 문서 다른 곳에서 쓰는 *연화* Shue 형식에는 꼬리 길이 `L`이라는 세 번째 파라미터가 있으니 공정하게
시험할 값이 있습니다. 그런데 이것도 구조적으로 실패합니다. 폐곡선 항
`ε = 1/((L/r₀)^(1/α) − 1)`은 `L ≫ r₀`이면 무시할 만큼 작아지고, ε ≪ 0.5가 되는 순간 명암경계선 폭은
다시 `r₀·2^α`로 돌아갑니다. α와 `L`을 전수 격자로 금성의 세 제약에 맞춰 보면 최소가 **32.5% 오차**
입니다. α 0.505, `L` 103에서 명암경계선 +32.5%, −20 R_V 폭 −32.5%로 차이를 반씩 나눠 갖는 것이
α 하나로 할 수 있는 최선입니다. 세 번째 파라미터가 사 오는 것은 닫힘이고 허리가 아닙니다. 채택한
형식은 자기 몫의 폐곡선 파라미터 하나를 쓰고 **0.8%** 에 도달합니다.

유도 경계는 **허리가 좁고 꼬리는 꾸준히 넓어지는** 모양인데, Shue 계열의 어떤 구성원도 어떤 파라미터
값에서도 이 둘을 갈라놓지 못합니다.
...
**닫는 것은 선택이 아니라 필수 단계입니다.** 5.8° 원뿔은 무한히 뻗고, 관측된 영역 안에서는 자료가
실제로 그렇게 말합니다. 그런데 이 형상의 소비자는 엔진입니다. 유계 cfg 부피가 되어야 하므로, 닫히지
않는 형상은 애초에 쓸 수가 없습니다. 측정 한계 `d₀`까지는 원뿔을 그대로 두고, 그 뒤에만 캡을 씌웁니다.

    d ≤ d₀ :  ρ = ρ_term + s·d                          (측정된 원뿔, 손대지 않음)
    d > d₀ :  ρ = (ρ_term + s·d)·(1 − u^k),  u = (d−d₀)/(X−d₀)

`d₀` = **20 R_p**, `X` = **40 R_p**, `k` = **2** 입니다. "자료가 어디서 끝나는지"와 "캡이 얼마나
긴지"를 분리하는 것이 핵심입니다. 파라미터 하나로 묶은 `(1 − (d/X)^m)` 형식은 이 둘이 붙어 있어서
꺾임을 앞으로 당길 때마다 측정 구간이 망가집니다. 꺾임을 길이 절반에 두면 20 R_p에서 **23%** 오차가
납니다. 분리하면 측정 구간이 정확해지고(인수가 그 구간에서 항등적으로 1이므로 **0.00%**), 닫힘이
*시작되는* 지점이 전체 길이의 정확히 절반이 되며, 최대폭은 d ≈ 25에 놓입니다. `k` ≥ 2면 이음부가 값과
기울기 모두 연속이면서 끝 접선은 유한하게 남아, 반구형 벽에 부딪는 대신 뾰족하게 잦아듭니다.

금성의 폭 표본입니다. 명암경계선 1.13, 10에서 2.14, **20에서 3.15**(마지막 측정점), 25에서 3.43,
30에서 3.12, 35에서 2.04, 40에서 닫힙니다.

20 → 40 R_p 구간은 **경계에 대한 주장이 아닙니다.** 선언된 캡이고, 인식적 진술은 `d₀`가 담당합니다.
캡이 꺾이기 전에 마지막 측정폭보다 9% 넓어지기는 하는데, `d₀`에서 원뿔의 기울기를 물려받는 매끄러운
캡이라면 어떤 형태든 피할 수 없습니다. `pause_extension`은 `X`에서 따라 나옵니다. 금성 **0.0285**,
화성 **0.0368** 입니다.

**Shue를 닫는 방식으로 닫지 않는 이유.** Shue의 폐곡선 처리는 분모가 0에 닿지 못하도록 `ε`을 더하는
것이고, 여기에도 대응물이 있습니다. 원뿔의 극형식은 `r(θ) = ρ_term/(sin θ + s·cos θ)`로
`θ` = 180° − arctan(s) = 174.2°에서 발산하니, 거기에 `ε`을 더하면
`r(180°) = ρ_term/(ε − s)`에서 닫힙니다. 시도해 봤고 구조적으로 실패합니다. 닫히려면
**`ε` > `s`** 여야 하므로, 교정항이 자기가 교정하려는 벌어짐보다 작아질 수가 없습니다. `L` = 25에서
명암경계선이 12.8% 낮고 20 R_V 폭이 92.8% 낮으며, `L`을 키워도 구제되지 않고 벌어짐이 지워진 반지름
1.1의 원통으로 수렴합니다(20 R_V에서 64.9% 낮음). Shue의 `ε`이 싼 것은 Shue의 꼬리가 **축 방향**
이어서입니다. 점근선이 정확히 θ = 180°이므로 `ε`을 얼마든지 작게 만들 수 있습니다. 벌어지는 경계의
점근선은 축에서 벗어나 있고, 그 차이가 전부입니다.

**렌더에 드러나는 엔진 한계 하나.** Kerbalism의 pause는 눌린 구라서 벌어질 수 없습니다. 그래서 약
2 R_p 밖에서는 `pause_extension`을 어떻게 두어도 엔진 면이 측정된 경계 안쪽에 놓입니다. 아래의 미구현
Shue-native 필드와 같은 부류의 공백이고, 형상함수 선택의 문제가 아니라 Harmony 패치 백로그에
속합니다.
