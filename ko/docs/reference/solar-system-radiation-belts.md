...
Shue 삼중값도 같은 실측에서 나오고 ⚗ 오버레이로 싣습니다.
`α = log₂(1.1405/1.0545) = 0.113`, 노즈 1.055, 꼬리 11입니다. 다만 성격을 분명히 해둡니다. 이것은
*측정된 노즈→명암경계선 벌어짐을 Shue 형식으로 재매개화한 것*이지, 유도 경계를 Shue로 적합한
결과가 아닙니다(그런 적합은 발표된 바 없습니다). 아무 측정도 뒤에 없는 cfg `compression` 환산과는
성격이 다릅니다.
...
- Shue 매개화 `α = log₂(1.47/1.29)` = **0.188**, 노즈 1.29, 꼬리 8.82
