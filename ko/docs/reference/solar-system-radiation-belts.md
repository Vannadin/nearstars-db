...
**여기 오버레이는 2-α 형태를 씁니다.** α 하나로는 작은 주간면 벌어짐과 긴 꼬리를 동시에 낼 수
없고, 유도 자기권이 바로 그 조합입니다(측정 벌어짐의 α 0.113로는 꼬리 11 R_V에 θ = 179.9964°
에서야 닿습니다). 그래서 지수를 각도의 함수로 둡니다. `α_day` 0.113은 실측 노즈·명암경계선이
고정하고, `α_night` 0.34가 꼬리가 벌어지는 정도를(꼬리 폭을 명암경계선의 ~2 % 안에 묶어 불룩해지지 않는
가장 큰 값), `L` 11 R_V는 관측이 정합니다. `α_night`은
측정값이 아니라 형상 손잡이입니다. 방법론의 유도 자기권 절을 보십시오.
...
- 금성과 같은 2-α 오버레이. `α_day` 0.188은 실측 벌어짐, `α_night` 0.34는 미고정 형상 손잡이(같은 불룩함 기준),
  꼬리 8.82 R_M
