...
`extension` **0.057**이고, 마지막 값은 꼬리를 **20 R_V**에서 닫은 결과입니다. 유도 자기권 경계의
가장 먼 확인 통과 지점입니다(Edberg 2024, [`2024JGRA..12932603E`](https://ui.adsabs.harvard.edu/abs/2024JGRA..12932603E),
[2410.21856](https://arxiv.org/abs/2410.21856)).
...
야간면 직선을 하류로 갈수록 흐려지게 그린 것은 의도한 것입니다. 영원히 넓어지지는 않습니다. 먼
꼬리에서는 전이가 태양풍에서 "흔히 보이는 것과 비슷하고… 뚜렷하거나 급격하지 않다"고 하므로
(Edberg 2024), 경계는 되돌아 휘는 대신 경계이기를 그칩니다.
...
이어집니다. `extension` = **0.0735**는 엔진 부피를 20 R_M에서 닫는데, 금성의 가장 먼 확인 통과 지점과
행성 반지름 기준 같은 거리이고 같은 유추를 따릅니다.
