...
꼬리 = `pause_radius/extension`)으로 옮기면 `pause_radius` **1.14**, `compression` **1.0197**,
`extension` **0.0567**이고, 여기에 일반화 필드 둘 `pause_smooth` **0.5**와 `pause_waist` **0**이
붙습니다(둘 다 스톡 Kerbalism에는 없는 필드입니다. 아래를 보세요). 노즈 1.055와 명암경계선 1.13은
정확히 떨어지고 꼬리는 20 R_V에서 닫히는데, 유도 자기권 경계의 가장 먼 확인 통과 지점입니다(Edberg 2024,
...
**형상은 새로 만든 것이 아니라 스톡 함수를 일반화한 것입니다.** 이 경계에 대한 문헌 자신의 모델은
...
`pause_radius` **1.47**, `compression` **1.1063**, `extension` **0.0737**에 `pause_smooth` **0.5**이고,
꼬리를 20 R_M에 둡니다. 금성의 확정 범위와 행성 반지름 기준 같은 거리이고 같은 유추를 따릅니다.
