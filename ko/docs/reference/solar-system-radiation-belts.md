...
**오버레이는 Shue가 아니라 원뿔입니다.** 이 경계들은 문헌에서 원뿔 단면으로 적합되고, `e` < 1이면
원뿔은 스스로 닫힙니다. 추가 항이 필요 없고 노즈·명암경계선·꼬리가 서로 독립인 파라미터 세 개에
얹힙니다. 금성은 발표된 원뿔이 없어 실측 제약 세 개로 풀었습니다. `X₀` 0.7071, `e` 0.9424,
`L` 0.6748이고 노즈 1.0545·명암경계선 1.1405·꼬리 11 R_V를 정확히 만족합니다. Shue를 쓰지 않는
이유는 α 하나가 주간면과 꼬리를 묶기 때문입니다. 측정 벌어짐에서 나온 α로는 11 R_V에
θ = 179.9964°에서야 닿습니다.
...
- 꼬리는 그들의 원뿔에서 바로 나옵니다. `e` < 1이라 닫히고 `r(180°) = L/(1−e)` = 9.60 R_M,
  초점이 `X₀` 0.78이므로 중심 기준 **8.82 R_M** → `extension` = 1.47/8.82 = **0.1667**
- 오버레이는 Vignes의 원뿔 그대로다. `X₀` 0.78, `e` 0.90, `L` 0.96. x ≈ −5 R_M 부근에서
  2.20 R_M(명암경계선 폭의 약 1.5배)까지 벌어졌다가 닫히는데, 이는 적합의 실제 특징이고 이전
  2-α 튜닝이 억눌렀던 부분이다
...
둥근 형태) 꼬리가 훨씬 깁니다(8.8 대 1.7 R_M).
