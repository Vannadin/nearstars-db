...
`extension` **0.0456**이고, 마지막 값은 꼬리를 **25 R_V**에서 닫은 결과입니다. 유도 자기권 경계의
가장 먼 확인 통과 지점인 20 R_V 바로 밖입니다(Edberg 2024,
...
원뿔은 닫아 두었습니다. 이 형상이 유계 cfg 부피가 되어야 하기 때문입니다. 측정된 원뿔에
`(1 − (d/X)^m)`을 곱하며 `X`는 25 R_p, `m`은 20입니다. 측정 구간은 건드리지 않고(15 R_p 안쪽 0.00%,
20에서 1.15%), 벽에 부딪는 대신 뾰족하게 잦아듭니다. Shue를 닫는 방식으로 닫는 것도 시도했으나
...
이어집니다. `extension` = **0.0588**은 엔진 부피를 25 R_M에서 닫는데, 금성의 폐곡선 지점과 행성 반지름 기준
같은 거리이고 같은 유추를 따릅니다.
