...
**문헌 경계**입니다. 비교용으로 그리며 점이 노즈입니다. 자기화된 천체에는 위성 관측을 피팅한 경험
형상인 Shue를, 유도 천체(금성·화성)에는 발표된 원 + 원뿔 IMB 형식을 씁니다. 회색 = 바디,
...
`extension` **0.0285**이고, 마지막 값은 꼬리를 **40 R_V**에서 캡으로 닫은 결과입니다. 측정된 원뿔은
가장 먼 확인 통과 지점인 20 R_V까지 손대지 않고 그대로 갑니다(Edberg 2024,
...
**오버레이는 원 + 원뿔입니다. Shue도 아니고 원뿔 단면(conic)도 아닙니다.** 발표된 IMB 모델이 정확히
그 모양입니다. "주간면의 원과 야간면의 직선"(Martinecz 2009,
[`2009JGRA..114.0B30M`](https://ui.adsabs.harvard.edu/abs/2009JGRA..114.0B30M))이고, 야간면 직선은 `ρ = 1.13 − 0.101·X'`입니다. Edberg
2024가 이것을 Solar Orbiter·BepiColombo·Parker Solar Probe 통과 자료로 시험해 최소 20 R_V까지
그대로 유효함을 확인했습니다. 그래서 오버레이는 **중심 x −0.0777, 반지름 1.1327의 주간면 원**에
**5.77° 원뿔**을 이어 붙인 형태입니다. 금성에서 원뿔 단면이 쓰이는 것은 *bow shock*이고 이 경계가
아닙니다.

두 대안은 여기서 먼저 시도해 봤고 둘 다 실패했습니다. Shue는 파라미터 둘로 제약 셋을 감당해야
합니다. α 0.10은 명암경계선 폭은 맞히지만 꼬리를 즉시 닫아 버리고, α 0.58은 −20 R_V에서의 폭을
맞히면서(3.16 대 3.15) 명암경계선을 40% 넘깁니다. 노즈와 명암경계선에 맞춘 원뿔은 꼬리를 과하게
넓히고(−20 R_V에서 5.05 대 3.15), 타원으로 억지로 닫으면 **0.9745 R_V까지 내려가 표면 아래로**
들어갔습니다. 전체 논증은 방법론 문서 Part A에 있습니다.

원뿔은 닫아 두었습니다. 이 형상이 유계 cfg 부피가 되어야 하기 때문입니다. 관측 한계인 20 R_p까지는
원뿔이 손대지 않은 채로 가고, 그 뒤에 `u = (d − 20)/20`인 `(1 − u²)` 캡을 씌워 40 R_p에서 닫습니다.
그래서 측정 구간은 정확하고(0.00%), 닫힘이 시작되는 지점이 전체 길이의 절반이며, 벽에 부딪는 대신
뾰족하게 잦아듭니다. Shue를 닫는 방식으로 닫는 것도 시도했으나
실패합니다. 그 수법은 `ε`이 벌어짐 계수보다 커야 해서 벌어짐 자체를 지워 버립니다. 자세한 내용은
방법론 문서 Part A에 있습니다.

렌더에서 엔진 면(청록)이 꼬리 쪽에서 측정된 경계(주황) 안쪽에 놓이는 것을 눈여겨보실 만합니다.
Kerbalism의 pause는 눌린 구라서 벌어질 수 없고, `pause_extension`을 어떤 값으로 두어도 해결되지
않습니다. Harmony 패치 백로그에 있습니다.
...
이어집니다. `extension` = **0.0368**은 엔진 부피를 40 R_M에서 닫는데, 금성의 캡 지점과 행성 반지름 기준 같은
거리이고 같은 유추를 따릅니다.
