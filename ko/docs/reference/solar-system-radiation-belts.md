...
**여기에는 Shue 오버레이를 넣지 않습니다. 이유가 구조적입니다.** 측정된 벌어짐에서 α를 뽑으면
0.113인데, 그 α로는 꼬리 11 R_V에 θ = 179.9964°에서야 닿습니다. 사실상 구이고 마지막 각도에서
급히 닫힙니다. α 하나로 작은 주간면 벌어짐과 긴 꼬리를 동시에 낼 수 없는데, 유도 자기권이 바로
그 조합입니다. cfg 형식이 오히려 낫습니다. `compression`과 `extension`이 독립이기 때문입니다.
(자세한 것은 방법론의 유도 자기권 절.)
...
- Shue 오버레이 없음. 금성과 같은 구조적 이유이고, 벌어짐에서 나온 α 0.188로는 꼬리 8.82 R_M에
  θ = 179.31°에서야 닿는다
