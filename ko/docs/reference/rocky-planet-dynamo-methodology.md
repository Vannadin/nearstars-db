...
⚠ **이 스케일링이 외계행성에 내는 모든 값은 외삽이며, 얼마나 멀리인지 여기 적습니다.** OC06 의 적합은
"모형 데이터 공간이 3×10⁻⁶ ≤ E ≤ 10⁻², 10⁴ ≤ Ra ≤ 1.5×10¹⁰, 0.06 ≤ Pm ≤ 24, 0.1 ≤ Pr ≤ 20 을 덮고,
모든 경우 r* = 2.85"(p. 564)인 수치 다이나모에서 나왔습니다. 행성 핵은 E ~ 10⁻¹² 근처 — **적합이 본 가장
낮은 Ekman 수보다 여섯 자릿수 아래** — 이고 Pm 도 0.06 훨씬 아래입니다. 전도도 지수와 같은 모양입니다.
좁은 도구의 출력은 일반 사실이 아닙니다. RM22 가 이 법칙을 태양계에 적용해 관측 모멘트를 재현한 것이
사다리를 허가하지만, 외삽을 줄이지는 않으며 레시피의 등급(judgment)이 그렇게 말합니다.
...
1. **다이나모 생존 게이트** — 자기 레이놀즈수가 임계값 `Rm > 40`을 넘어야 합니다
   ...
   얼어붙었거나 대류하지 않거나 너무 느린 코어는 여기서 탈락합니다. 공식은 `Rm = V L / λ`, `λ = 1/(μ₀σ)`
   (Gaidos+ 2010; OC06 는 `Rm = u d/λ`, 40 은 OC06 자신이 측정한 개시값) — **인용이지 여기서 평가하지 않습니다**.
   V 와 σ 를 대는 것이 우리 손에 없기 때문입니다. OC06 의 주의도 함께 싣습니다. "Rm_crit 너머에서는 Lo_dip 과
   Rm 사이에 단순한 관계가 없다" — 켜짐/꺼짐 게이트일 뿐 세기 예측기가 아닙니다. Gaidos+ 2010 은 이 조건이
   거의 항상 만족된다고 봅니다("10⁻⁴ m/s 의 대류 운동으로 충분", "행성 질량에 매우 약하게만 의존") — 아래
   클래스 사다리를 **지지하는** 논거입니다.
2. **Regime 게이트(국소 Rossby수)** — `Ro_ℓ < 0.12` → **쌍극형**(강하고 조직화된 자기장),
   `Ro_ℓ > 0.12` → **다극형**(모멘트가 쌍극값의 `≈ 0.06 ×`로 붕괴 — **RM22 의 계수**("태양계에서 감소
   계수는 약 0.06")이지 OC06 의 것이 아닙니다. OC06 은 "이 전이를 지나며 쌍극 모멘트가 거의 20배 떨어진다"
   (≈ 0.05×)와, 따로, 다극 영역이 "10배 이상 줄어든다"를 인쇄합니다. 귀속은 2026-09-03 두 캐시 논문 대조로 정정.
   ⚠ **Grießmeier 2009 의 0.15 는 이 양의 값이 아니며** 2026-09-04 에 격자에서 뺐습니다. 그 §2.2 는 한 구성(지구형,
   0.2 AU, 0.5 M☉)에 대해 0.02–0.15 *M_E*(분모가 지구의 현재 모멘트)를 주고 "우주선 플럭스의 하한을 얻기 위해"
   최댓값을 채택한 것이며, Table 1 은 다른 잠김 케이스에 0.37/0.65/0.96 을 인쇄하고, 그 논문에는 Rossby 수도 0.12 도
   없습니다. `Ro_ℓ = 0.12` 경계와 다극 붕괴는 OC06 의 것이고 RM22 가 실행합니다(F·Ra_Q·Ro·Ro_ℓ 식과 Table 6 입력).
   엔진의 격자는 **OC06 자신의 폭 {0.05, 0.10}** — 본문 "거의 20배", 초록 "10배 이상" — 이고 RM22 의 태양계 검증점 0.06 은
   그 안입니다(RM22 채택값은 0.05, OC06 을 읽은 값; OC06 은 0.05 를 인쇄하지 않습니다). ⚠ **base-heated 다이나모 한정** —
   OC06: 내부가열형은 "더 완만하고 산포가 크며 더 작은 Ro_ℓ 에서 시작"; RM22 가 그 조건을 답니다("지구가 그렇듯"). 로스터
   천체의 핵이 base-heated 인지는 **이 레시피가 판정하지 못하며**, 그 라벨이 모든 다극 값과 0.12 문턱 자체에 실립니다).
...
- **Olson & Christensen 2006** (OC06), E&PSL 250, 561 ([`2006E&PSL.250..561O`](https://ui.adsabs.harvard.edu/abs/2006E%26PSL.250..561O)).
  ...
  스케일링 법칙, `Ro_ℓ = 0.12` 쌍극/다극 경계, 다극 붕괴는 "거의 20배"(≈ 0.05×)로 인쇄됨 — 사다리의 ~0.06 은
  RM22 의 태양계 값이지 OC06 의 값이 아니다. **적합 범위, p. 564**: 3×10⁻⁶ ≤ E ≤ 10⁻², 10⁴ ≤ Ra ≤ 1.5×10¹⁰,
  0.06 ≤ Pm ≤ 24, 0.1 ≤ Pr ≤ 20, r* = 2.85 — §법칙의 외삽 주석 참조.
