...
| 4→emit | `phase4/<system>.yaml`의 `decisions[].fields[]` | board filename = `to_file_slug(system_name)`; `body:` = db `name`과 **정확히 일치**(SPEC §3 naming contract); fiction body는 identity 행에 `fictional: true` 표기 (ResearchBodies discoverability 층은 2026-07-28 폐기) | schema gate (check.sh 8); body↔db + roster gates (10a/10b) |
