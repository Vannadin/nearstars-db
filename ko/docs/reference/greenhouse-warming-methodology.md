...
**1단계 — 대기가 없을 때의 온도는 곧바로 나온다.** `T_eq`는 천체가 받는 별빛과 반사율만으로
정해진다. 닫힌 식 한 줄이면 되고 모형이 필요 없다.

**2단계 — 대기가 얹는 몫은 거저 정해지지 않는다.** 상승폭은 대기가 방출 적외선에 얼마나 불투명한지로
...
- [Robinson & Catling 2014](https://arxiv.org/abs/1312.6859)는 적외선 투명도의 기압 의존성이
  **사실상 모든 두꺼운 대기에서 대류권계면을 0.1 bar 근처에** 놓는다는 것을 보였다. 추가 가정 없이 얻는
