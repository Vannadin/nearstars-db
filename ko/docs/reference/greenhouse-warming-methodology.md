...
**2단계 — 대기가 얹는 몫은 공짜가 아니다.** 이쪽은 닫힌 식이 없다. 상승폭은 복사-대류 모형이나
순환 모형을 돌려야 나오는 값이므로, 정직하게 값을 적는 유일한 방법은 이미 발표된 계산을 빌려오는
것이다.

**3단계 — 숫자 하나가 아니라 곡선으로 빌린다.** 기후 논문들은 같은 질문을 반복해서 푼다.
*이 별빛 세기에서 지표를 273 K로 유지하려면 CO₂가 얼마나 있어야 하는가?* 답 하나하나가
(별빛, CO₂) 평면의 점이 되고, 그 점들이 곡선을 그린다. 그 곡선이 도구다.

곡선이 있으면 읽는 일은 단순하다. 우리 천체의 CO₂가 같은 별빛 세기에서 곡선보다 많으면 273 K보다
따뜻하고, 적으면 춥다. **얼마나** 따뜻한지는 두 번째 곡선, 즉 288 K 곡선이 알려준다. 두 곡선
사이의 가로 간격이 15 K에 해당하므로, "곡선에서 얼마나 떨어졌는지"를 켈빈으로 환산할 수 있다.

이 곡선의 모양에 물리의 대부분이 들어 있다.

- **별빛이 줄면 곡선이 급격히 치솟는다.** 별빛이 반으로 줄어드는 값을 CO₂ 두 배로 메울 수 없다.
  시생대 앵커에서 `S/S₀`를 0.80에서 0.75로 낮추면, 273 K에 필요한 CO₂가 여섯 배가 된다.
- **곡선에는 끝이 있다.** CO₂가 대략 8 bar를 넘으면 레일리 산란이 온실 심화보다 빠르게 행성을
  밝게 만들어서 **CO₂를 더 넣어도 더워지지 않는다.** 그 끝점이 거주가능대의 바깥 경계이고,
  충분히 멀리 있는 천체는 CO₂를 얼마나 쌓아도 CO₂로는 데울 수 없다는 뜻이 된다.

이 문서의 나머지는 그 위에 얹는 실무다. 앵커 점의 출처, CH₄·haze·총압 보정, 그리고 이 곡선을
어디까지 믿어도 되는지다.

## 앵커 점

1 bar 배경 대기를 가정한 1차원 복사-대류 계산들이다. **Ts = 273 K** 등온선을 고정하는 세 점은
이렇다.
...
8 bar 점은 앞서 말한 곡선의 끝이다. 덕분에 거주가능대 바깥 경계가 별도 규칙이 아니라 곡선
안에 이미 들어 있다.

## 실용 공식

앵커 사이는 `S/S₀`에 대해 `log₁₀ pCO₂`를 선형 보간한다. 그러면 천체의 별빛 세기에서 두 곡선의
위치 `pCO₂_273(S)`와 `pCO₂_288(S)`가 나오고, 나머지는 위에서 설명한 읽기 그대로다.
...
말로 풀면 이렇다. `pCO₂_eff / pCO₂_273`은 천체가 273 K 곡선에서 얼마나 떨어져 있는지를 비율로
잰 값이다. `m(S)`는 그 비율이 열 배가 될 때 몇 K에 해당하는지이며, 두 곡선 사이 간격에서 얻는다.
마지막 두 항은 haze와 총압에 대한 보정이다. 각 항의 근거는 이렇다.
...
문헌에 다 있다. [Byrne & Goldblatt 2014a](https://ui.adsabs.harvard.edu/abs/2014GeoRL..41..152B)는
CO₂ 강제력을 100 ppmv에서 50,000 ppmv까지 계산해(최대 38.1 W/m²) 고농도에서 어긋나는 IPCC
간이식을 대체하는 표현식을 제시했고, [2014b](https://arxiv.org/abs/1409.1880)는 이를 시생대
기체 28종으로 확장했다.

문제는 곱하는 순간이다. 민감도는 상수가 아니다. 수증기와 얼음 알베도 때문에 지금 구하려는 그
온도에 민감도가 의존하므로, 하나의 λ로 얼어붙은 세계와 온화한 세계를 함께 덮을 수 없다.
NearStars 천체들이 정확히 그 전이 구간에 있어서 이 레시피는 발표된 모형 출력을 쓴다. 나중에
강제력 기반으로 다시 짠다면 이 두 논문이 출발점이고, 민감도는 숫자가 아니라 상태의 함수가
되어야 한다.
