...
**등급** — analog. 그 등급은 **클래스 수준의 스케일링을 한 천체에 적용한다**는 사실 위에 서고,
그것이 `analog` 이라는 낱말이 말하는 바다. **아래 각주에 기대지 않는다.**

⚠ **보유 현황 — 등급이 아니라 범위에 붙는 각주다.** despin 식 자신의 출처는 미보유다
(Goldreich & Soter 1966 · Murray & Dermott 1999). 그래서 `τ_lock` 은 **이 문서가 인쇄한 것**으로
싣는다. 아래 여섯 중 넷은 2026-09-06 기준 보유이고 그중 둘은 읽었다 — 무엇이 무엇에 기대는지는
레시피 헤더에 적혀 있다. **이것은 원전과 대조할 수 있는 범위를 한정할 뿐, 등급이 `analog` 인
이유가 아니다.** 두 문장을 하나로 읽으면 등급이 보유 현황에서 나온 것처럼 보이는데, 아니다.
...
일정 위상지연("fixed-Q") 결과는, Goldreich & Soter 1966([`1966Icar....5..375G`](https://ui.adsabs.harvard.edu/abs/1966Icar....5..375G))이 유도하고
Murray & Dermott 1999 *Solar System Dynamics*([`1999ssd..book.....M`](https://ui.adsabs.harvard.edu/abs/1999ssd..book.....M)) §5에서 교과서 형태로
정리한 것, 다음과 같다.
...
- **달**: 1:1 고정의 정석이다. 섭동체 지구가 3.84×10⁵ km에 있고 암석질 `Q/k₂`를 쓰면 `τ_lock`은
  ...
  면만 보여 준다. 교과서적 앵커다(Murray & Dermott 1999 §5; Peale 1977, [`1977plsa.conf...87P`](https://ui.adsabs.harvard.edu/abs/1977plsa.conf...87P),
...
- **Mercury**: 자전이 감속되었으나(원시의 빠른 자전은 사라진 지 오래라 `τ_lock ≪ t_sys`)
  ...
  결과다(Goldreich & Peale 1966, [`1966AJ.....71..425G`](https://ui.adsabs.harvard.edu/abs/1966AJ.....71..425G); 포획의 설명은 Correia & Laskar 2004,
  [`2004Natur.429..848C`](https://ui.adsabs.harvard.edu/abs/2004Natur.429..848C)). "감속은 일어났다"는 판정은 옳고, "1:1"은 틀렸을 것이다. §4의 예시다.
...
- **Venus**: *중력* 조석만 따진 `τ_lock`은 시스템 나이보다 작거나 비슷해서, 순진하게 적용하면
  ...
  (Gold & Soter 1969, [`1969Icar...11..356G`](https://ui.adsabs.harvard.edu/abs/1969Icar...11..356G); Dobrovolskis & Ingersoll 1980, [`1980Icar...41....1D`](https://ui.adsabs.harvard.edu/abs/1980Icar...41....1D)).
...
- **가까운 안쪽(M형 왜성 HZ 행성, 행성 반경 몇 배에 도는 위성).** M형 왜성의 거주가능 영역은
  ...
  Reynolds 1993, [`1993Icar..101..108K`](https://ui.adsabs.harvard.edu/abs/1993Icar..101..108K)는 M형 왜성 HZ에서의 고정을 짚었고, Barnes 2017,
  [`2017CeMDA.129..509B`](https://ui.adsabs.harvard.edu/abs/2017CeMDA.129..509B)은 그 크기를 준다, §7 참조).
...
근처에서 가장 강하므로, 평형 자전은 평균운동보다 *위*에 자리 잡는다. Hut 1981([`1981A&A....99..126H`](https://ui.adsabs.harvard.edu/abs/1981A%26A....99..126H))은
...
**표류한다.** Dobrovolskis 2007([`2007Icar..192....1D`](https://ui.adsabs.harvard.edu/abs/2007Icar..192....1D))이 이런 이심 자전 상태의 기후 결과를 다룬다.
...
**3:2**가 전형적인 예다(Goldreich & Peale 1966, [`1966AJ.....71..425G`](https://ui.adsabs.harvard.edu/abs/1966AJ.....71..425G)). 포획은 *확률적*이며 이심률과
조석 모형에 달려 있다. Correia & Laskar 2004([`2004Natur.429..848C`](https://ui.adsabs.harvard.edu/abs/2004Natur.429..848C))는 Mercury의 혼돈적 궤도 진화가
3:2를 가장 가능성 높은 결말로 만든다는 것을 보였고, Makarov 2012([`2012ApJ...752...73M`](https://ui.adsabs.harvard.edu/abs/2012ApJ...752...73M))와
Noyelles et al. 2014([`2014Icar..241...26N`](https://ui.adsabs.harvard.edu/abs/2014Icar..241...26N))는 포획 확률을 주며 현실적인(진동수 의존) 조석 모형에서는
...
- **Venus가 태양계의 증거다.** 느린 *역행* 자전이 그 표식이다. Gold & Soter 1969([`1969Icar...11..356G`](https://ui.adsabs.harvard.edu/abs/1969Icar...11..356G))가
  대기 조석 기구를 제안했고, Dobrovolskis & Ingersoll 1980([`1980Icar...41....1D`](https://ui.adsabs.harvard.edu/abs/1980Icar...41....1D))이 토크 균형을
  계산했다: 열조석이, 중력 조석이라면 강제했을 1:1 상태에서 금성을 떼어 놓고 있다.
- **지구형 외계행성이 이를 일반화한다.** Leconte et al. 2015([`2015Sci...347..632L`](https://ui.adsabs.harvard.edu/abs/2015Sci...347..632L), [arXiv:1502.01952](https://arxiv.org/abs/1502.01952))는
  **저질량 별 주위 지구 질량 HZ 행성이 대기 열조석에 의해 비동기 자전으로 몰릴 수 있음**을 보였다:
  즉 순진한 "M형 왜성 HZ 행성 ⇒ 1:1 고정" 추론은 대기가 충분히 두꺼우면 실패할 수 있다. 그보다
  앞서 Correia, Levrard & Laskar 2008([`2008A&A...488L..63C`](https://ui.adsabs.harvard.edu/abs/2008A%26A...488L..63C), [arXiv:0808.1071](https://arxiv.org/abs/0808.1071))이 대기 조석 가지를
...
([`2017CeMDA.129..509B`](https://ui.adsabs.harvard.edu/abs/2017CeMDA.129..509B))은 그런 궤도에 대해 10⁶–10⁸ yr 차수의 고정 시간을 주며, 이는 수 Gyr인
...
([`2015Sci...347..632L`](https://ui.adsabs.harvard.edu/abs/2015Sci...347..632L)) 열조석 단서(§5)가 적용되어 1:1 상태가 더 이상 보장되지 않는다: 단언하지
말고 표시하라. Turbet et al. 2016([`2016A&A...596A.112T`](https://ui.adsabs.harvard.edu/abs/2016A%26A...596A.112T))이 바로 이 범위의 가능한 Proxima b
자전/기후 상태를 탐색한다.

**행성에 고정된 NearStars 위성(α Cen 계, 정성적).** 행성 반경 몇 배에서 행성을 도는 위성은
`a⁶` 게이트의 깊은 안쪽에 있다. `τ_lock ≪ t_sys`이므로 시스템 시간척도에서 **사실상 즉시
1:1로** 고정된다. [조석가열 문서](tidal-heating-methodology.md)가 α Cen Phase 4 위성들(Alpha Centauri A b I (Dante),
Alpha Centauri A b II (Hades), Alpha Centauri A b III (Pandora))에 `(21/2)e²` 동기 가열 형식을 적용할 때 *전제*하는 상태가 바로 이것이며, 그
전제를 정당화하는 것이 이 문서다. (가까운 안쪽 위성을 빠르게 고정시키는 그 `a⁶`은, 가까운 안쪽
위성을 세게 *데우는* `a⁻⁷·⁵`의 역이다. 가까운 안쪽 위성은 가장 빨리 고정되고 **동시에** 가장 많이
데워지므로, 지질학적으로 흥미로운 것들이다.) 먼 바깥 위성(예: 가열 문서의 Alpha Centauri A b V (Chaos), ~20 R_p)은
