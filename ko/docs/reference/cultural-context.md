...
- **"거주 가능 Earth-analog" 프레이밍에서 갈라짐**: Weir 의 Adrian 은 wet Earth-analog 아닙니다. CO₂ 우세 세계로 광화학 CH₄ haze 와 대기 부유 미생물 생태계 (Taumoeba 가 대기 상층 거주). "거주 가능 super-Earth" 해석은 2017–2024 popular-science 가 Feng 의 숫자에 입힌 gloss 이지 Weir 의 묘사 아닙니다. 1.4 g 표면 중력 (카탈로그 1.20 g 가 아니라) 도 Weir 가 popular-science sub-Neptune 핏보다 더 암석질 작은 반지름 천체를 채택했음을 함의.
- **Adrian = 유일한 Astrophage-저항 생물권**: 소설의 핵심 worldbuilding 전제. Astrophage 가 단일 Adrian-origin panspermia 이벤트에서 Earth, Erid, Adrian 을 seeding (Weir, Astronomy.com 인터뷰). 오직 Adrian 만 Taumoeba 자연 포식자를 가지고 있어 Astrophage 가 다른 시스템 모두에서 포식자를 넘어 자라났다. NS 는 cfg 에서 이걸 모델링 못 함 — flavor-text 전용.
- **Adrian ≠ Erid (worldbuilding 혼동 경고)**: Adrian (Tau Cet e) 는 1.4 g + CO₂ 우세. Erid (40 Eri A b) 는 Weir 의 worldbuilding 부록 문서 기준으로 **2.09 g**, **28 atm NH₃** (made-up 이지만 관측과 비충돌), **5.1시간 항성일** (약 18,397 초), **210 °C** 표면온도 (28 atm 압력이 물의 끓는점을 끌어올려 표면 액체수 존재), **지구의 25배** 자기장, **8.47 M⊕** (Ma 2018 측정값), **12,835 km** 반경 (Weir 도출값). Popular summary (일부 검색 엔진 answer box 포함) 가 두 세계를 자주 혼동. 위의 anchor 파라미터는 Adrian 전용. Erid 는 아래 40 Eri A 항목 참조.
- **NS DB**: e (= Adrian) 는 NEA False Positive 로 누락 상태였으나, 이제 **프로젝트 결정(2026-06-03)으로 철회 이전 Feng 2017 값으로 재등재**되어 Phase 3 (`docs/phase3/tau-cet-e.md`, documented-divergence)에 반박 표기됩니다 — 40 Eri b(Erid)와 짝을 이루는 PHM 케이스. f/g/h 는 여전히 `pl_controv_flag=1` 이지만 retraction watch (Figueira 2025 가 detection floor 근처/이하 보고).
...
- **NS DB**: `40_eridani_a.json` (호스트 A — **사용자 결정 2026-06-03 으로 행성 b 를 반박표기 documented divergence 로 재등재**) + `40_eridani_b.json` (백색왜성, 별개 별) + `40_eridani_c.json` (M 왜성) — 다중성계. Phase 3 (`docs/phase3/40-eridani-a.md`) 가 Burrows 2024 반박을 문서화.
- **Eridian 우주관 (PHM, flavor 전용)**: Erid 의 **짙은 대기가 빛을 차단**하고 **강력한 자기장이 방사선을 차단**하여, Eridian 은 **시각 없이 진화**해 **반향정위**로 감지합니다 ("고래 노래 + 소나"에 가까운 음향/주파수 언어. Rocky 는 사각지대 없이 360도를 지각). 그 결과 이들의 천문학은 매우 늦게 발달했고, **이웃 행성을 최근에야 발견**했으며 방사능이나 특수 상대성 같은 개념이 없었습니다 (이 상대성 지식의 공백이 Erid→Tau Ceti 항행을 거의 무너뜨릴 뻔합니다). NS 는 이를 모델링하지 않습니다 (flavor-text 전용). 다만 이는 위의 Weir Erid 파라미터를 **교차 검증**해 줍니다. 눈이 없으려면 worldbuilding 문서의 불투명한 짙은 NH₃ 대기 + 매우 강한 (지구의 약 25배) 자기장이 정확히 필요하기 때문입니다. 출처. PHM Wiki (Eridian), Wikipedia (Project Hail Mary).
