...
**NearStars 규칙.** 별의 **기본 광구 색**은
[`stellar-photospheric-color-methodology.md`](stellar-photospheric-color-methodology.md)
가 정의한 근거화된 값을 쓴다 (세 구역 규칙. FGK/백색왜성 → Teff blackbody, M
왜성 → Pickles 실측 SED 의 옅은 따뜻한 주황, L/T/Y → blackbody 의 깊고 흐린
빨강). 이게 저장소의 유일한 색 경로다. 나머지 별 색 (`sunspotColor`,
`rimColor`, `*SunlightColor`, `sunLensFlareColor`, `emitColor*`) 은 그 기본
hex 를 **렌더링용으로 변형**한 파생물이지 두 번째 계산이 아니다. `rimPower`
...
- 색상. `sunlightColor`, `scaledSunlightColor`, `IVASunColor`,
  `sunLensFlareColor`, `ambientLightColor` — 모두 별의 근거화된 기본 hex 에서
  파생된다 ([`stellar-photospheric-color-methodology.md`](stellar-photospheric-color-methodology.md)
  참고. M/K 는 따뜻한 주황, G/F 는 거의 흰색, A 는 청백, L/T/Y 갈색왜성은
  **깊고 흐린 빨강** — "자홍" 이라는 렌더 직감이 아니다 — neutron star 는
  파랑).
...
- **Brown / L-dwarf.** 어두운 `luminosity` (~1–2), 어두운
  `ambientLightColor`, **깊고 흐린 빨강** `emitColor0/1` (내재 잉걸불 발광.
  ~1000–1300 K 의 근거화된 기본 hex,
  [`stellar-photospheric-color-methodology.md`](stellar-photospheric-color-methodology.md)
  §6.3 — 팩들이 쓰는 자홍 렌더 선택이 아니다), 억제된 corona
  (`mainTexScale = 0,0`), 가까이서만 플레어가 피어나도록 가파른
  `brightnessCurve`.
...
있는 건 디스크 *렌더링 기법*이다. 디스크 **색**은
[`debris-disk-color-methodology.md`](debris-disk-color-methodology.md) 로 따로
도출한다 — 거기서 나온 색조를 어떤 메커니즘에든 먹이되, 여기서 디스크 색을
지어내지 말 것.)
