...
티가든의 별 b 는 최소질량 1.16 M⊕ (전형적 i = 60° 기하학적 경사에서
실제 질량은 ~1.34 M⊕; Fujii 2026) 의 암석 행성으로, M7.0 V 모성 둘레를
4.91 일 주기로 공전합니다. 궤도 장반경 0.0259 AU 와 모성 볼로메트릭
광도 7.32 × 10⁻⁴ L☉ 가 결합되어 입사 플럭스 1.08 ± 0.08 S⊕ 가 나옵니다.
지구보다 약간 강한 셈으로, b 를 보수적 거주가능 영역의 안쪽 가장자리에
위치시킵니다. Bond 알베도 0.3 의 평형 온도는 277 ± 5 K (Dreizler 2024),
알베도 0 이면 ~280 K (Fujii 2026 기준값) 입니다. 두 값 모두 물의 어는
점을 끼고 있어, b 는 질량과 입사 플럭스 기준만으로도 NearStars
카탈로그에서 가장 진짜 지구-아날로그에 가까운 후보 중 하나입니다.
...
1.05 R⊕ 는 Zeng 2016 / 2019 의 지구형 벌크 질량-반지름 관계를 따르며,
최근 GCM 논문들 (자매 행성에 대한 Boukrouche 2025, Fujii 2026, Hammond
2025) 도 일관되게 같은 관계를 사용합니다. 질량은 최소 질량 (m sin i)
이므로 낮은 궤도 경사에서는 실제 질량-반지름이 상당히 더 클 수 있지만,
1.16-1.34 M⊕ 범위에서 반지름 예측은 여러 질량-반지름 척도에 걸쳐 ~5 %
안에서 안정적입니다.

b 에 관한 cfg 결정적 질문은 **runaway vs habitable** 입니다. Boukrouche
2025 (arXiv:2510.11940) 는 Phase 2 권장 별 광도 (입사 플럭스 = 1481
W/m²) 에서 Isca GCM 을 돌려, 표면 알베도 α = 0.07 (해양) 과 α = 0.30
(육지) 모두에서 b 가 runaway 온실 임계값 아래에 편안히 머문다고 결론
지었습니다. 즉 현재 지구형 조성의 1 bar 대기는 지속 가능합니다. Dreizler
2024 의 대안 별 매개변수 (Teff = 3034 K, R = 0.120 R☉) 를 쓰면 입사
플럭스가 1565 W/m² 로 올라가고 같은 GCM 이 b 를 runaway 임계값 너머로
보냅니다. Phase 2 는 Schweitzer 2019 의 R = 0.107 R☉ 를 권장으로
선택했고, 이 합성은 그에 대응하는 **habitable** cfg 시나리오를 채택합니다.
Runaway 변형 cfg 는 Open items 에 보존됩니다.
...
| `mass_estimate_mearth_true` | 1.34 (기하학적 i ≈ 60°) | medium | Fujii 2026 — 3D GCM 사용 |
| `radius_rearth` | 1.05 | medium | Zeng 2016 지구형 MR; 모든 GCM 논문이 이 척도 채택 |
| `surface_gravity_g_earth` | 1.21 | medium | 유도 = 1.16 / 1.05² |
| `density_g_cc` | 5.5 (지구형 가정) | low | 통과 없음; Zeng 2016 MR 에서 가정 |
| `insolation_s_earth` | 1.08 | high | Dreizler 2024 (Phase 2 별 매개변수) |
| `equilibrium_temp_k` (A=0) | 280 | high | Fujii 2026 기준값 |
...
| `surface_temp_substellar_k` | 295 | medium | Boukrouche 2025 Isca GCM 지구형 시나리오 substellar 온대 |
| `surface_temp_nightside_k` | 240 | medium | Boukrouche 2025 GCM 야간측 차가운 트랩 |
| `surface_temp_global_mean_k` | 280 | medium | Boukrouche 2025 — 1481 W/m² 에서 runaway 임계값 아래로 habitable |
| `atmosphere_present` | true | medium | Tie-break: 직접 대기 검출 없음 (통과 없음); Wandel 2019 + Boukrouche 2025 GCM 의 정전 지구형 시나리오 |
| `atmosphere_surface_pressure_pa` | 100 000 | medium | Tie-break: Boukrouche 2025/26 과 Fujii 2026 기준이 채택한 1 bar 지구형 |
| `atmosphere_composition` | N₂ 78%, O₂ 21% (광분해 비생물적), CO₂ 400 ppm, H₂O 0.1–1% 포화, Ar 0.5% | medium | Tie-break: Boukrouche 2025/26 명시적 조성의 지구형; 높은 M-왜성 XUV 하 H₂O 광분해에서 비생물적 O₂ 축적 |
| `atmosphere_scale_height_km` | 8.1 | medium | 유도: kT/μg 에서 T=280 K, μ=29, g=11.9 m/s² |
...
| `cloud_cover_fraction` | 0.55 | medium | Boukrouche 2025/26 Isca GCM — 1-10 mbar 의 상부 구름층 + 종단경계의 중고도 구름 |
| `cloud_morphology` | 주간측 1-10 mbar 의 성층권 상부 구름층; 종단경계의 200-500 mbar 중고도 고리에서 습한 공기가 응결 | medium | Boukrouche 2026 Fig. 1-2 — 주간측 구름 피복; substellar 습기가 종단경계 cold-trap 으로 이송 |
...
| `ocean_extent_substellar_radius_deg` | 45 | medium | Tie-break: Boukrouche 2025 substellar 온대 반경 (명시 표는 없지만 Fig. 1 의 따뜻한 표면이 ~40-50°); TRAPPIST-1 e 의 35° 보다 크게 — 입사 플럭스가 높기 때문 |
| `ocean_tint_rgb_hex` | `#1a2540` (희미한 빨강 별 아래 깊은 물 — 짙은 남색) | low | Tie-break: 깊은 바다 + M7 V 조명; TRAPPIST-1 e 관례 따름 |
| `surface_ice_caps` | substellar 열린 물 원반 바깥의 해빙 + 빙하 얼음; 표면의 ~50% | medium | Boukrouche 2025 — 1481 W/m² 에서 substellar 로부터 ~45° 의 빙선 |
...
| `surface_morphology` | substellar 로부터 ~45° 안의 바다; 고위도 / 고경도의 해빙 + 빙하 지형; 종단경계 압력 능선의 노출 암반 | medium | Boukrouche 2025/26 Isca GCM; Pierrehumbert 2011 조석 고정 aquaplanet 템플릿 |
...
| `aurora_intensity_kR_typical` | 30 | medium | Tie-break: 지구의 10 kR 의 3 배 (조용한 별 + 적당한 장) — 티가든 플레어 빈도가 ~25 배 낮아서 TRAPPIST-1 e 의 150 kR 보다 훨씬 낮음 |
...
1. **입사 플럭스** S = 1.08 ± 0.08 S⊕ (Dreizler 2024) — 지구보다 약간
   ...
   해당합니다. Boukrouche 2025 (현재 지구의 대기 조성을 가진 Isca GCM)
...
2. **질량과 벌크 밀도.** m sin i = 1.16 M⊕ 와 Zeng 2016 지구형
   ...
   전형적 i = 60° 기하학적 경사에서 실제 질량은 ~1.34 M⊕ (Fujii 2026);
   반지름 예측은 ~5 % 이상 바뀌지 않습니다.
...
Yang 2014, Boukrouche 2025 Isca GCM 으로 여기에 적용) 이 TRAPPIST-1 e
...
- **종단경계와 야간측** (substellar 에서 >90°): 얼어붙은 바다 기질 위
  의 두꺼운 빙하 얼음; 표면 온도 ~240 K (Boukrouche 2025 GCM 야간측).
...
cfg 는 Boukrouche 2025 (Isca GCM) 와 Boukrouche 2026 (LIFE 반구 논문)
을 따라 **1 bar N₂ 지배 aquaplanet 대기** 를 채택합니다. 그 시뮬레이션
들에서 사용된 명시적 조성은 본질적으로 현재 지구입니다.

- **압력** 1 bar (100 kPa) — 지구형, Boukrouche 2025 에 따라 1481 W/m²
  에서 runaway 임계값 아래에서 지속 가능.
- **조성** N₂ 78%, O₂ 21% (비생물적 광분해 — cfg 는 생물권 없음 가정;
  Lincowski 2018 류의 M-왜성 비생물적 O₂ 축적), CO₂ 400 ppm (지구
  현대), CH₄ 1 ppm 미량, H₂O 0.1-1% 포화, Ar 0.5%.
- **구름.** Boukrouche 2026 Isca GCM Fig. 1-2 는 주간측의 1-10 mbar
  높이 (지구에서는 성층권에 해당) 의 상부 구름층과, 종단경계 근처의
  중고도 (~500 mbar) 물 구름을 보여줍니다. 전 지구 구름 피복 ~55%.
  주간측 상부 구름층은 알베도를 지배할 정도로 광학적으로 두껍지 않습니다
  — 외부 장파 복사 패턴은 구름 불투명도가 아니라 대기 온도 구조에서
  주로 결정됩니다. 이는 cfg 의 핵심 구분점입니다. TRAPPIST-1 e (중고도
  층적운이 지배) 와 달리 b 의 주간측 구름 시그너처는 성층권에 있습니다.
...
된 산란광 (작지만 무시 불가 — Boukrouche 2025 GCM 은 야간측 온도가
~50 W/m² 의 이송된 열로 ~240 K 유지됨을 보임), (b) 결합 시 자매 행성 c
와 d 의 반사광 (~0.3° 시지름, m_v ≈ −8 ~ −10), 그리고 (c) 배경별 별빛.
야간측 하늘은 어둡지만 완전히 어둡지는 않습니다 — KSP 야간측 환경광은
주간측의 ~3-5% 가 적절합니다.

**대기 손실.** 오래된 나이 (8 Gyr) + 낮은 현재 XUV 플럭스 + 조용한 별이
티가든 b 의 대기 보존 전망을 프록시마 b 나 TRAPPIST-1 d 보다 훨씬 좋게
만듭니다. Wandel 2019 §3.1 은 Lammer 류의 침식 모델을 리뷰하고, 현재의
항성풍은 지구 질량 대기를 벗기기에 너무 약하다고 언급합니다. 초기 전-
주계열 단계 (티가든 질량 같은 극저온 왜성에서 ~1 Gyr 지속) 는 더 뜨겁고
더 활동적이었고 모든 원시 H/He 외피를 벗겨냈을 수 있지만, 후기 accretion
/ 외기 방출로 획득된 무거운 원소 2차 대기는 살아남아야 합니다. 오늘날의
L_X / L_bol = −4.9 는 상대적으로 높지만 절대 L_X ≈ 3.7 × 10²⁵ erg/s 는
지구 위치에서 태양 X선 플럭스의 1/10⁵ 만 줍니다. Boukrouche 2025 GCM
은 대기 탈출을 포함하지 않으며 지구형 조성이 유지된다고 가정합니다.

**오로라** 는 존재하지만 차분한 시각 특징입니다. 1 bar 대기와 지구형
자기장 (tie-break 가정) 으로 b 는 정전적인 ~60° 자기 위도의 극지 오로라
타원을 지지해야 합니다. 티가든이 연간 ~0.4 회의 큰 (≥10³⁵ erg) 플레어와
100 일당 ~2.6 회의 적당한 (~10³² erg) 플레어 (Fuhrmeister 2024 + Dreizler
2024) 만 만들기 때문에, 오로라 강도는 TRAPPIST-1 e 의 150 kR 보다 훨씬
낮습니다. cfg 는 30 kR (지구 전형 10 kR 의 3 배) 를 채택해, 인지 가능
하지만 절제된 오로라 팔레트를 줍니다. 지배적 방출은 [OI] 557.7 nm 녹색
(지구형 색 `#4DFF4D`), 약한 N₂⁺ 391.4 nm 보라 성분이 보조합니다.

## Rotation & spin synthesis

8 Gyr 에 걸친 4.91 일 주기의 조석 감쇠는 명백하게 동기 (1:1) 자전을
...
**대기 순환 체제.** Boukrouche 2025/26 Isca GCM 과 Hammond 2025
ExoCAM GCM (후자는 c 용이지만 동역학 체제는 외삽 가능) 모두 적도 초회전
을 찾습니다. substellar 주간측의 정적 열 강제력으로 구동되는 단일 광폭
순방향 동서 제트입니다. Boukrouche 2026 의 명시적 구름 분율 Fig. 2 는
substellar 점에 중심을 두고 동쪽으로 꼬리가 뻗는 주간측 구름 원반을
보여주는데, 조석 고정 대기 순환의 Matsuno-Gill 패턴과 일치합니다.
TRAPPIST-1 e 에서 보이는 명백한 중위도 제트 분기는 없습니다. b 는 동역학
위상도 (Haqq-Misra 2018) 의 "초회전" 측에 완전히 있는데, b 가 더 뜨겁기
때문입니다 (1.08 대 0.66 S⊕).
...
- **Wandel A. & Tal-Or L. 2019** — *On the Habitability of Teegarden's
  ...
  분석적 1D 거주가능성 모델 — b habitable 범위 H_atm = 0.32-3.7.
- **Boukrouche R., Caballero R., Lewis N. T. 2025** — *Near the
  Runaway: The Climate and Habitability of Teegarden's Star b*, ApJ
  993, L19 (`2025ApJ...993L..19B`, arXiv:2510.11940). 1481 W/m² 의
  Isca GCM — α_s = 0.07 와 0.30 양쪽에서 habitable; 1565 W/m² 는
  runaway 너머. cfg 의 habitable 시나리오 선택을 결정한 핵심 논문.
- **Boukrouche R. et al. 2024** — *The Impact of Water Clouds on the
  ...
  LIFE*, ApJ 977, 88 (`2024ApJ...977...88B`, arXiv:2411.07922). LIFE
  로 물 구름 피복 검출 가능성 — 거주가능성 추적자로서의 구름 피복 확인.
- **Boukrouche R. & Janson M. 2026** — *Disentangling the Hemispheres
  of Teegarden's Star b with LIFE*, ApJ 996, L43 (`2026ApJ...996L..43B`,
  arXiv:2512.19231). Isca GCM 반구 지도; 1-10 mbar 의 상부 구름층;
  LIFE 3 일 광대역 1σ 반구 구별.
- **Fujii Y. et al. 2026** — *Probing Thermal Gradient of Habitable-
  ...
  arXiv:2512.16575). 1 과 10 bar 의 b 의 ROCKE-3D 해양-vs-비해양
  시나리오; LIFE 가 두 위상에서 1 일에 구별 가능. T_eq(A=0) = 280 K
  기준값 제공.

### Read (context / methodology, not decision-driving)

- **Hammond T. et al. 2025** — 근접 온대 암석 표적의 기후 (arXiv:
  2504.00978). 티가든 c 에 직접 사용; b 에는 맥락으로 (b 는 그들의 눈덩이
  목록에 없음 — ExoCAM 격자에서 habitable).
- **Schweitzer A. et al. 2019** — 별 매개변수 (arXiv:1904.03231), 모성
  통해 사용.
- **Fuhrmeister B. et al. 2024** — 대기 보존을 위한 별 활동 맥락
  (arXiv:2504.02338).
...
- **runaway 임계값에 대한 별 매개변수 모호함** (핵심):
  ...
  M7 에서는 불가능한 별진동) 가 Dreizler 의 더 큰 R 을 지지하면, cfg 는
  **runaway Venus 아날로그 변형** 으로 전환해야 합니다. 1-100 bar CO₂
  대기, 표면 물 없음, 뜨거운 주간측 (~700 K), 두꺼운 황산 구름, 얼음
  없음. 백업 변형으로 보존.
- **낮은 경사에서의 실제 질량**: 궤도 경사가 60° 보다 훨씬 낮으면 실제
  질량은 1.5-3 M⊕ 가 될 수 있고 행성은 암석이 아니라 미니-Neptune 일
  수도 있습니다. Fujii 2026 은 1.34 M⊕ 추정을 위해 명시적으로 i = 60°
  기하학적을 채택합니다. Gaia DR4 나 Theia 의 측성학적 검출이 경사를
  제약할 수 있습니다.
