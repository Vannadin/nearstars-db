...
M5.5V 조명 (L = 0.00155 L☉) 에서는 일조량이 지구의 ~1.9 배에 그치며,
평형 온도는 326 K (A = 0.3) — Mercury 와 비슷하고 어떤 액체 물 임계점
보다도 높습니다.
...
**NearStars 시나리오 선택. airless 하게 햇볕에 구워진 basalt 표면
(substellar 최고 ~460 K, silicate solidus 한참 아래 → 용융 없음), vestigial
sodium-vapor exosphere 를 가진 뜨거운 조석 고정 Mercury-like sub-Earth-mass
암석 행성으로, Proxima 의 flare 출력에 대한 대기 차폐가 없습니다.** 32 cfg 픽. 17 은 canonical-aligned
...
| `insolation_s_earth` | 1.9 | high | L = 0.00155 L☉, a = 0.029 AU 로 유도 |
...
| `surface_temp_substellar_k` | 460 | medium | airless substellar 최고치 (재분배 없음) 가 ~1.9 S⊕ 에서 ≈ 460 K. silicate solidus (~1300 K) 한참 아래 → 고체 암석, 용융 없음 |
| `surface_temp_nightside_k` | 80 | medium | Tie-break. airless cold trap (Mercury 야측 ~100 K). 약간의 열 inertia → ~80 K |
| `atmosphere_present` | false (vestigial Na vapor 만) | medium | Tie-break. ~1.9 S⊕ + Proxima flare 환경에서 escape ratio 가 강하게 손실 favor. interesting-first 가 완전 airless 보다 Mercury-analog vestigial exosphere 선택 |
...
| `ocean_present` | false | high | airless, 안정한 표면 액체 없음. 주간 ~460 K 가 지속 물 배제 |
...
| `magnetic_dipole_moment_normalized_earth` | 4 | low | Zapatero Osorio 2026 (`2605.22925`) 의 SPI 유도 극지방 자기장 ~16 G (중앙값, Mars-size) 로부터. 지구(극지방 0.6 G) 대비 M ∝ B_pol·R³ → Mars 반지름이면 지구의 ~4배. **차수 수준만 유효** — 논문의 3–280 G 범위 × 반지름 불확실성은 지구의 ~0.5–70배에 걸치며, 논문 스스로 이 자기장이 "차수 수준에서만 신뢰할 수 있다"고 밝힌다. 기존 Mercury-analog 0.001 을 대체한다. **도출 스케일링과의 긴장.** 암석형 다이나모 사다리([rocky-planet-dynamo-methodology.md](../reference/rocky-planet-dynamo-methodology.md), RM22)는 *정반대*를 예측한다 — 4.85 Gyr에 소질량 0.26-M⊕ 코어는 사멸~약장(ℳ ≲ 0.01)일 가능성이 크다. SPI *관측*이 *도출*을 이기지만(측정이 스케일링에 우선), 이 불일치 자체가 주목할 만하다. d가 예상 밖으로 강한 다이나모를 유지하거나, 위상고정 플레어 귀속을 재검토해야 한다는 뜻이다 |
...
뜨겁지만 airless 한 dayside 와 깊이 차가운 nightside 가 sharp 한 terminator
기울기를 만듭니다. ~1.9 배 지구 일조량과 albedo 0.15 에서 평형 온도 (326
K) 는 전역 평균값입니다. airless substellar 점은 (주야 재분배 없음) 최고
~460 K 정도까지만 올라갑니다.

이 ~460 K substellar 최고치는 basalt/silicate solidus 인 ~1200–1300 K
보다 한참 아래라서, dayside 는 고체로 햇볕에 구워진 basalt 표면입니다 —
용융도 magma pond 도 없습니다. Io 식 silicate 화산활동은 조석 가열 엔진이
있어야 하지만 Proxima d 는 의미 있는 조석 가열이 없어서 그 경로도
배제됩니다. 따라서 substellar 액센트는 광채가 아니라 건조한 regolith
reddening 톤입니다. 가상 우주선이 행성에 접근하면서 mid-day terminator 에
다가가는 모습에서 보이는 따뜻하게 구워진 basalt 입니다
(`surface_tint_rgb_hex_accent = #6a4632`).
...
overprint 된 햇볕에 구워진 substellar regolith — 는 전부 Mercury analog 에서
...
중력 0.62 g_⊕ 와 substellar 최고치 ~460 K 에서 어떤 그럴듯한 secondary
atmosphere 의 Jeans-escape 파라미터도 H₂ 와 H₂O 에 대해 Gyr 시간 스케일
에서 thermal escape 에 우호적입니다. Proxima 의 강한 XUV 출력은
hydrodynamic escape 도 강하게 구동합니다. Lee 2021 ([2109.06963](https://arxiv.org/abs/2109.06963)) 은 Proxima b 도 0.05 AU 에서
...
만들고, 햇볕에 구워진 substellar 영역에 미묘하게 더 따뜻한 액센트 (`#6a4632`)
...
- **Vida K. et al. 2019** — *Flaring Activity of Proxima Centauri
  from TESS Observations* ([arXiv:1907.12580](https://arxiv.org/abs/1907.12580)). 도즈 스케일링을 위한
  superflare 빈도 + 에너지 분포.

### Read (context / methodology, not decision-driving)

- **Lee Y. et al. 2021** — *Exosphere Modeling of Proxima b*
  ([arXiv:2109.06963](https://arxiv.org/abs/2109.06963)). d 의 escape rate 에 대한 자릿수 스케일링.
...
- [proxima-cen](proxima-cen.md) — 호스트별. 플레어 환경이 대기 박탈 시나리오를 끌고 갑니다.
- [proxima-cen-b](proxima-cen-b.md) — HZ 에 위치한 외곽 형제. 대조 — d 는 지구의 ~1.9 배 일사로 어떤 HZ 해석에서도 내측 경계 안쪽에 놓입니다.
