...
를 줍니다. 기본 파라미터는 VLTI/VINCI 간섭계 (Di Folco 2004) 로
탄탄히 정박돼 있어 R = 1.842 ± 0.019 R☉, Mamajek 2012 의 진화 모델
질량 M = 1.92 ± 0.02 M☉ 입니다. 유효 온도는 Di Folco 2004 가 볼로
메트릭 플럭스와 간섭계 θ_LD 로부터 직접 유도한 Teff = 8751 ± 90 K
이고, 대응하는 광도 L = 16.63 ± 0.48 L☉ (Mamajek 2012) 로 인해
...
영상은 ~10 AU 의 **안쪽 소행성대 analog** 와 ~40–80 AU 의 **중간
...
렌더링합니다.** 15 개 cfg 픽 모두 canonical 파라미터 셋을 따르며,
tie-break 픽은 시각 hex 색 (A4V SED 백청색) 과 원반 RGB 색조 (먼지
온도 → 광학 색) 에 한정됩니다.
...
| `mass_msun` | 1.92 ± 0.02 | high | Phase 2 권장. Mamajek 2012 — Di Folco 2004 의 간섭계 R 에 정박된 Y² 진화 트랙 등시선 fit |
| `radius_rsun` | 1.842 ± 0.019 | high | Phase 2 권장. Di Folco et al. 2004 — VLTI/VINCI K 밴드 limb-darkened 간섭계. θ_LD = 2.223 ± 0.022 mas (1% 정밀도) |
| `teff_k` | 8751 ± 90 | high | Phase 2 권장. Di Folco et al. 2004 — 볼로메트릭 플럭스 + θ_LD. 이 합성의 이전 초안에서 인용했던 SIMBAD 컴파일레이션 스칼라 8689 K 를 대체합니다. 8689 vs 8751 K 차이는 1σ 안에 들어가며, 원반 일사량이나 HZ 경계 픽으로 전파되지 않습니다 |
| `luminosity_lsun` | 16.63 ± 0.48 | high | Phase 2 권장. Mamajek 2012 — 측광 SED + Hipparcos 시차에서 직접 적분한 볼로메트릭 플럭스. 이전 초안 표기 "R 과 Teff 로 Stefan–Boltzmann 유도" 는 부정확합니다. 실제 채택값은 Mamajek 2012 의 직접 측정 볼로메트릭 L 입니다 |
| `metallicity_fe_h_dex` | −0.03 ± 0.05 | medium | Phase 2 권장. Saffe et al. 2008 — Castor 운동성단 A 형 별의 고분해 분광. 사실상 태양과 동일. Erspamer & North 2003 의 자동화 DETAIL 합성은 −0.27 ± 0.13 (cross-check 로 보존). Castor-MG 전용 분석보다 체계적으로 낮은 값을 줍니다 |
| `age_gyr` | 0.44 ± 0.04 | high | Phase 2 권장. Mamajek 2012 — 등시선 + Castor 운동성단 운동학 멤버십 결합. 이전 ~200 Myr 추정치 (Barrado y Navascués 1998) 를 대체 |
| `rotation_period_days` | 0.97 ± 0.1 | medium | Phase 2 권장. Royer et al. 2007 — v sin i = 93 ± 3 km/s + R = 1.842 R☉ + 원반 정렬 자전축 i ≈ 65.6° (Le Bouquin 2009) ⇒ P_rot = 2πR sin i / (v sin i) ≈ 0.97 d. spin-disk 정렬을 가정하며, Le Bouquin 2009 가 ~3° 이내로 입증 |
| `limb_darkening_alpha_h` | 0.211 ± 0.020 | high | Davis et al. 2005 — SUSI 광학 간섭계. A 형 별 limb darkening 이 태양형 G 형보다 살짝 강함 |
| `visual_surface_tint_hex_primary` | `#cfe0ff` (시원한 청백, A 형 별) | medium | Tie-break. 8751 K A4V 흑체 + 볼로메트릭 보정. Vega 의 살짝 더 푸른 A0V 색조와 시각적으로 구분 가능 |
| `stellar_color_temp_k` | 8751 | high | Teff 유도 (Phase 2 Di Folco 2004) |
...
| `disk_inner_radius_au` | 133 | high | Boley 2012 / White 2017 ALMA — 편심 궤도 보정 후 주 고리 안쪽 가장자리 |
| `disk_outer_radius_au` | 158 | high | Boley 2012 / White 2017 ALMA — 주 고리 바깥쪽 가장자리 |
| `disk_dust_temperature_k` | 65 | high | Stapelfeldt 2004 Spitzer. Acke 2012 Herschel 의 차가운 성분 SED fit |
| `disk_tint_rgb_hex` | `#d8c4a6` (따뜻한 먼지빛 buff, T_dust ≈ 65 K) | low | Tie-break. 차가운 잔해 먼지는 청색보다 적색을 더 효율적으로 산란. 청백색 별 배경 위에 "먼지 고리" 로 읽히도록 선택 |
| `disk_opacity` | 0.35 | low | Tie-break. 실제로는 광학적으로 얇음 (τ ≈ 10⁻⁴). interesting-first 룰에 따라 게임 내 가시성을 위해 opacity 를 부풀림 |
| `disk_morphology` | 안쪽 따뜻한 소행성 analog 가 있는 3 벨트 편심 주 고리 | high | Kalas 2005 + Boley 2012 + Gáspár 2023 JWST — 완전한 다중 벨트 구조 |
...
| `disk_imaging_inclination_deg` | 65.6 ± 0.4 | high | Le Bouquin 2009. Boley 2012 ALMA 가 재확인 |
| `disk_mass_mearth` | 0.015 (주 고리, mm-cm 입자) ~ 4–10 (행성소체 reservoir) | medium | Holland 2017 SCUBA-2 + ALMA. 하한은 영상에 잡힌 입자, 상한은 충돌 cascade 모델링 |
| `disk_planetesimal_belt_inferred` | true | high | 날카로운 안쪽 가장자리 (Kalas 2005) 는 양치기 천체를 요구. 충돌 cascade 가 먼지를 보충하려면 부모 행성소체 종족이 필요 (Wyatt 2008 프레임워크, Boley 2012 가 Fomalhaut 에 적용) |

## Surface synthesis

Fomalhaut 의 광구는 Teff = 8751 K 로 A 형 주계열의 한가운데에
자리잡으며, 태양보다 약 3000 K 뜨겁고 canonical A0V 표준성 Vega
보다는 약 650 K 차갑습니다. 이 온도에서 연속체 불투명도는 수소의
...
형성됩니다. 살짝 sub-solar 한 금속도 ([Fe/H] = −0.03, Saffe 2008
운동성단 평균) 는 SED reddening 에 무시할 만한 영향을 줍니다.
...
UV/FUV 플럭스는 본질적으로 높습니다. 8751 K 광구의 Wien 꼬리가
...
대응하는 자전 주기는 P_rot = 2πR/v_eq ≈ **0.97 일** 로, 태양보다
...
밝기는 T⁴ 스케일링 (Teff = 8751 K vs 5778 K → 단위 입체각당 5.3
...
어떤 광시야 뷰에서도 손쉽게 보임). 고리는 cfg 의 `disk_tint_rgb_hex
= #d8c4a6` 으로 렌더링되는데, ~65 K 먼지 열복사가 A 형 별 빛을
재산란하는 모습을 표현한 따뜻한 먼지빛 buff 색입니다. `disk_opacity
= 0.35` 는 물리 광학적 깊이 (~10⁻⁴) 에서 끌어올린 tie-break
boost 로, 고리가 수치적 잡티가 아니라 게임 내에서 실제로 보이는
대상으로 읽히도록 보정한 값입니다.

**40–80 AU 의 중간 벨트** (Gáspár 2023 JWST) 는 동일한 먼지 색조에
...
- **Mamajek E. E. 2012** — *On the age and binarity of Fomalhaut*,
  ApJ 754, L20 (`2012ApJ...754L..20M`, arXiv:1206.6353). 등시선 +
  Castor 운동성단 운동학 멤버십 결합으로 440 ± 40 Myr 나이를 확립.
  Di Folco 2004 의 간섭계 반지름에 정박한 Y² 진화 트랙으로
  1.92 ± 0.02 M☉ 질량을 fit. 현재 표준 나이와 질량 참조 논문.
...
- **Di Folco E. et al. 2004** — *VLTI near-IR interferometric
  ...
  (`2004A&A...426..601D`, arXiv:astro-ph/0408390). VLTI/VINCI K 밴드
  limb-darkened 각지름 θ_LD = 2.223 ± 0.022 mas. Hipparcos 시차에서
  R = 1.842 ± 0.019 R☉ 유도. 반지름 측정을 정박.
...
- **Gáspár A. et al. 2023** — *Spatially resolved imaging of the
  ...
  영상이 이전에 알려지지 않았던 **~40–80 AU 의 중간 벨트** 를
...
- **Royer F. et al. 2007** — *Rotational velocities of A-type stars*,
  ...
  주기 P_rot ≈ 0.97 d 를 유도. **Phase 2 회전 권장 출처** (직접
  광도 주기나 asteroseismic 주기가 없으므로 `v_sin_i` 메서드).
...
- **Saffe C. et al. 2008** — *Spectroscopic metallicities of Vega-
  ...
  arXiv:0805.3936). Fomalhaut 를 포함한 Castor 운동성단 A 형 별의
  고분해 금속도. [Fe/H] = −0.03 ± 0.05. **Phase 2 금속도 권장
  출처.**
- **Erspamer D. & North P. 2003** — *Automated spectroscopic
  abundances of A- and F-type stars using echelle spectrographs.
  II.* A&A 398, 1121 (`2003A&A...398.1121E`,
  arXiv:astro-ph/0211123). 폭넓은 A/F 표본에 적용된 자동화 DETAIL
  합성 파이프라인. Fomalhaut 에 대해 [Fe/H] = −0.27 ± 0.13 을
  보고. Castor-MG 전용 고분해 분석 (Saffe 2008) 보다 체계적으로
  낮은 값이라 Phase 2 cross-check 로만 보존하고 권장 픽으로는
  채택하지 않음.
- **Böhm-Vitense E. 2007** — *Chromospheric activity in G- and
  K-main-sequence stars, and what it tells us about stellar
  dynamos*, ApJ 657, 486 (`2007ApJ...657..486B`). 대류 envelope
  가 자기 다이나모와 색채권을 유지하기에 너무 얇아지는 경계가
  ~F5 V 에 위치함을 확립. Fomalhaut 의 `activity_measurements`
  를 Phase 2 에서 비워두기로 한 결정의 근거.
...
- **TW Piscis Austrini (HD 216803, K4V) 와 LP 876-10 (M4V) 의 별도
  ...
  DB 확장 후보로 플래그.
- **Phase 2 `disk_measurements` 인제스트.** 이 합성은
  `db/systems/fomalhaut.json` 에 `disk_measurements` 블록이 없어
  원반 파라미터를 문헌에서 직접 가져옵니다. Kalas 2005 / Boley
  2012 / White 2017 / Gáspár 2023 의 원반 기하를 DB 에 파싱해
  넣어, 하류 Kopernicus cfg writer 가 Phase 3 마크다운을 재파싱
  하지 않고도 데이터를 읽을 수 있게 해야 합니다.
...
- **A 형 별 색채권 활동도 프록시.** DB 의 `activity_measurements`
  는 의도적으로 비워 둡니다. A4 V 는 대류 envelope 가 없어 색채권
  Ca II H&K 핵이 형성되지 않고 따라서 log R'HK 지표가 정의되지
  않기 때문입니다 (Bohm-Vitense 2007 의 색채권 경계가 ~F5 V 부근,
  Fomalhaut 보다 훨씬 차가운 위치에 놓입니다). 유일한 관측적
  대응물은 ROSAT 미검출 (Schröder & Schmitt 2007, log L_X < 27.5
  erg/s) 인데, 이는 `meta_notes` 에 서술적으로만 기록합니다.
  스키마의 `x_ray` 메서드 화이트리스트가 검출용이지 상한
  미검출용은 아니기 때문입니다. 향후 XMM-Newton 이나 eROSITA
  검출이 작은 분리의 자기 활동성 동반성이 아니라 Fomalhaut A
  자체에 명확히 귀속되면, 그때 paper 인용 `x_ray` 측정치로
  격상시켜야 합니다.
