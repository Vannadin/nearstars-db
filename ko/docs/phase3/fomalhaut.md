...
질량 M = 1.92 ± 0.02 M☉ 입니다. 유효 온도 Teff ≈ 8689 K 와 대응하는
광도 L ≈ 16.6 L☉ 로 인해 Fomalhaut 는 볼로메트릭 기준 태양보다 약
17 배, 가시광 밴드에서는 약 24 배 밝게 빛납니다.
...
| `radius_rsun` | 1.842 ± 0.019 | high | Di Folco et al. 2004 — VLTI/VINCI K 밴드 limb-darkened 간섭계. θ_LD = 2.223 ± 0.022 mas |
| `teff_k` | 8689 | high | SIMBAD 컴파일레이션. Di Folco 2004 는 볼로메트릭 플럭스 + θ_LD 에서 8751 ± 90 K 를 유도, 1σ 안에서 일관 |
| `luminosity_lsun` | 16.6 | high | R 과 Teff 로 Stefan–Boltzmann 유도. Mamajek 2012 가 16.63 ± 0.48 인용 |
| `metallicity_fe_h_dex` | −0.03 ± 0.05 | medium | Saffe et al. 2008 — Castor 운동성단 A 형 별의 고분해 분광. 사실상 태양과 동일 |
| `age_gyr` | 0.44 ± 0.04 | high | Mamajek 2012 — Castor 운동성단의 등시선 + 운동학 결합. 이전 200 Myr 추정치를 대체 |
| `rotation_period_days` | 0.97 | medium | v sin i = 93 km/s (Royer 2007) + R = 1.842 R☉ 로 유도. 원반 경사 (Le Bouquin 2009) 에서 i ≈ 65° 가정 |
| `limb_darkening_alpha_h` | 0.211 ± 0.020 | high | Davis et al. 2005 — SUSI 광학 간섭계. A 형 별 limb darkening 이 태양형 G 형보다 살짝 강함 |
| `visual_surface_tint_hex_primary` | `#cfe0ff` (시원한 청백, A 형 별) | medium | Tie-break. 8689 K A4V 흑체 + 볼로메트릭 보정. Vega 의 살짝 더 푸른 A0V 색조와 시각적으로 구분 가능 |
| `stellar_color_temp_k` | 8689 | high | Teff 유도 |
...
Fomalhaut 의 광구는 Teff = 8689 K 로 A 형 주계열의 한가운데에
...
UV/FUV 플럭스는 본질적으로 높습니다. 8689 K 광구의 Wien 꼬리가
...
밝기는 T⁴ 스케일링 (Teff = 8689 K vs 5778 K → 단위 입체각당 5.1
