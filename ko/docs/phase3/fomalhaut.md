...
| `disk_warm_tint_rgb_hex` | `#f0d0a0` (따뜻한 amber, 뜨거운 내부 벨트) | low | Tie-break. 뜨거운 내부 먼지가 A 별 빛을 cold 고리보다 더 따뜻하게 산란. interesting-first |
...
| `disk_intermediate_tint_rgb_hex` | `#e0cca8` (buff, warm 과 cold 사이) | low | Tie-break. 안쪽 warm 과 바깥 cold 벨트 사이 톤 |
...
| `disk_cold_tint_rgb_hex` | `#c8ccd4` (차가운 회색, 65 K cold 고리) | low | Tie-break. 65 K 얼음 먼지가 내부 벨트보다 차갑게 읽힘. 간판 편심 고리 |
| `disk_cold_opacity` | 0.35 | low | Tie-break. 실제로는 광학적으로 얇음 (τ ≈ 10⁻⁴). 가시성 위해 boost |
| `disk_morphology` | 3 벨트 편심 구조. warm 내부 벨트 (~10–90 AU) + intermediate 벨트 (~60–110 AU) + 고전적 편심 cold 주 고리 (133–158 AU, 날카로운 안쪽 가장자리) | high | Kalas 2005 + Boley 2012 + Gáspár 2023 JWST/MIRI — 완전한 다중 벨트 구조 |
...
| `disk_imaging_inclination_deg` | 65.6 ± 0.4 (cold 고리); ~67.5 (내부 벨트, Gáspár 2023) | high | cold 고리는 Le Bouquin 2009 / Boley 2012 ALMA. 내부 벨트는 Gáspár 2023 |
...
`Ring` 으로 `disk_cold_tint_rgb_hex = #c8ccd4` — ~65 K 먼지가 A 형
별 빛을 재산란하는 모습의 차가운 회색으로 렌더링되며, 안쪽 warm
(`#f0d0a0`) 과 intermediate (`#e0cca8`) 벨트는 같은 `Rings {}` 블록
