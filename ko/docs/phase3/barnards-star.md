...
| `visual_surface_tint_hex_primary` | `#ffd487` (옅은 따뜻한 주황, M4 V) | medium | 실제 광구 색. 3195 K 에서의 Pickles 1998 관측 M4 SED 를 공유 CIE→sRGB 엔진 (`scripts/refs/stellar_photospheric_color.py`) 으로 적분하고 peak-channel 정규화한 값. 옅은 따뜻한 주황으로, 3195 K 흑체가 그럴듯한 1차 근사이며 TiO/VO 밴드는 표시 색도를 약간만 이동시킵니다 (큰 효과는 가시 색조가 아니라 색지수에 나타남). M1V AU Mic `#ffcb94` 과 더 차가운 M5.5 Proxima `#ffcc75` 에 대비해 거의 평평하며 — 모두 옅은 따뜻한 주황 계열이고 Proxima 가 아주 살짝 더 따뜻할 뿐입니다. (앞서의 렌더-포화된 brick-red 짐작값 `#cf5a30` 을 대체합니다.) |
...
NearStars 렌더러에서 바너드 별은 옅은 따뜻한 주황빛의 매우 희미한
M4 V 왜성으로 — 가장 가까운 단독성이지만 밝기 면에서는 눈에 띄지
않게 — 표현됩니다. 표면 색조는 `#ffd487` 로 인코딩되며, 실제 광구
색입니다. 3195 K 에서의 Pickles 1998 관측 M4 SED 를 공유 CIE→sRGB
엔진으로 적분하고 peak-channel 정규화한 값입니다. 3195 K 흑체가
그럴듯한 1차 근사이며, TiO/VO 밴드는 표시 색도를 약간만 이동시킵니다.
카탈로그의 M-왜성 색도는 거의 평평해서, 이 값은 사실상 M1 V AU Mic
(`#ffcb94`, Teff 가 ~470 K 더 뜨거움) 과 M5.5 Proxima (`#ffcc75`,
Teff 가 ~145 K 더 차가움, 아주 살짝 더 따뜻함) 사이에 나란히
