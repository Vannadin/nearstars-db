...
AU 의 약 0.102 L☉ 일사량은 지구의 ~9.3 배 — 어떤 합리적 거주가능
...
| `semi_major_axis_au` | 0.105 | medium | Wittrock 2023 보고값 |
...
| `insolation_s_earth` | 9.3 | high | L = 0.102 L☉ (Donati 2023, recommended) 과 a = 0.105 AU 에서 유도 |
...
| `dayside_surface_temp_k` | 530 | medium | 얇은 대기 → 제한된 주야 재분배. 주간이 T_eq 보다 살짝 높음. Cowan & Agol 2011 (`1001.0012`) 의 주야 열재순환(+ 내부열) 매개변수화를 약(弱)재순환 극한에서 적용해 추정함 |
| `nightside_surface_temp_k` | 250 | medium | 약한 대기 운반. 현무암 표면의 열 관성. Cowan & Agol 2011 (`1001.0012`) 의 열재순환(+ 내부열) 매개변수화로 추정함 |
...
| `atmosphere_scale_height_km` | 8.7 | medium | 유도. T = 450 K, μ = 43 (CO₂), g = 9.9 m/s² 의 kT/μg |
...
| `surface_tint_rgb_hex_accent` | `#a04020` | low | 매우 어린 (~22 Myr) 근접 행성의 조석 가열이 Io-analog 화산활동을 구동할 수 있음 (Driscoll & Barnes 2015). 국지적 냉각 lava hotspot 은 low-confidence 가능성이며, 일사 용융이 아님 (T_eq 478 K 가 silicate solidus 아래) |
...
| `magnetic_field_strength_microtesla_equator` | 5 | low | 지구질량 암석 행성 → **암석** dynamo 스케일링 RM22 (Rodríguez-Mozos & Moya 2022, `2203.01065`, cached) + 느린-자전 패널티 (Garraffo 2017). 활동적인 젊은 코어에도 불구하고 약한 자기장이다. (암석 행성에 적용되지 않는 거대 행성/갈색왜성 다이나모 논문인 Reiners & Christensen 2010 인용을 대체.) |
| `surface_radiation_dose_msv_yr` | 5000 | low | 얇은 대기 column (~100 g/cm²) + 약한 자기장 → AU Mic super-flare 하 높은 표면 dose. Atri 2019 (1910.09871) 가 SEP→dose 프레임워크(event 당 Gray + enhancement 계수이지 연간 mSv/yr 가 아니다)를 제공하므로, 여기 적힌 연간값은 변환 추정 → low |
...
| `star_apparent_angular_diameter_deg` | 4.4 | high | 유도. 2 × 0.862 R☉ / 0.105 AU × (180/π) ≈ 4.4° |
...
조 (`#a04020`) 는 substellar 부근의 국지적 냉각 lava hotspot 을 표현
합니다 — 이 매우 어린 (~22 Myr) 근접 행성의 조석 가열이 구동하는
low-confidence 가능성이며 (Driscoll & Barnes 2015), 일사 용융이
아닙니다 (T_eq 478 K 가 silicate solidus 아래).
...
- **전반 외관.** 철 산화물 reddening 과 밝은 substellar lava province
  를 가진 어두운 현무암 디스크. 4.4° 각지름 (지구에서 본 태양의
...
- **super-flare 동안의 오로라.** Mars-analog 가시 오로라 (`#ff6b6b`
  ...
  자기 위도에서만.
- **하늘의 별.** AU Mic 이 d 의 하늘에서 4.4° 를 차지 (지구에서 본
  태양의 8 배) — 주간 하늘을 지배하는 광활한 깊은 붉은 디스크.
  substellar 에서 표면 조명 지구의 9.3 배이지만 근적외선 쪽으로
...
- **Plavchan P. et al. 2020** — *A planet within the debris disk around the pre-main-sequence star AU Microscopii*, Nature 582, 497 (`2020Natur.582..497P`, arXiv:2006.13248). TESS 의 b 발견. d 의 대기 보전 논의에 정보를 주는 항성 활동 맥락 제공.
