...
| `insolation_s_earth` | 3.9 | low | L = 0.102 L☉ (Donati 2023, recommended) 과 a = 0.161 AU 에서 유도 |
...
| `atmosphere_scale_height_km` | 51 | low | 유도. kT/μg 에서 T = 380 K, μ = 2.5, g = 24.5 m/s² |
...
| `stellar_illumination_color_temp_k` | 3665 | high | host 별 Teff 에서 (e 의 확인 여부에 의존하지 않는 유일한 파라미터) |
...
내려가는) e 의 주간 하늘은 AU Mic 의 각지름 2.9° 가 비추는 더
깊은 크림 색조의 띠 표면으로 보일 것입니다. 표면 일사량은 지구의
3.9 배 — 상당하지만 b (20.8×) 나 c (7.2×) 보다는 낮습니다. 시각적
...
- **전반 외관.** 옅은 zonal 띠를 가진 따뜻한 sub-Neptune 디스크.
  AU Mic 의 각지름 2.9° (지구에서 본 태양의 5 배 — 여전히 지배적
...
- **하늘의 별.** AU Mic 이 e 의 하늘에서 2.9° 를 차지 (지구에서
  본 태양의 5 배) — 주간 하늘을 지배하는 적당한 크기의 붉은 디스크.
  substellar 구름 꼭대기에서의 표면 일사량은 지구의 3.9 배.
...
- **Plavchan P. et al. 2020** — *A planet within the debris disk around the pre-main-sequence star AU Microscopii*, Nature 582, 497 (`2020Natur.582..497P`, arXiv:2006.13248). TESS 의 b 발견. e 논란과 관련된 시스템의 항성 활동 맥락 정의.
