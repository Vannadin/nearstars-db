...
있습니다 (Kervella 2017 이 Kervella 2016 의 시차 747.17 ± 0.61 mas 에서
...
나오며, Kervella et al. 2016 의 수정된 이중성 궤도 fit 은 역학질량을
...
으로 ≈ 1.5 AU 떨어진 점원 후보 "S1" 을 보고했습니다. 아직 미확인이지만
이제 별도 합성을 갖춘 documented candidate 로 등재돼 있습니다 (α Centauri A b 참고).
...
| `mass_msun` | 1.1055 ± 0.0039 | high | Kervella et al. 2016. 수정된 이중성 궤도 fit |
...
| `luminosity_lsun` | 1.521 | high | R 과 Teff 로 Stefan–Boltzmann 유도. Bigot 2008 일치 |
...
- **Beichman C. et al. 2025** — *Worlds Next Door: A Candidate Giant
  ...
  (`2025NatAs.tmp.../...`, [arXiv:2508.03814](https://arxiv.org/abs/2508.03814)) 및 **Sanghi A. et al.
  2025** *Worlds Next Door II* ([arXiv:2508.03812](https://arxiv.org/abs/2508.03812)). ~1.5 AU 점원 후보의
...
- **Quarles B. et al. 2022** — *Milankovitch cycles for a
  ...
  ([arXiv:2108.12650](https://arxiv.org/abs/2108.12650)). A 또는 B 의 행성에 대한 이중성 perturbation
  obliquity 진동 envelope 설정.

### Read (instrument / non-cfg-decisive)

- **Salmon S. J. A. J. et al. 2021** — α Cen AB asteroseismic 제약을
  최신 진동 코드로 재조사 ([arXiv:2011.14932](https://arxiv.org/abs/2011.14932)).
...
α Cen A 와 B 는 **하나의 공유 astrosphere** 안에서 공전합니다. 가까운
(a ≈ 24 AU) 쌍을 감싸는 단일 항성풍 공동이 그것입니다 (Wood 2001). 결합
항성풍 Ṁ ≈ 2 Ṁ⊙ 가 이 공동을 부풀려 nose standoff 가 **~176 AU** (태양
heliopause 의 약 1.5 배) 에 이르며, 시스템이 **G 구름** 에서 받는
~25 km/s 의 inflow 와 균형을 이룹니다 (Wood 2005 는 α Cen 에 LIC 가 아닌
G-cloud flow 벡터를 쓰므로, LIC-frame 천체측정값 ~19 km/s 는 여기서 쓰지
않습니다). 코로나 X 선 출력은 잠정적인 ~19 년 주기로 변동할 수 있지만,
A 는 ~2005 년 이래 Maunder-minimum 같은 저활동 상태에 잠겨 있어 (Ayres
2014) 이는 확인된 사이클이 아니라 불완전하게 샘플링된 한 번의 변동입니다.
입자 방사선 가혹도는 태양형입니다. astrosphere 필드는 AB 시스템 대표값
으로 여기에 기록하며, B 는 자기 사이클만 따로 들고 있습니다.

| Field | Value | Confidence | Basis |
|---|---|---|---|
| `solar_cycle_yr` | 19.1 (tentative) | low | Ayres 2014 X 선. Maunder-low 에서 나온 단 한 번의 지나치게 긴 변동, 반복 사이클 아님 |
...
| `astrosphere_standoff_au` | ~176 | medium | 120·√(Ṁ_rel)·(V_⊙/V_ISM). A+B 공유 공동. 태양과 같은 항성풍 속도 (V_⊙ ≈ 400 km/s, Wood et al. 2002/2005 의 고정 속도 가정) 를 전제하며, 별마다 Ṁ 과 V_ISM 만 달라집니다 |
