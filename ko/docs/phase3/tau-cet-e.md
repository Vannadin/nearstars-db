...
| `insolation_s_earth` | 1.69 | low | derived L_bol/a²: 0.488 L☉ (Teixeira 2009, recommended) / (0.538 AU)² — 지구 (1.0) 와 금성 (1.91) 사이. G8V 보수적 HZ 안쪽 가장자리 안 (Kopparapu 2014: ~1.05 S⊕) |
| `equilibrium_temp_k` (A=0) | 317 | low | derived 278 × (L/a²)^0.25 — 지구의 255 K 보다 살짝 위 |
| `equilibrium_temp_k` (A=0.3) | 290 | low | 지구형 albedo 로 derived — 액체 수 영역 안에 편하게 위치 |
...
| `stellar_illumination_color_temp_k` | 5370 | low | 호스트 Teff (Pavlenko 2012). 역사적 시나리오에만 적용 |

## Surface synthesis

Feng 2017 그림에서 τ Ceti e 는 1.69 S⊕ 에 있었습니다 — G8V 호스트의
보수적 거주가능영역 안쪽 가장자리 (Kopparapu 2014: ~1.05 S⊕) 와
비슷하거나 살짝 안쪽. 이 조사량에서 e 는 네 행성 중 "흥미로운"
하나였습니다 — 지구보다 따뜻하지만 금성형 runaway 영역은 아니며,
평형 온도는 albedo 0 에서 317 K, 지구형 0.3 bond albedo 아래
290 K — 액체 수 영역 안에 편하게 자리잡았습니다. Kopparapu et al.
2014 의 optimistic 안쪽 가장자리 (~0.75 S⊕ 까지 안쪽으로 확장) 는
2017 년과 2025 년 사이 e 가 거주가능 타겟으로 논의될 때
가장 자주 인용된 프레임워크였습니다.
...
작업 가정이었습니다. Wordsworth 2015 프레임워크 아래, 1.69 S⊕ 의
...
- **하늘의 별 (역사적).** τ Ceti 는 e 표면에서 0.786° 를 채웠을 것
  ...
  노랑 원반으로, substellar 지점에서 지구 조사량의 1.69× 를 전달.
...
- **Kopparapu R. K. et al. 2014** — *주계열별 주위 거주가능영역*,
  ...
  으로 묘사한 대중 과학 프레이밍을 견인. 보수적 (~1.05 S⊕) 과
  optimistic (~0.75 S⊕) 안쪽 가장자리 간의 차이는 철회 이전
  e 의 거주가능성 분류 논쟁의 일부였음.
- **Wordsworth & Pierrehumbert 2015** (general atmospheric-escape
  argument) — 역사적 reading 에서 가정한 1 bar 지구형 대기 바닥의
  일반 대기-유지 참조로 사용. 구체적 제목은 핀하지 않음.
...
- **Weir A. — Eridian worldbuilding 부록 문서** (사용자가 2026-05-27 에 `eridian.docx` 로 제공). 권위 있는 Erid (40 Eri A b) 파라미터. **28 atm NH₃**, **210 °C** 표면, **2.09 g**, **5.1시간 항성일** (약 18,397 초), **지구의 25배** 자기장, 밀도 5,710 kg/m³, 반경 12,835 km, 질량 8.47 M⊕ (Ma 2018 측정값). 또한 panspermia 이벤트가 **Tau Ceti** 에서 지구 + Erid 로 seeding 했음을 명시 — Adrian 이 panspermia 발원지, Erid + 지구가 수용지. 여기서는 popular summary 의 Adrian-vs-Erid 파라미터 혼동을 정리하는 용도로만 인용.
