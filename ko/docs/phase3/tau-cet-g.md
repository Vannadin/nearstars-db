...
세 행성 중 가장 안쪽이며, 호스트 광도 0.457 L☉ 기준 조사량은
**25.85 S⊕** — 더 어둡고 차가운 호스트라는 점을 빼면 거의 수성급
조명입니다. 평형 온도는 albedo 0 에서 627 K, 지구형 albedo 에서
574 K 로, H₂O 임계점보다 한참 위이고 일부 광물상의 규산염 기화에
...
없다는 의미지만, 627 K dayside 와 0.13 AU 간격에서는 7 Gyr 동안 저
...
| `equilibrium_temp_k` (A=0.3) | 574 | high | 지구형 albedo 로 derived |
...
| `atmosphere_present` | false | medium | Tie-break. 맨 암석 무대기 채택. 조용한 호스트 XUV 가 무거운 분자를 보존하지만, 627 K Teq 에서는 ~2 M⊕ 체에서 N₂ 와 CO₂ 도 7 Gyr 에 걸쳐 Jeans 탈출. 차가운 내부 super-Earth 에서는 탈가스도 너무 느림 |
...
τ Ceti g 는 **맨 암석** 영역에 있습니다. 25.85 S⊕ 조사량은
...
1. 호스트 별이 예외적으로 조용함 — log L_X ≤ 26.5 cgs (Schmitt 1985),
   ...
   는 blow-off 제한이 아님.
2. 그러나 Teq = 627 K 에서의 thermal Jeans 탈출은 무시할 수 없음.
   반지름 1.18 R⊕, 질량 1.75 M⊕ 행성의 탈출 속도는 13.7 km/s.
   627 K 에서 N₂ 의 열 속도는 0.67 km/s 로 Jeans 파라미터 21 —
...
에 있음 — 627 K Teq 에서 적당한 상부 대기 가열로 thermal Jeans 탈출이
...
25.85 S⊕ 의 옅은 노랑 G8V 조명을 받고, dayside 표면 온도는 700 K
...
**KSP 구현 노트.** 3:2 의 경우. `rotationPeriod` = (3/2) × 궤도 주기
= 30 일 (2.592×10⁶ s). 1:1 대안은 20 일 (1.728×10⁶ s).
...
- **Terminator.** grazing 5344 K G8V 빛 아래 높은 대비. 지형 그림자
