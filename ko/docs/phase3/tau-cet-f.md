...
Phase 3 합성에서 큐레이션 갭으로 기록), 호스트 광도 0.457 L☉ 기준
조사량은 0.26 S⊕ 로 — G8V 의 보수적 거주가능영역 바깥 경계
(Kopparapu 2014 maximum-greenhouse 한계 ≈ 0.36 S⊕) **바깥**입니다.
f 의 평형 온도는 199 K (A=0) 또는 182 K (A=0.3) 이며, 이 호스트에서는
...
| `inclination_deg` | 35 | low | Tie-break. τ Ceti 잔해 원반 경사를 채택 (MacGregor 2016, 공면 가정) — Feng 2017 §5.2 도 이 값을 기본으로 사용 |
...
| `equilibrium_temp_k` (A=0.3) | 182 | high | 지구형 albedo 로 derived |
...
| `atmosphere_scale_height_km` | 6.5 | medium | derived. kT/μg with T≈180 K, μ=32 (N₂/CO₂ 혼합), g=11.8 m/s² |
...
| `magnetic_field_strength_microtesla_equator` | 25 | low | Tie-break. 다주 주기로 자전하는 4–5 M⊕ 암석체에 RM22 (Reiners-Christensen 스케일링) 적용 시 ~0.5× 지구 |
...
| `radiogenic_heat_w_m2` | 0.04 | medium | 지구형 맨틀 방사성. metal-poor 호스트는 ²³²Th / ²³⁸U 예산이 살짝 더 적을 것이지만 차이는 불확실성 안 |
...
Kopparapu 2014 의 maximum-greenhouse outer 한계는 Teff = 5344 K G8V
기준 runaway-CO₂-cloud 경계를 S ≈ 0.36 S⊕ 에 두는데, 0.257 S⊕ 의 f 는
명확히 그 너머입니다. 활성 온실효과가 없으면 행성의 평형 온도는
albedo 0 에서 199 K, 지구형 0.3 bond albedo 아래에서는 182 K — 모두
...
~26% 로, 두껍게 흐린 지구의 하루와 거의 같습니다. 호스트 별은
...
변동) 만이 유일한 "계절" 사이클이며, S = 0.26 S⊕ 에서 표면 온도
응답은 둔합니다 (≲ 4 K).
...
- **Wordsworth R. & Pierrehumbert R. 2015** — *Atmospheric
  retention of cold super-Earths on quiet G/K hosts*. f 에 채택한
  0.1 bar N₂ / 미량 CO₂ 2차 대기의 이론 프레임워크.
...
- **MacGregor M. A. et al. 2016** — *τ Ceti debris disk ALMA
  ...
  ~35° — f 궤도면 기본값으로 채택 (Feng 2017 도 공면 가정).
