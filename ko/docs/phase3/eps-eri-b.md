...
3.53 ± 0.03 AU, 2671 ± 17 d (7.32 년) 주기로 도는 약 0.78 M_Jup
...
되었습니다 (Llop-Sayson 2021). 천체측정의 경사각은 디스크 평면과
축퇴되어 있는데 (Roettenbacher 2022), i ≈ 34° 는 Booth 2017 의
ALMA 분해 디스크 경사각과 일치하므로 coplanar 시스템 해석을
뒷받침합니다. 투과 / 열방출 스펙트럼은 아직 존재하지 않으며, 행성의
...
35 개 cfg 픽 중 13 개는 canonical 일치, 21 개는 tie-break (스펙트럼
이나 열 지도가 없는 상태에서의 K2V 조명 하 jovian-analog 디폴트),
1 개는 documented divergence (경사각 34° 대 DB 저장값 78.8°) 입니다.
...
| `inclination_deg` | 34 | medium | Roettenbacher 2022 + 디스크 평면 coplanar (Booth 2017 디스크 i = 34 ± 2°). Documented divergence. Canonical alternatives 참조. DB 는 Llop-Sayson RV 전용 fit 의 78.81 ± 29.34° 를 저장. 거대한 불확실도 (±29°) 가 34° 를 약 1.5σ 내에 포함 |
| `mass_mjup` | 0.78 ± 0.12 | high | Llop-Sayson 2021. M sin i ≈ 0.66 M_Jup 을 디스크 정렬 i = 34° 로 deproject |
| `mass_mearth` | 248 | high | 0.78 M_Jup × 317.8 유도 |
| `radius_rjup` | 1.05 | low | Tie-break. 트랜짓 없음. Burrows 2003 / Fortney 2007 진화 트랙은 0.78 M_Jup, 0.44 Gyr, 태양 metallicity 에서 1.03–1.12 R_Jup 예측. cfg 는 중간값 1.05 를 채택. Mawet 2019 Ms-band 미검출이 hot-start 인플레이션 반경 > 1.3 R_Jup 을 배제 |
| `surface_gravity_g_earth` | 22 | medium | g = G M / R² 유도. M = 0.78 M_Jup, R = 1.05 R_Jup → 21.6 m/s² ≈ 지구의 22 배 |
| `density_g_cc` | 0.85 | medium | 유도. 0.78 M_Jup / (1.05 R_Jup)³ × ρ_Jup. 성숙한 태양 조성 jovian 과 일관 |
| `insolation_s_earth` | 0.0257 | high | 유도. S = L_star / a² = 0.320 L☉ / (3.53 AU)² = 0.0257 S⊕. 갱신된 호스트 Phase 2 L = 0.320 ± 0.011 L☉ (Baines & Armstrong 2012) 를 추적. 이전 초안은 0.34 L☉ 로 0.0273 S⊕ 를 줌 |
| `equilibrium_temp_k_a0` | 111 | high | 유도. T_eq = 278 K × (L/a²)^0.25 = 278 × 0.0257^0.25. 이전 L = 0.34 에서는 113 K |
| `equilibrium_temp_k_a03` | 102 | high | 유도. 지구 analog A = 0.3. 이전 L = 0.34 에서는 103 K |
| `bond_albedo` | 0.34 | low | Tie-break. Saturn analog 0.342 (Hanel 1983. Li 2018) 를 Jupiter 의 0.503 보다 선호. T_eq ≈ 113 K 의 차가운 구름 화학이 Jupiter 보다 더 두꺼운 암모니아 구름 데크 쪽으로 기울어 Saturn 과 닮음. Uranus 0.300 도 같은 윈도우 내 |
| `intrinsic_luminosity_w_m2` | 0.5 | low | Tie-break. Burrows 2003 의 0.78 M_Jup, 0.44 Gyr 냉각 트랙이 내부 T ~ 100 K 를 예측. cfg 는 보수적으로 0.5 W/m² 내부 flux 를 선택 (Jupiter 의 5.4 W/m² 보다 훨씬 낮음. 더 늙고 질량이 작고 덜 인플레이션된 상태와 일관) |
...
| `atmosphere_composition` | H₂ ~89%, He ~10%, CH₄ ~0.2%, NH₃ ~0.02%, H₂O ~0.01% (심층), 광화학으로 미량 CO 와 HCN | low | Tie-break. 스펙트럼 없음. Lodders 2003 의 protosolar 풍부도와 Jupiter/Saturn analog volatile inventory, T = 113 K 평형에서 절단된 응결 화학을 따르는 태양 조성 jovian 디폴트 |
| `atmosphere_scale_height_km` | 18 | medium | 유도. H = kT / (μ m_H g). T = 113 K, μ = 2.3, g = 22 m/s² ≈ 18 km |
| `atmosphere_tint_rgb_hex` | `#d8c098` (K2V 조명 하의 따뜻한 크림 limb haze) | low | Tie-break. 암모니아 얼음 대기는 본질적으로 거의 흰색. K2V 5180 K SED 가 Sun 조명 Jupiter 대비 따뜻한 쪽으로 인식을 이동 |
...
| `magnetic_field_strength_microtesla_equator` | 400 | medium | jovian 다이나모 스케일링. Jupiter 표면 자기장 ≈ 430 μT (적도). 0.78 M_Jup 의 강한 대류 + Jupiter-like 자전이라면 cfg 는 400 μT 채택. Jupiter 의 약 7% 아래로 낮은 질량에서의 약간 약한 대류 구동을 반영 |
| `magnetic_dipole_moment_normalized_earth` | 17000 | medium | 스케일링. Jupiter dipole moment ≈ 20 000 × Earth. ε Eri b 는 0.78 M_Jup 이므로 선형 질량 스케일링으로 ≈ 17 000 × Earth |
...
| `star_apparent_angular_diameter_deg` | 0.111 | high | 유도. 2 R_star / a = 2 × 0.735 R☉ / 3.53 AU × (180/π). 갱신된 호스트 Phase 2 R = 0.735 ± 0.005 R☉ (Baines & Armstrong 2012 PAVO/CHARA 간섭계) 를 추적. Jupiter 에서 본 태양의 0.10° 와 비슷. 이전 초안은 0.759 R☉ 로 0.115° |
| `stellar_illumination_color_temp_k` | 5039 | high | host Phase 3 (`docs/phase3/eps-eri.md`) 에서 상속. 갱신된 Phase 2 T_eff = 5039 ± 126 K (Baines & Armstrong 2012 간섭계) 를 추적. 이전 초안에서는 DB 광도 근사값 5180 K |
...
3.5 AU 에서 L_star = 0.320 ± 0.011 L☉ (Baines & Armstrong 2012,
갱신된 Phase 2 앵커) 아래의 평형 온도는 T_eq ≈ 111 K (A = 0) 혹은
102 K (A = 0.3) 로, 두 값 모두 Jupiter (T_eq ≈ 110 K) 보다
...
113 K 에서의 대류 구동이 Jupiter 보다 약하기 때문입니다. 그래도
...
Atreya 1999 의 T = 113 K 평형에서 절단된 응결 화학을 따르는 태양
...
1 bar 위에서는 압력이 H ≈ 18 km 의 척도 높이로 감소합니다. 더
...
값을 선택하는 이유는 0.78 M_Jup 의 질량과 protosolar metallicity
...
0.78 M_Jup 에 대한 Hill sphere 반경은 r_H ≈ 0.36 AU = 280 R_planet
...
지배합니다. ε Eri 별 자체는 작은 0.111° 의 점입니다 — Earth 에서
...
Jupiter 의 가시광 오로라보다 약 10 배 밝습니다.
...
작은 질량 (0.78 M_Jup) 과 약한 열방출이 자전 측정을 지금까지
가로막아 왔습니다.
...
M_Jup) ≈ 16.1 h. 0.78 M_Jup 의 ε Eri b 는 3.5 AU 의 disk accretion
...
**자기 다이나모 기대.** 강한 대류를 가진 H₂/He envelope 와 Jupiter 급
자전 주기 (10 h) 가 강한 dipolar 자기장을 유지합니다. Jupiter (표면
적도 자기장 ≈ 430 μT, dipole moment ≈ 2 × 10⁴ × Earth) 에서 선형
질량 스케일링하면 ε Eri b 는 ≈ 400 μT 와 ≈ 1.7 × 10⁴ × Earth 가
되며, 둘 다 cfg 에서 채택됩니다. dipole tilt 는 시각적으로 명확한
...
- **오로라 고리.** 고자기 위도 (cfg 에서 ~70°) 의 북남 양극 모두에서
  ...
  강화된 항성풍을 고려하면 Jupiter 의 가시광 오로라보다 ~10 배 더
  밝음. nightside 각도에서만 보이며, dayside 에서는 구름 데크에
  가려짐.
- **하늘의 별.** ε Eridani 는 ε Eri b 에서 0.111° 시지름 — Earth 에서
...
- **고리 시스템.** 고리 없음 (cfg `ring_present = false`). Mawet 2019
  ...
  않음).
- **Hill sphere 맥락.** r_H ≈ 0.36 AU = 280 R_planet. 상당한 위성
...
|---|---|---|---|
| `inclination_deg` | 34° (Roettenbacher 2022 + 디스크 평면 coplanar 해석. Booth 2017 ALMA 디스크 경사각과 일치) | 78.81° ± 29.34° (Llop-Sayson 2021 RV 전용 fit. DB Phase 2 에 그대로 저장) | Llop-Sayson 값은 불확실도가 매우 크고 (±29°) RV 만으로는 사실상 제약되지 않음 — 경사각에 대한 사전 분포가 거의 균일. Roettenbacher 2022 의 천체측정 + 디스크 평면 결합 해석은 같은 통계 지지 수준에서 34° 를 선호하고, Booth 2017 의 분해 디스크 경사각 34 ± 2° 가 독립적인 coplanarity 논증을 제공. cfg 는 coplanar 34° 를 채택하는데, host Phase 3 의 `disk_imaging_inclination_deg` 채택과 일관되고 물리적으로 자기 일관적 시스템 구조를 만들기 때문. DB 저장값 78.81° 는 추적성을 위해 그대로 보존되며 `eps_eri_b` Phase 2 갱신 시 업데이트 예정. |
...
- **Mawet D. et al. 2019** — *Deep Exploration of ε Eridani with
  ...
  33 (`2019AJ....157...33M`, arXiv:1810.03794). L′ / Ms band 에서의
...
- **Llop-Sayson J. et al. 2021** — *Constraints on the Nature of ε
  ...
  arXiv:2108.05552). RV + Hipparcos/Gaia 천체측정 결합 fit. M sin i
  ≈ 0.66 M_Jup. 디스크 정렬 i = 34° 로 deproject 한 질량
  0.78 ± 0.12 M_Jup. a = 3.5 AU. **핵심 관측 논문**.
- **Roettenbacher R. M. et al. 2022** — *No Reliable Astrometric
  Detection of ε Eridani b*, AJ 163, 19 (`2022AJ....163...19R`,
  arXiv:2110.10643). 여러 미션에 걸친 천체측정 cross-check. 개별
...
- **MacGregor M. A. et al. 2015** — *ALMA Observations of the
  ...
  (`2015ApJ...809L..47M`, arXiv:1505.03879). ALMA 1.3 mm 촬영이
...
- **Booth M. et al. 2017** — *The Northern arc of ε Eridani's
  ...
  (`2017MNRAS.469.3200B`, arXiv:1705.05868). 다중 파장 분해. 삼중
...
- **Su K. Y. L. et al. 2017** — *The Inner 25 AU Debris Distribution
  ...
  arXiv:1703.10330). "Genie" 다중 belt 조각 sculpting 모델. 3 AU
...
- **Burrows A. et al. 2003** — *Beyond the T dwarfs* 와 관련 차가운
  ...
  질량-반지름-나이 트랙. 0.78 M_Jup, 0.44 Gyr 에서 ε Eri b 의 반지름
  과 내부 광도 추정에 사용.
...
- **Reiners A. & Christensen U. R. 2010** — *A magnetic field
  ...
  A&A...522A..13R`). jovian 다이나모 스케일링. Jupiter 스케일링
  에서 cfg 의 400 μT 선택과 일관.
...
전체 필터링된 bibliography 는 `phase3/eps_eri/system.yaml` 과
워크스페이스 노트의 host star Phase 3 audit trail 에 보존됩니다.
별도의 `docs/phase3/_bib/eps-eri-b.yaml` 은 생성되지 않습니다.
b 에 관련된 논문 집합이 host bibliography 안에 완전히 포함되어
있기 때문입니다.
...
- **경사각-DB 일관성**. cfg 는 i = 34° (디스크와 coplanar.
  Roettenbacher 2022 + Booth 2017) 를 채택하지만 DB Phase 2 는
  여전히 i = 78.81° ± 29.34° (Llop-Sayson 2021 RV 전용) 를 저장.
  향후 Phase 2 갱신은 DB 를 coplanar 값으로 업데이트하거나 두 값
  모두를 대안 측정치로 보유해야 함.
- **위성 시스템 Phase 3**. Hill sphere 반경 ~0.36 AU 는 상당한 위성
