...
| `insolation_s_earth` | 0.027 | high | 유도. S = L_star / a² = 0.34 L☉ / (3.53 AU)² = 0.0273 S⊕ |
| `equilibrium_temp_k_a0` | 113 | high | 유도. T_eq = 278 K × (L/a²)^0.25 = 278 × 0.0273^0.25 |
| `equilibrium_temp_k_a03` | 103 | high | 유도. 지구 analog A = 0.3 |
...
| `cloud_tint_rgb_hex` | `#e8dac4` (따뜻한 크림. NH₃ 얼음 + K2V 조명) | low | Tie-break. Saturn 의 암모니아 구름은 태양광 하에서 크림-흰색으로 보임. 5180 K K2V 하에서는 따뜻한 크림 쪽으로 더 이동 |
...
| `star_apparent_angular_diameter_deg` | 0.115 | high | 유도. 2 R_star / a = 2 × 0.759 R☉ / 3.53 AU × (180/π). Jupiter 에서 본 태양의 0.10° 와 비슷 |
| `stellar_illumination_color_temp_k` | 5180 | high | host Phase 3 (`docs/phase3/eps-eri.md`) 에서 상속 |
...
3.5 AU 에서 L_star = 0.34 L☉ 아래의 평형 온도는 T_eq ≈ 113 K (A = 0)
혹은 103 K (A = 0.3) 로, 두 값 모두 Jupiter (T_eq ≈ 110 K) 보다
...
띱니다. 5180 K K2V 조명 아래에서는 태양광 Jupiter 팔레트 대비 더
...
지배합니다. ε Eri 별 자체는 작은 0.115° 의 점입니다 — Earth 에서
...
- **오로라 고리.** 고자기 위도 (cfg 에서 ~70°) 의 북남 양극 모두에서
  ...
  가려짐.
- **하늘의 별.** ε Eridani 는 ε Eri b 에서 0.115° 시지름 — Earth 에서
