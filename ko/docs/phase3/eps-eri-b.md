...
| `surface_gravity_g_earth` | 1.8 | medium | g = G M / R² 유도. M = 0.78 M_Jup, R = 1.05 R_Jup → 17.5 m/s² = 1.79 g⊕ (다른 곳에서 쓰이는 무차원 g_Earth 단위. 예: TRAPPIST-1 d = 0.624) |
...
| `atmosphere_scale_height_km` | 23 | medium | 유도. H = kT / (μ m_H g). T = 111 K, μ = 2.3, g = 17.5 m/s² ≈ 23 km |
...
| `magnetic_dipole_moment_normalized_earth` | 15600 | medium | 스케일링. Jupiter dipole moment ≈ 20 000 × Earth. ε Eri b 는 0.78 M_Jup 이므로 선형 질량 스케일링으로 ≈ 15 600 × Earth (0.78 × 20 000) |
...
0.78 M_Jup 에 대한 Hill sphere 반경은 r_H ≈ 0.237 AU = 472 R_planet
...
질량 스케일링하면 ε Eri b 는 ≈ 400 μT 와 ≈ 1.56 × 10⁴ × Earth 가
...
- **고리 시스템.** 고리 없음 (cfg `ring_present = false`). Mawet 2019
  ...
  않음).
- **Hill sphere 맥락.** r_H ≈ 0.237 AU = 472 R_planet. 상당한 위성
...
- **경사각-DB 일관성**. cfg 는 i = 34° (디스크와 coplanar.
  ...
  모두를 대안 측정치로 보유해야 함.
- **위성 시스템 Phase 3**. Hill sphere 반경 ~0.237 AU 는 상당한 위성
