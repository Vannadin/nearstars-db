...
| `disk_dust_temperature_k` | 170 (내부 따뜻); 50 (외부 차가움) | medium | Su 외 2013 두 성분 SED fit — 따뜻한 170 K + 차가운 ~50 K dust 분포 |
| `disk_tint_rgb_hex` | `#ffd9a8` (외부 차가운 띠는 따뜻한 amber, 내부 따뜻한 띠는 `#ffe4b8` 로 약간 더 밝게) | low | Tie-break. 50 K 흑체 peak 는 ~60 μm (비가시) 이므로 cfg 는 A0V 조명 하에 scattered light dust 로 읽히는 가시광 proxy 를 합성. 약간 따뜻한 amber 톤은 Hubble/HST-STIS 의 차가운 잔해 렌더링 관행 팔레트 (cf. Schneider 2014 HST-STIS Fomalhaut 렌더링). 무채색 회색 대신 interesting-first |
| `disk_opacity` | 0.06 (차가운 띠); 0.02 (따뜻한 띠); 0 in 14–62 AU 간극 | low | Tie-break. 광학 깊이 τ ~ 10⁻⁴ (Su 2013) 는 시각적 인지 한계를 한참 밑돌므로 cfg 는 플레이어가 실제로 원반을 볼 수 있도록 visible-but-faint opacity 채택. 보수적 값은 Open items 에 보존 |
| `disk_morphology` | 두 띠 (14 AU 내부 따뜻 소행성대 유사 + 62–200 AU 외부 차가운 카이퍼대 유사 + 14–62 AU cleared gap = 행성소체 휘젓는 행성 함의) | high | Su 외 2013 — 두 성분 SED + Spitzer-MIPS 영상. Cleared gap 이 Vega 에서 가장 강한 간접 행성 증거 |
| `disk_resolved_imaging` | true | high | 여러 분해 검출. Holland 1998 SCUBA 850 μm (`1998Natur.392..788H`); Wilner 2002 OVRO 1.3 mm; Su 2013 Spitzer-MIPS 24/70/160 μm; Sibthorpe 2010 Herschel-PACS; Hughes 2012 ALMA 초기 과학 880 μm |
| `disk_imaging_observatory` | IRAS (1983 발견) → JCMT-SCUBA (Holland 1998) → Spitzer-MIPS (Su 2005, 2013) → Herschel-PACS (Sibthorpe 2010) → ALMA (Hughes 2012) | high | Vega-like 잔해 원반의 표준 관측 사슬 |
| `disk_imaging_inclination_deg` | 6.2 (정면, 별의 pole-on 기하에 lock) | medium | Sibthorpe 2010 / Su 2013 — 원반 경사가 별 자전축 (i ≈ 6°) 과 일치하며 coplanar formation 지지 |
| `disk_mass_mearth` | 0.013 (차가운 띠, dust 만); +0.0003 (따뜻한 띠) | medium | Su 외 2013 — SED + 반지름에서 dust 질량 유도. 모천체 행성소체 질량은 훨씬 크지만 미관측 |
| `disk_planetesimal_belt_inferred` | true | high | 60–200 AU 에서 Poynting-Robertson + radiation pressure 제거 하의 dust 수명이 0.45 Gyr 시스템 연령보다 훨씬 짧음. 행성소체 부모 집단의 보충이 필수 (Sibthorpe 2010 §5 명시) |
...
- **잔해 원반 시각 (간판 특징).** Vega 는 IRAS 원형 잔해 원반이고 cfg 는 이를 거리에서 시스템의 시각적 지배 요소로 묘사. 원반은 두 개의 구분된 성분을 가진 정면 luminous 구조로 렌더링됩니다. ~14 AU 의 내부 따뜻한 띠는 `#ffe4b8` 의 따뜻한 amber (170 K dust 의 산란광) 로 빛나는 옅고 좁은 환형, 궤도 뷰 zoom 에서는 본질적으로 비가시이고 플레이어가 내부 시스템에 접근할 때 보이기 시작; 14–62 AU 의 cleared gap 은 미발견 행성소체-휘젓는 행성이 존재해야 할 위치를 시각적으로 식별하는 빈 환형 — cfg 가 Vega 의 "잃어버린 행성" 을 가장 강력하게 암시; 62–200 AU 의 외부 차가운 띠는 주된 시각 요소, `#ffd9a8` 따뜻한 amber 톤 (50 K dust 의 산란광 proxy 색), opacity 0.06, Sibthorpe 2010 morphology 따라 외부보다 내부 가장자리가 더 선명한 정면 밝은 ring — IRAS 가 발견한 구조이자 cfg 의 "당신은 Vega 에 있다" 단서.
...
- **가상 내부 행성에서 본 원반.** 62–200 AU 의 외부 띠가 정면 ring 으로 하늘에서 ~30° 에서 ~90° 까지 차지 — NearStars 에서 가장 시각적으로 두드러진 특징 중 하나인 striking 한 야간 "Vega ring". Pole-on 기하 덕에 ring 이 edge-on streak 가 아니라 진짜 ring (완전 환형) 으로 보입니다.
...
- **Phase 2 `disk_measurements` ingest 필요.** `db/systems/vega.json` 에 `disk_measurements` 블록이 없어 — 모든 disk Decisions 행이 DB 가 아닌 문헌을 직접 인용 중. Phase 2 refresh pass 에서 Su 2013, Sibthorpe 2010, Hughes 2012, Holland 1998 의 `disk_measurements` 항목을 `recommended: true` 로 Su 2013 두 성분 fit 에 부여하며 추가해야 함.
...
- **Su 2013 의 14–62 AU gap-clearer 행성.** 현재는 원반 morphology 에서만 유도. 미래 JWST-NIRCam 또는 extreme-AO 직접 영상 확인이 추론을 확정된 행성 body 로 전환하면 cfg 추가 트리거.
