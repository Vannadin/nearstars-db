...
| `disk_warm_tint_rgb_hex` | `#c7d8fa` (옅은 청백색) | low | 측정된 광학 색 없음 (열복사/mm 만). Mie 합성. blowout 크기의 큰 규산염 입자 (a_min ~11 µm) 가 거의 중성으로 산란하므로, 벨트가 A0V 의 청백색 별빛을 거의 그대로 반사 → 옅은 청백색 |
...
| `disk_cold_tint_rgb_hex` | `#c7d8fa` (옅은 청백색) | low | 측정된 광학 색 없음 (열복사 IR/mm 만). Mie 합성. 따뜻한 띠와 동일한 거의 중성인 큰 입자 → A0V 청백색 별빛을 반사. 두 벨트는 색조가 아니라 반지름/밝기로 구분 |
...
- **원반 dust 크기 + 색 합성 — 완료 (2026-05-30).** 팔레트 관행 tint 를 Mie 산란 합성 (`scripts/phase3/disk_color_mie.py`) 으로 교체. blowout 크기의 큰 규산염 입자 (a_min ~11 µm) 가 거의 중성으로 산란하므로 두 벨트 모두 A0V 청백색을 반사 → `#c7d8fa`. 측정된 두 원반 색 (AU Mic 의 파란색, Fomalhaut 의 회색) 으로 검증.
