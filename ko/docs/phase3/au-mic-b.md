...
`2020Natur.582..497P`. arXiv:2006.13248) 이 TESS 통과에서 행성을
...
| `insolation_s_earth` | 20.8 | high | L = 0.102 L☉ (Donati 2023, recommended) 과 a = 0.07 AU 에서 유도 |
...
| `atmosphere_scale_height_km` | 567 | high | 유도. T = 600 K, μ = 2.3 (H/He), g = 3.8 m/s² 의 kT/μg — 큰 H/He 스케일 높이가 Allart 2023 의 검출가능성을 구동 |
...
- **대기 탈출 꼬리 (행성의 시그너처 피처).** Allart 2023 의 He I
  ...
  tie-break 으로 선택.
- **하늘의 별.** AU Mic 이 b 의 하늘에서 6.6° 를 차지 (지구에서 본
  태양의 12 배) — 반구의 약 1/30 을 차지하는 광활한 깊은 붉은
  디스크. 주간 구름 꼭대기에서 표면 일사량은 지구의 약 20.8 배
...
- **Plavchan P. et al. 2020** — *A planet within the debris disk around the pre-main-sequence star AU Microscopii*, Nature 582, 497 (`2020Natur.582..497P`, arXiv:2006.13248). TESS 가 발견한 AU Mic b 의 통과 hot Neptune. 통과 유도 반지름 4.0 R⊕ (이후 4.79 로 정밀화). host XUV 환경과 대기 탈출 영역 논의. cornerstone 논문.
