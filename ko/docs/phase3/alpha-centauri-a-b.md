...
안정성으로 선택한 거의 원형 궤도 (e ≈ 0.1) ~1.6 AU 를 도는 온화한
(~225 K) 토성급 가스 행성으로, 어둡고 광학적으로 두꺼운 암석 고리에
둘러싸인 단일-관측 JWST 후보이며, 가깝고 이심한 α Cen AB 쌍성계 안에
상호 경사 ~16° (관측된 ~50° 는 Kozai-불안정) 로 놓여 있습니다.**
질량, 온도, 물-구름 대기는 논문이 선호하는 해이며 canonical 과
정렬됩니다. 반면 궤도의 이심률과 경사는 NearStars 안정성 선택값입니다
(관측된 e ≈ 0.4 / i ≈ 50° 궤도는 Kozai-불안정. `## Canonical
alternatives` 참조). 고리 (고리 없는 1.1 R_Jup 팽창 거대 행성 대신),
...
| `magnetic_field_strength_microtesla_equator` | 170 | low | Reiners & Christensen 2010 ([arXiv:1007.1514](https://arxiv.org/abs/1007.1514)) 깊이 보정 dipole 스케일링. 0.378 M_Jup·5.3 Gyr에서 B_eq ≈ 172 µT (B_pol ≈ 345 µT), 목성에 같은 식을 넣은 sanity check 통과. 기존의 미근거 ~500 µT 토성 아날로그 추측을 대체. 단 0.378 M_Jup은 거대행성 영역 하한이라 He 성층화로 더 낮아질 수도 있음 (토성 ~21 µT). canonical method는 docs/reference/planetary-dynamo-scaling.md |
...
α Cen A b 는 별빛이 데우는 만큼만 따뜻하게 빛납니다. e ≈ 0.1 궤도는
행성을 ~1.44 AU (근일점) 에서 ~1.76 AU (원일점) 까지 흔들어 ~1.5배의
완만한 일사 범위를 만들므로, 구름 데크는 근일점 근처에서 약간 밝아지고
banding 이 살짝 선명해질 가능성이 큽니다 — ~2 년 주기의 부드러운 계절
순환입니다.
...
~0.45°, 원일점에서 ~0.37° 로 흔들립니다 — 따뜻한 흰 G2V 원반
...
- **하늘의 별.** α Cen A 는 ~0.41° (근일점 ~0.45°, 원일점 ~0.37°) 를
...
## 문화 컨텍스트 — 아바타의 폴리페무스와 판도라

α Centauri A b 는 과학 언론과 2025년 후속 연구가 제임스 캐머런 *아바타*
의 가스 행성 **폴리페무스**와 동일시한 현실 속 후보입니다. 영화의 설정
에서 폴리페무스는 **Alpha Centauri A** 의 거주 가능 영역을 도는 토성급
가스 행성으로 열네 개의 위성을 거느리며, 다섯 번째 위성 **판도라**가
나비족의 고향입니다. 일치도가 높습니다. 토성급 (90–150 M⊕) 가스 행성,
측정된 고리 없음, α Cen A 의 HZ 안, 가장 가까운 태양형 별. Beichman et
al. 2025 가 S1 을 보고했을 때 NPR 과 Live Science 는 이를 "현실 속
판도라"라 불렀고, 2025년 우주생물학 논문 *Seeking the Worlds of Avatar*
는 바로 그런 거주 가능 영역 위성을 검출할 전망을 평가했습니다.
...
- **Sanghi & Beichman et al. 2025** — *Worlds Next Door II*
  ([arXiv:2508.03812](https://arxiv.org/abs/2508.03812)). 검출 검증 + 궤도 역학 논문.
