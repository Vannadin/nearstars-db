<!-- Barnard's Star b Phase 3 합성. cfg-ready 결정과 근거 -->
...
가지며, 이는 Kopparapu 2014 의 M4 V 호스트 보수적 거주 가능 영역 (HZ 안쪽
가장자리 ~0.1 AU, P ≈ 10 d) 의 한참 안쪽입니다. 행성은 transit 을 보이지
않으므로 (González Hernández 2024 + Stefanov 2024 TESS 비검출, 3σ 에서
i ≤ 87.9°), 반지름과 조성은 Msini 와 더 폭넓은 hot rocky USP analog 에서
추론됩니다.

3.15-일 궤도는 조석 잠금을 본질적으로 확실하게 만듭니다 (Walterová 2020
은 낮은 이심률 궤도의 저질량 암석 행성에서 동기 회전이 지배함을 보입니다).
substellar 점이 행성 프레임에 고정됩니다. Basant 2025 의 β-분포-prior
이심률은 e = 0.03 (+0.03/−0.02) 으로 원과 일치하며, SPOCK 안정성 분석은
장기 4-행성 공존을 위해 e < 0.02 를 선호합니다. 6.8 S⊕ 일사와 안쪽 형제
d 에 영향을 미치는 동일한 France 2020 대기 손실 환경에서, b 는 10 Gyr 에
걸쳐 어떤 1차 대기도 대부분 strip 된 것으로 예상됩니다. cfg 는 canonical
시각 기준선으로 잔여 sodium-vapor exosphere 가 있는 노출 암석 시나리오를
채택합니다.
...
35 cfg 픽. 23 canonical-aligned / measured / derived, 12 tie-break.
문서화된 divergence 없음.
...
|---|---|---|---|
| `tidally_locked` | true | high | 3.15-d 궤도 → 낮은 e 에서 동기 회전 지배 (Walterová 2020) |
| `obliquity_deg` | 0 | high | 조석 감쇠 |
| `eccentricity` | 0.03 | medium | Basant 2025 β-prior 피팅 (+0.03/−0.02). 원 궤도와 일관 |
| `sidereal_period_days` | 3.1542 ± 0.0004 | high | Basant 2025 Table 3 |
| `semi_major_axis_au` | 0.0229 ± 0.0003 | high | Basant 2025 Table 3 |
| `argument_of_periapsis_deg` | 3.8 | medium | Basant 2025 Table 3 (낮은 ecc → 약한 제약) |
| `epoch_jd` | 2460243.38 | high | Basant 2025 t₀ (BJD − 2450000 = 10243.38) |
| `mass_mearth` | 0.299 ± 0.026 | high | Basant 2025 Msini |
| `radius_rearth` | 0.72 | medium | Tie-break. 비-transit. 0.3 M⊕ 지구 analog 암석의 질량-반지름 → 0.66–0.76 R⊕. interesting-first 가 0.72 선택 (= DB 값) |
| `surface_gravity_g_earth` | 0.58 | medium | 유도 = 0.299 / 0.72² = 0.577 |
| `density_g_cc` | 4.42 | medium | 유도 = 5.513 × 0.299 / 0.72³. 0.3 M⊕ 암석체는 자기압축이 적어 지구의 5.51 아래 |
| `insolation_s_earth` | 6.78 | high | 유도 = L / a² = 0.003558 / 0.0229² |
| `equilibrium_temp_k` (A=0, full redistribution) | 438 | high | Basant 2025 Table 3 |
| `equilibrium_temp_k` (A=0, no redistribution) | 521 | high | 유도: 438 × 2^¼ (반구 dayside) |
| `equilibrium_temp_k` (A=0.1) | 426 | high | 유도: 438 × (1−0.1)^¼ |
...
| `magnetic_dipole_moment_normalized_earth` | 0.0005 | low | Tie-break. Mercury 류 매우 작은 쌍극자 (Mercury 유추, 다이너모 모델링 아님) |
| `radiation_belt_present` | false | high | 대기 없음 + 무시 가능 B-field |
| `surface_radiation_dose_msv_yr` | 7000 | low | Atri 2020 (`1910.09871`) 항성 양성자 이벤트→선량 스케일링. 무대기 + 6.8 S⊕ XUV × France 2020 (`2009.01259`) flare 듀티 사이클 |
| `atmospheric_shielding_g_cm2` | 0 | high | 무대기 |
| `aurora_present` | false | high | 대기 없음 |
| `star_apparent_angular_diameter_deg` | 4.35 | high | 유도: 2 R★ / a × (180/π) = 2 × 0.187 R☉ / 0.0229 AU |
...
d 와 구분합니다. 지배적인 톤은 산화철 basalt regolith (`#5e3526`) 로,
...
Stefanov 2024 의 TESS transit 비검출은 b 의 반지름이 아니라 궤도 경사를
i ≤ 87.9° (3σ) 로 제약합니다. Stefanov 은 강한 반지름–경사 축퇴를
지적하므로, 광도는 transit 기하를 배제하지만 특정 반지름을 못 박지는
않습니다. 따라서 cfg 의 R = 0.72 R⊕ 는 질량-반지름 추론입니다. 0.3 M⊕
지구 analog 암석 물질에 대한 스케일링은 0.66–0.76 R⊕ 를 주며 (Stefanov
이 채택한 spright N(0.76, 0.04) R⊕ 기대값과 일관), 0.72 값이 시각 구분을
위한 중간값이자 DB 반지름과 일치하도록 선택됩니다.

## Atmosphere synthesis

b 는 의미 있는 대기가 없습니다. France 2020 (Mega-MUSCLES HST + Chandra.
25% flare 듀티 사이클) 이 기술한 대기 손실 환경은 거주 가능 영역
(~0.1 AU) 에서 이미 상당한 열적 escape 를 구동합니다. b 의 한참 안쪽
0.0229 AU 에서 XUV 플럭스는 ~19× 더 높으므로 (반경 스케일링), 10 Gyr
누적 손실은 0.3-M⊕ 행성의 어떤 plausible 1차 대기 질량보다 훨씬 큽니다.
(France 2020 은 로컬 paper 캐시에 없습니다. 구체적인 escape-rate 수치는
보고서의 이전 context 요약에서 가져온 것으로, Decisions 헤드라인 값이
아니라 노출 암석 시나리오를 동기 부여하는 데만 사용됩니다.)
...
~10⁻⁹ Pa 로, 어떤 Rayleigh 산란 서명도 한참 밑돌아 가시 외관은 표면
톤에 의해 전적으로 지배됩니다 (cfg `atmosphere_tint_rgb_hex = n/a`).

Barnard 의 모데스트한 flare 동안의 에피소드 강화 (~25% 듀티 사이클,
France 2020) 가 일시적인 sputtering 증가를 구동합니다. 이는 analog
Proxima superflare 사건보다 훨씬 작지만, 10 Gyr 에 걸친 누적 대기
손실의 대부분에 기여합니다. 게임 내 효과는 sub-resolution 렌더링에
제한되며, 가시 exospheric plume 은 없습니다.

## Rotation & spin synthesis

b 는 1:1 (동기) 로 조석 잠겨 있으며 substellar 점이 행성 프레임 0°
경도에 고정되어 있습니다. Walterová 2020 은 낮은 이심률 궤도의 저질량
암석 행성에서 동기 회전이 지배적 최종 상태임을 보입니다 (더 높은
spin-orbit 공명은 상당한 이심률이 필요). 0.16 M☉ M 왜성 주위 0.0229 AU
의 0.3-M⊕ 암석 행성의 despinning 시간 척도는 10 Gyr 시스템 나이보다
훨씬 짧습니다. 경사 또한 0° 로 감쇠.
...
비 3.15:4.12 ≈ 4:5. 2.34:3.15 ≈ 3:4). 그러나 시스템은 Basant 2025 의
안정성 분석 당 엄격한 mean-motion 공명에는 *없습니다*. 행성들은 공명
잠금 없이 장기 안정성을 위해 충분히 잘 분리되어 있습니다.
...
Barnard 는 b 의 하늘에서 4.35° 를 차지하며, 지구에서 본 태양의 약 8×
입니다. 어둑한 빨강 조명은 영구히 어둑한 따뜻한-오렌지 표면 조명을
만듭니다. 누적 표면 dose 는 정상 플레이에서 치명적으로 유지됩니다 (cfg
...
cfg 의 flare-duty-cycle 속도 (~25%) 에서 일시적인 주변광 brightening
으로 나타나며, exotic 스펙트럼 특징은 없습니다. 게임 내 렌더링은 더
차가운 톤 basalt 표면을 강조해 b 를 더 뜨겁고 partial-melt 액센트가
있는 d 와 시각적으로 구분해야 합니다.
...
Atri 2020 방사선-dose 스케일링 연구 (표면 dose 행에 context-cite 로만
사용), Barnard 를 RV 표준으로 언급하는 기기 개발 논문에 대한 역사적
2024 년 이전 논문. `docs/phase3/_bib/barnards-star-b.yaml` 에 `status:
skipped` 로 보존.
...
- **직접 반지름 측정**. transit 없음. 미래 astrometric 검출 (Gaia
  ...
  cfg 의 `radius_rearth = 0.72` 는 tie-break 이며, 유도 `density_g_cc`
  와 `surface_gravity_g_earth` 가 여기서 cascade 되므로 측정된 반지름
  은 세 값을 함께 갱신합니다.
- **질량-반지름에 의한 조성**. 반지름 없이는 암석 대 얼음 풍부가
  축퇴됩니다. cfg 는 지구 analog 암석 (ρ ≈ 4.4 g/cc) 을 기본값으로
  사용하며, Mercury 류 철 농축 대안 (더 작은 R, 더 높은 ρ) 도 윈도우
  내이며 cfg 변종으로 보존됩니다.
...
- **이심률 vs. 안정성**. Basant 2025 의 e = 0.03 (+0.03/−0.02) 은 b
  가 정확히 원이거나 약하게 이심률인지를 열어둡니다. 30 cm/s 정밀도의
