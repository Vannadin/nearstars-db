...
10 d (a ≈ 0.115 AU) 에 있으며, e 는 6.74 d 에서 그 경계 바로 안쪽에
자리합니다 — 네 행성 중 HZ 에 가장 가깝지만 여전히 폭주 온실 매개
변수 공간에 단단히 위치합니다. cfg 는 e 를 경계선 금성 analog 로
해석합니다 — XUV-구동 escape 를 통해 10 Gyr 에 걸쳐 어떤 1차 물
대기도 잃기에 충분히 뜨겁지만, France 2020 의 손실 속도에 대해 모데
스트한 2차 CO₂ 대기가 plausible 하게 유지될 수 있는 네 중 유일한
것입니다.

궤도 이심률은 e = 0.04 ± 0.04 (Basant 2025 β-prior). 원과 일관 . 안정
성은 장기 4-행성 공존을 위해 e < 0.02 를 요구합니다 (SPOCK 분석 당).
6.74-일 주기는 조석 잠금 시간 척도를 10⁵ 년 아래로 두어 (Walterová
2020) 동기 spin-orbit 구성을 보장합니다. c 와의 궤도 주기 비 (6.739 /
4.124 = 1.634) 는 5:3 mean-motion 공명에 가깝지만 정확하지는 않습니다.
...
|---|---|---|---|
| `tidally_locked` | true | high | 6.74-d 궤도 → 조석 잠금 < 10⁵ yr (Walterová 2020) |
| `obliquity_deg` | 0 | high | 조석 감쇠 |
| `eccentricity` | 0.04 | medium | Basant 2025 β-prior 피팅. 원과 일관 |
| `sidereal_period_days` | 6.7392 ± 0.0028 | high | Basant 2025 |
| `semi_major_axis_au` | 0.0381 ± 0.0005 | high | Basant 2025 |
| `argument_of_periapsis_deg` | −27.5 | medium | Basant 2025 (낮은 ecc → 약한 제약) |
| `epoch_jd` | 2460245.3 | high | Basant 2025 t_peri |
| `mass_mearth` | 0.193 ± 0.033 | high | Basant 2025 Msini |
| `radius_rearth` | 0.64 | medium | Tie-break. 비-transit. 0.19 M⊕ 지구 analog 암석의 질량-반지름 → 0.58–0.69 R⊕. interesting-first 가 0.64 선택 |
| `surface_gravity_g_earth` | 0.47 | medium | 유도 = 0.193 / 0.64² |
| `density_g_cc` | 4.0 | medium | Tie-break. 네 중 가장 낮은 밀도 (질량-반지름 기울기). 약간 휘발성-농축 가능 |
| `insolation_s_earth` | 2.45 | high | L = 0.00356 L☉ 와 a = 0.0381 AU 로 유도 |
| `equilibrium_temp_k` (A=0, full redistribution) | 340 | high | Basant 2025 Table 3 |
| `equilibrium_temp_k` (A=0, no redistribution) | 405 | high | dayside 공식으로 유도 |
| `equilibrium_temp_k` (A=0.3) | 311 | high | 유도 |
...
| `atmosphere_scale_height_km` | 11 | medium | 유도. kT/μg with T = 340 K, μ = 44, g = 4.6 m/s² |
...
| `magnetic_dipole_moment_normalized_earth` | 0.0003 | low | Tie-break. 네 중 가장 작음 |
| `radiation_belt_present` | false | medium | 얇은 대기 + 약한 B-field → 기껏해야 marginal trapped 집단 |
| `surface_radiation_dose_msv_yr` | 3000 | medium | Atri 2020 스케일링. 얇은 대기 차폐 (~10 g/cm²) + 2.45 S⊕ XUV × France 2020 듀티 사이클 |
...
| `star_apparent_angular_diameter_deg` | 2.61 | high | 유도. 2 R★ / a × (180/π) |
...
의 0.64 R⊕ 선택은 중간값. 4.0 g/cc 의 밀도는 네 행성 중 가장 낮음 —
(a) 약간의 휘발성 농축 (mantle 의 H₂O / CO₂ 가 몇 wt%) 또는 (b) 단순
히 작은 암석 행성에 대한 질량-반지름 기울기와 일관. 직접 반지름 측정
이 구분할 수 있지만 transit 검출은 존재하지 않습니다.
...
AU 의 0.19 M⊕ 암석 행성에 대한 조석-잠금 시간 척도는 Walterová 2020
에서 10⁵ 년을 한참 밑돌아 10 Gyr 나이보다 훨씬 짧습니다. 경사는 0°
로 감쇠.

Basant 2025 의 최적합 e = 0.04 ± 0.04 는 원과 일관. SPOCK 안정성은
장기 4-행성 거주를 위해 e < 0.02 를 요구합니다. 권장 낮은 이심률에서
의 libration 진폭은 < 1° 이며 측정 가능한 diurnal 변조를 만들지
않습니다.

e 의 6.74-일 궤도는 네 행성 중 가장 깁니다. 보수적 HZ 안쪽 가장자리
(10 d) 와의 근접성은 ~10–20 d 의 실제 HZ 행성에 대한 미래 RV 기반
검출에 가장 관련된 타겟이 되게 합니다. Basant 2025 는 10–42 d 윈도우
에서 ~0.5 M⊕ 보다 큰 행성을 99% 신뢰도로 명시적으로 배제합니다.
...
- **Duvvuri G. et al. 2021** — *Reconstructing the EUV Emission of
  Cool Dwarfs* (arXiv:2102.08493). quiescent EUV 재구성. 대기 escape
  모델링 입력.
