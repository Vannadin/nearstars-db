<!-- Barnard's Star c Phase 3 합성. 4행성 중 가장 무거운 sub-Earth, 금성에 가까운 평형 온도 -->
...
1/3 이상인 유일한 것입니다. 0.0274 AU 의 4.12-일 궤도에서 지구의
4.7× 일사를 받으며, 평형 온도는 400 K (Basant 2025 Table 3, A =
0, 완전 열재분배) — 초기 금성에 비교됩니다. González Hernández
2024 ESPRESSO 데이터의 후보였고, Basant 2025 MAROON-X 분석이 ESPRESSO
데이터와 독립적으로 c 신호를 회수해 확정 상태로 격상했습니다.

궤도 이심률은 Basant 2025 β-분포 prior 에서 e = 0.08 ± 0.06 — 네
행성 중 가장 높은 명목 이심률 값이지만, 여전히 1.3σ 에서 원과 일관
하며 e < 0.02 의 장기 안정성 요구사항과도 일관합니다. b 와의 궤도
주기 비 (4.124 / 3.154 = 1.31) 는 4:3 mean-motion 공명에 가깝지만
정확하지는 않습니다. Basant 2025 SPOCK 안정성 테스트는 4-행성 구성이
명시적 공명 잠금 없이 10⁹ 궤도에 걸쳐 안정함을 발견합니다.
...
transit 을 보이지 않고 (Stefanov 2024 의 암묵적 비검출), 반지름 / 조성
은 질량-반지름 스케일링에서 추론됩니다. France 2020 의 대기 escape
계산 — HZ (0.1 AU) 에서 87 지구대기 Gyr⁻¹ — 은 c 의 0.0274 AU 에서
누적 손실이 단위 면적당 ~13× 더 높음을 의미하며, 다시 의미 있는
1차 대기 유지를 배제합니다. cfg 는 b 와 d 에 사용된 동일한 노출 암석
해석을 채택합니다. c 는 약간 더 낮은 일사 (더 차가운 표면 톤) 와 네
행성 중 가장 높은 절대 질량 (작은 형제들 대비 약간 더 큰 하늘에서의
각지름) 으로 시각적으로 구별됩니다.
...
| `eccentricity` | 0.08 | medium | Basant 2025 β-prior 피팅. 네 행성 중 가장 높은 명목 e. 그러나 안정성은 e < 0.02 선호 |
| `sidereal_period_days` | 4.1244 ± 0.0006 | high | Basant 2025 |
| `semi_major_axis_au` | 0.0274 ± 0.0004 | high | Basant 2025 |
| `argument_of_periapsis_deg` | 90.8 | medium | Basant 2025 |
| `epoch_jd` | 2460242.92 | high | Basant 2025 t_peri |
| `mass_mearth` | 0.335 ± 0.030 | high | Basant 2025 Msini |
| `radius_rearth` | 0.74 | medium | Tie-break. 비-transit. 0.34 M⊕ 지구 analog 암석의 질량-반지름 → 0.69–0.79 R⊕. interesting-first 가 0.74 선택 |
| `surface_gravity_g_earth` | 0.61 | medium | 유도 = 0.335 / 0.74² |
| `density_g_cc` | 4.6 | medium | Tie-break. 작은 형제들보다 약간 더 낮은 밀도 (질량-반지름 기울기) |
| `insolation_s_earth` | 4.74 | high | L = 0.00356 L☉ 와 a = 0.0274 AU 로 유도 |
| `equilibrium_temp_k` (A=0, full redistribution) | 400 | high | Basant 2025 Table 3 |
| `equilibrium_temp_k` (A=0, no redistribution) | 476 | high | dayside 공식으로 유도 |
| `equilibrium_temp_k` (A=0.1) | 390 | high | 유도 |
...
| `magnetic_dipole_moment_normalized_earth` | 0.001 | low | Tie-break. c 가 가장 무거우므로 작은 형제들보다 약간 더 큼 |
...
| `star_apparent_angular_diameter_deg` | 3.63 | high | 유도. 2 R★ / a × (180/π) |
...
~600 K 에 도달 — basalt 광물의 열 weathering 에는 충분하지만, d 에서
보이는 silicate 용융 임계값보다는 한참 아래입니다. cfg 는 이를 partial-melt
액센트의 부재로 인코딩합니다 (`surface_tint_rgb_hex_accent = #6e3d2c`
는 terminator 산능 bedrock 용) — c 는 따뜻한-오렌지 substellar 글로우
가 없다는 점에서 d 와 시각적으로 구별됩니다.

substellar 반구는 산화철 basalt 평원이 지배합니다. 더 이른 (더
뜨거운) 초기 열 단계에서의 가능한 relict 화산 지형이 존재할 수 있지만
직접 추론은 불가합니다. cfg 의 `surface_morphology` 필드는 canonical
...
c 는 1:1 (동기) 로 조석 잠겨 있습니다. 0.16 M☉ M 왜성 주위 0.0274 AU
...
β-prior 피팅의 e = 0.08 ± 0.06 가 권장 값이지만, 안정성 제약 (SPOCK
10⁹-궤도 테스트) 은 e < 0.02 를 선호합니다. e = 0.08 에서의 libration
진폭은 사소하지 않을 것이며 (~5°), 측정 가능한 diurnal 일사 변동
을 만들 수 있습니다. 권장 안정성-제약 e < 0.02 에서는 libration 이
무시 가능합니다.

c 는 컴팩트 시스템 아키텍처에 참여하지만 b 와 엄격한 4:3 mean-motion
공명에는 있지 않습니다 (주기 비 1.31, 정확히 4/3 아님). 10 년 항성
활동 사이클이 일사를 수 퍼센트 변조 — cfg 의 스칼라 해상도에서 시각
변화를 구동하기에 너무 작음.
...
Barnard 는 c 의 하늘에서 3.6° 를 차지 — 지구에서 본 태양의 약 7×
. 어둑한 빨강 조명은 dayside 에 영구 따뜻한 황혼을 만듭니다.
nightside 는 완전히 비조명 (알려진 위성이 없으므로 위성-조명 효과
없음).

aurora cfg 필드는 모두 `false` / `n/a`. Barnard 의 flare 사건은 25%
듀티 사이클로 일시적인 주변광 brightening 으로 나타납니다. 누적 표면
dose 는 정상 플레이 조건 하에서 치명적으로 유지됩니다.
...
시각적 강조는 c 를 "중간 형제" 대비로 — 그을린 뜨거운 안쪽 쌍도 비교
적 차가운 가장 바깥 e 도 아닌 것으로 — 두는 것에 있습니다.
...
- **Basant R. et al. 2025** — *Four Sub-Earth Planets Orbiting
  ...
  0.030 M⊕, a = 0.0274 AU, T_eq = 400 K (A=0, full redistribution).
...
- **Toledo-Padrón B. et al. 2019** — *Stellar activity analysis of
  Barnard's Star* (arXiv:1812.06712). 호스트 회전 + 활동.
...
에 `status: skipped` 로 보존.

## Open items for follow-up

- **직접 반지름 측정**. 네 행성 모두와 마찬가지로 transit 없음.
  미래 astrometry 또는 직접 영상화가 cfg 의 `radius_rearth = 0.74`
  placeholder 를 개선할 수 있습니다.
- **고-이심률 vs. 안정성**. Basant 2025 β-prior e = 0.08 은 네 행성
  중 가장 높은 명목값. 그러나 안정성은 e < 0.02 를 선호. 30 cm/s
  정밀도 RV 추가 모니터링이 이심률을 개선할 수 있습니다.
- **가장 무거운 sub-Earth 에 대한 질량-반지름 관계**. c 는 순수
  암석 (지구 analog) 과 약간 얼음 풍부 (예. 휘발성-농축 mantle 의
  Mars analog) 조성의 경계에 있습니다. cfg 는 암석을 기본값으로.
  asteroseismic 또는 직접 영상 캠페인의 미래 조성 제약이 개선할 수
  있습니다.
