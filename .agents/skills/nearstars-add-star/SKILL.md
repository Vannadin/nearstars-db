...
### Stellarium Web skysource ID lookup — automatic

`docs/index.html`'s Stellarium button falls back to `stellarium-web.org`
when the desktop app at `localhost:8090` is offline. The fallback needs
a noctuasky-resolved catalog ID. Pre-computing it at star-addition time
(a) makes the click instantaneous and (b) survives noctuasky outages.

**`run_pipeline.sh` runs this automatically as step 5/7.** New
components are queued; already-mapped components are skipped
per-component. No manual action required after appending a new system
to `target_list.json`.

Manual invocations for debugging / targeted work:
```bash
python3 scripts/pipeline/fetch_stellarium_ids.py                # new components only
python3 scripts/pipeline/fetch_stellarium_ids.py --retry-failed # re-query null entries
python3 scripts/pipeline/fetch_stellarium_ids.py --only "Procyon,GJ 412 A"
python3 scripts/pipeline/fetch_stellarium_ids.py --dry-run      # print only, no save
```

Lookup procedure (inside the helper):
1. Try the component name directly against the noctuasky API.
2. On failure, walk the SIMBAD `ident` table (GJ, HIP, HD, Bayer
   designation, etc.) and try each alias.
3. If both fail, store `null` — the HTML runtime path retries at click
   time, so a `null` here is harmless.

Result is saved in place to `db/target_list.json` under each system's
`stellarium_ids: {component: id}` field.
