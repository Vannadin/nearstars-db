---
title: REX (Real Exoplanets) vs NearStars 데이터 비교
status: active
created: 2026-05-21
---
...
NearStars README는 "spiritual successor to REX (KSP 1.8.0 era)" 라고만 명시할 뿐 정량 비교는 없음. 이 노트가 그 갭을 메움 — 커버리지, 수치값, 다성계 토폴로지, 출처.
...
(2020-04-11) 핀. NearStars 측은 `db/systems/` 의 recommended 값,
기준일 `2026-05-21`. **DB 변경 없음** — 발견만 기록.
...
Eps Eri, Eps Ind, Eta Cas, 그리고 GJ 계열 다수 + shortlist
([[project_nearstars_implementation_shortlist]]).
...
body의 "표면 온도"를 그 필드로 인코딩하기 때문에 REX가 실제 이렇게
인코딩함.
...
3-way 비교(REX/NS/Stellarium)는 [[reference_stellarium_comparison]] —
Stellarium도 하이어라키컬 다성계는 미지원이지만 NS의 Hilditch 회전
규약은 일치.
...
5. **Jacobi 다성계 선택 검증.** REX의 flat "Proxima-around-Sun"
   ...
   이게 [[reference_binary_epoch]] 파이프라인이 존재하는 정당화 자체.
