...
#
# ⚠ **기본이 2 다. 8 이 아니다** (오너 지시 2026-09-10 밤: 기계가 조용해야 한다). 논리 코어는 15 고
#   메모리는 남지만, **동시성은 기계를 시끄럽게 하는 축**이라 그 판단은 여기 코드가 하지 않는다.
#   8 의 벽시계 실측은 낮에 `GATE_POOL=8 ./scripts/check.sh …` 로 따로 잰다. `1` 은 완전 직렬로
#   되돌리는 손잡이이고, 그 세 값이 같은 코드 경로를 지난다 — 판정선이 «풀 크기» 하나만 다르다.
