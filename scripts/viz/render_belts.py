...
def render(body, out, size=560, z=0.0):
    ...
    if inner and inner.get('on',True):
        d = torus_sdf(Xr,Yr,Z,inner); m=d<0
        dose[m]+=np.clip(inner.get('grad',3.3)*(-d[m])/inner['rad'],0,1)*inner['radiation']
    if outer and outer.get('on',True):
        d = torus_sdf(Xr,Yr,Z,outer); m=d<0
        dose[m]+=np.clip(outer.get('grad',2.2)*(-d[m])/outer['rad'],0,1)*outer['radiation']
