...
## v11 — eccentric disk rings (why it differed from JWST)
**Diagnosis:** the Fomalhaut disk data is *already* JWST-faithful (Gáspár 2023 + ALMA White 2017 + HST Kalas 2005). The mismatch with JWST images was **rendering, not data**: (1) we drew *circular* concentric rings, but Fomalhaut's rings are **eccentric** (main ring e=0.11, star off-centre; intermediate belt e≈0.3, misaligned) — the famous offset asymmetry; (2) the inner disk (10–90 AU, PR-drag-fed, broad) genuinely **overlaps** the intermediate belt (60–110 AU), so a clean "3 separate rings" is partly the false-colour impression — radially they overlap.
**Fix (b+c):** added `eccentricity` to the disk schema + the cited values to disks_curated.json (main_cold 0.11, intermediate 0.3); the viewer now draws each belt as confocal in/out ellipses with the star at the focus (offset ring), tilted by inclination — instead of circular `RingGeometry`. The eccentric intermediate belt is now visually distinguishable from the circular broad inner disk (the honest "separation" — they really do overlap in radius). Belt-to-belt misalignment direction isn't quantified in the data, so not rendered (only the eccentric offset, which is cited).
