...
def _rewrite_md_href(href: str) -> str:
    """Rewrite an internal .md link to its rendered .html target so Related /
    cross-reference links resolve on the published site (Phase 3 pages live in
    docs/phase3/; reference & plans docs render into the doc-hub docs/wiki/).
...
def inline_md(text: str) -> str:
    ...
    # links: [text](url) — rewrite internal .md targets to their rendered .html
    out = _LINK_RE.sub(lambda m: f'<a href="{_rewrite_md_href(m.group(2))}">{m.group(1)}</a>', out)
    # inline code
...
def render_html(slug: str, title_en: str, title_ko: str, pairs: list[tuple[Block, Block]]) -> str:
    ...
    for en, ko in pairs:
        if en.kind == 'heading':
            ...
            if decisions:
                # 60행 넘는 결정 표를 눈으로 훑지 않아도 되게 필드명 필터를 붙인다.
                body_html_parts.append(
                    '<div class="card"><div class="dfilter">'
                    '<input class="dfin" type="search" autocomplete="off" '
                    'placeholder="Filter fields…" aria-label="Filter fields">'
    ...
    # Forward spine link: the Phase 4 board for this body, when one exists.
    # Board body-slugs equal Phase 3 slugs (art-name-only bodies have none),
    # so a glob match is authoritative — no board-side metadata needed.
    p4_boards = [p for p in sorted(REPO.glob(f'docs/phase4/*/{slug}.html'))
                 if p.parent.name != 'orbit-viewers']
    if p4_boards:
        rel = p4_boards[0].relative_to(REPO / 'docs').as_posix()
        phase4_crumb = f' · <a href="../{rel}" data-i18n="phase4_board"></a>'
    else:
        phase4_crumb = ''
...
const T = {{
  ...
  "methodology_index": "Methodology",
  "phase4_board": "Phase 4 board ↗",
  ...
  "methodology_index": "방법론",
  "phase4_board": "Phase 4 보드 ↗",
...
// 결정 표 필드 필터 (표가 있을 때만)
document.querySelectorAll('.dfin').forEach(box => {{
  const table = box.closest('.card').querySelector('.dt');
  const rows = [...table.querySelectorAll('tbody tr')];
  const count = box.parentElement.querySelector('.count');
  const run = () => {{
    ...
    count.textContent = q ? `${{shown}} / ${{rows.length}}` : `${{rows.length}}`;
  }};
