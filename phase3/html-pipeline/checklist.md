...
- [x] `scripts/pipeline/build_reports_index.py` scans
  `docs/phase2/*.html` and `docs/phase3/*.html`
- [x] Emit `docs/reports.html` — bilingual table (system, Phase 2 link,
  Phase 3 per-planet chips)
- [x] Also emits `docs/reports-manifest.json` — machine-readable, used
  by docs/index.html for the per-row chip integration (Stage 6)
- [x] Bilingual via the same `T = {ko, en}` pattern + localStorage

## Stage 6 — index.html DB integration ✓

- [x] Detail panel renders a "Curation reports" chip row at the top
  with Phase 2 + Phase 3-per-letter chips, sourced from
  `docs/reports-manifest.json` (loaded alongside data.json)
- [x] Chips link directly to the autogenerated reports
- [x] "Full index →" link to `reports.html` at row end
- [x] i18n via T.ko.reports + T.en.reports
- [x] Mobile: stacked layout (label on its own row, chips flow, index
  link right-aligned on its own row)
- [x] Test: TRAPPIST-1 row shows P2 + P3 d chips
