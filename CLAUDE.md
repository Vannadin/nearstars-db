...
**Once a direction is agreed, execute to completion without asking for step-by-step approval.**

- Don't pause mid-task to ask "should I proceed?" or "can I do X?" — if the direction is clear, just do it.
- New files, script runs, data transforms, calculations: proceed without confirmation.

**But judgment still applies — autonomy ≠ carelessness:**

- **Modifying existing files**: Read first. Use Edit (targeted diff), not Write (full overwrite). Know what you're keeping before you change anything.
- **Deleting existing content**: Always confirm. "I'll remove X — okay?" takes one line and prevents data loss.
- **Actions outside the agreed scope**: Stop and flag. Don't expand scope silently.

The distinction: skipping "should I proceed?" is good. Skipping "what am I about to overwrite?" is not.
