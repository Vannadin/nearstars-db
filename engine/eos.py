...
#: ⚠ **1348 K 를 상한으로 쓰는 것은 «공정선이 압력에 단조증가한다» 는 가정을 하나 쓴다** — 창 밖
#:   경계값을 창 안의 상한으로 읽는 것이기 때문이다. 원전들의 그림이 그 방향을 지지하지만 꺾인
#:   선이므로, **가정임을 여기 적어 둔다.** 대안(창 안 최고 1123 K)은 **하한 계열을 고르는 것**이라
#:   더 나쁘다 — 그것이야말로 «어느 쪽도 고르지 않는다» 를 깨는 선택이다.
#: ⚠ **판정이 바뀌는 띠는 (1348, 1473) K 다**: 그 사이의 핵 경계 온도는 예전에 «걸침» 이었고 이제
#:   «액체» 다. 오늘 로스터에는 그 띠의 천체가 없다 — Fe–S 를 선언한 천체가 **하나도 없고**(둘 다
