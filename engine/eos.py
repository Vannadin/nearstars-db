...
class ThermalSet:
    ...
    **부분 수리**이고, `Phase.thermal_label` 이 그 사실을 두 칸으로 돌려준다 (오너 검토 대기;
    ...
    def covers(self, p: float, t: float | None = None) -> bool:
        ...
        C58·180 B 가 이름 붙인 «온도 없이 물었을 때의 답» 이기 때문이고, `engine/test_fe_hcp.py@«「온도 모름」은 통과가 아니라 등급이다»` 가
...
class Phase:
    ...
    def delta_t(self, t: float, t_pot: float = 0.0) -> float:
        ...
        ⚠ **`t_pot ≤ 0` 은 «포텐셜 온도가 선언되지 않았다» 이고, 그때도 0 을 돌려준다** (C56).
        즉 «기준과 같아서 0» 과 «선언이 없어서 0» 이 **같은 수로 나온다** — 호출부가 그 둘을
        구별하려면 자기가 무엇을 넘겼는지 알아야 한다. 선언 없이 300 K 기준으로 열팽창을 얹는
        쪽을 고르지 않은 이유는 이 절 첫머리에 있다: PREM 적합을 그렇게 데우면 지구를 두 번 데운다."""
...
# 답**(C58·180 B 가 이름 붙인 그 경로, `engine/test_fe_hcp.py@«「온도 모름」은 통과가 아니라 등급이다»` 가 고정)으로만 남는다.
...
#     이고 우리에게 없다. 네 검산은 적합을 확인하지 앵커를 확인하지 않는다. ⚠ 그리고 그
#     앵커는 **하중이 걸리는 바로 그 압력에서, 우리가 가진 논문에 반박된다** (브리프 58):
#     Pommier+ 2018 (2018Icar..306..150P) 는 20 GPa · ~5 wt.% S 에서 Fei 의 예측 액상선
#     1900 °C 에 대해 1700 °C (±50) 를 재고, 액상선이 포물선(Fei)이 아니라 시그모이드
#     (Chen+ 2008a)라고 결론한다 — 오프셋이 아니라 모양 논쟁. Fei 를 얻어도 정착이 아니라
#     불일치 두 출처가 된다. 요청 목록에 넣지 말 것(2026-09-03 닫힘, 오너 몫).
...
# ⚠ **1348 K 은 Mori+ 2017 이 Fei+ 2000 을 읽은 값이고, 같은 논문을 Morard+ 2007 은 1380 K 으로
#   읽는다** (C55 (d), 2026-09-10). 두 읽기 다 2차라 등급이 같고, 우리가 고를 근거가 없으므로
#   **여기 있는 수는 안 움직인다** — 평균내는 것은 우리 산수다. Fei 2000 본문이 오면 그때 판정한다.
...
IRON_FES_GAP_REASON = ("10–21 GPa 의 Fe–S 융해는 Mori+ 2017(21 GPa 아래는 기준점 밖)도 "
                       ...
                       "Pommier+ 2018 (2018Icar..306..150P) 의 14·20 GPa, 1400–1850 °C 상평형 "
                       "실험 — 그러나 그 논문이 인쇄하는 공정 조성(20 GPa 15 wt.% S, 14 GPa "
                       "18 wt.% S)은 Fei+ 2000 등의 인용이지 자기 측정이 아니고(자기 시료는 "
...
#: 상한 계열 — Fei+ 2000 (2000AmMin..85.1830F) 21 GPa 1348 K, **2차 인용 등급**(전문 미보유).
...
#: ⚠ **점마다 «무엇인가» 를 든다** (브리프 185). 세 종류가 섞여 있었고, 그 섞임이 창의 천장을 공정이
#:   **아닌** 수로 세워 놓았다 — 그래서 등급이 아니라 **종류**를 필드로 든다.
#:     `eutectic` — 그 압력의 공정 온도로 인쇄된 값
#:     `ceiling`  — 그 온도에 **액체가 있었다**: 공정은 그 아래다 (상한 제약이지 측정이 아니다)
#:     `range`    — 실험 P–T 범위의 끝: 어느 압력의 공정도 아니다 (창에 **들어오지 못한다**)
EUT_KIND_EUTECTIC = "eutectic"
EUT_KIND_CEILING = "ceiling"
EUT_KIND_RANGE = "range"
IRON_FES_WINDOW_LOW_POINTS = (
    (15.0 * GPA, 1023.0, EUT_KIND_EUTECTIC, "Andrault+ 2009 본문 · Si 2 at% 장입"),
    (18.5 * GPA, 1073.0, EUT_KIND_EUTECTIC, "Andrault+ 2009 본문 · Si 없음"),
    (20.6 * GPA, 1123.0, EUT_KIND_EUTECTIC, "Andrault+ 2009 본문 · Si 2 at% 장입"),
)
#: ⚠ **25 GPa/1473 K 는 여기서 빠졌다** (브리프 185, Li+ 2001 전문 보유 뒤). 두 이유가 같은 방향을
#:   가리켰다 — (1) 1473 K 는 **어느 압력의 공정도 아니다**: Li Table 1 에서 8.5·10·14·25 GPa 의
#:   1473 K 런은 모두 **액체가 있고**, 부분 용융된 온도는 공정의 천장이다. 논문이 실제로 괄호 짓는
#:   25 GPa 공정은 **1373 < T_eut ≤ 1423 K** 다. (2) 그 점은 **창(10–21 GPa) 밖**인데 창의 천장을
#:   세우고 있었다 — 이 상수 자신의 주석이 «창 안 인쇄값의 최저·최고» 라고 적고 있었으므로, 둘째
#:   결함은 논문 없이도 보이는 것이었다.
IRON_FES_WINDOW_HIGH_POINTS = (
    (21.0 * GPA, 1348.0, EUT_KIND_EUTECTIC, "Fei+ 2000 · 2차 인용(전문 미보유)"),
)
#: Li+ 2001 (2001E&PSL.193..509L, **전문 보유** — 2026-09-11) Table 1 의 «액체 존재» 천장들.
#: 창 안 셋 중 20 GPa 하나가 정보를 준다: 공정을 1273 K **이하**로 눌러 21 GPa 의 225 K 불일치를
#: **위에서** 좁힌다 — 다만 천장이지 측정이 아니므로 그 불일치를 **풀지는 않는다**.
#: ⚠ **되돌림 런 표시(ᵃ)를 수와 함께 옮긴다** (감사 지적, 2026-09-11): 창 안에서 가장 정보가 많은
#:   점(20 GPa)이 «더 높은 온도로 가열한 뒤 냉각» 한 런이다. 논문은 정·역 실험의 조성이 다르지 않아
#:   평형에 근접했다고 적으므로 증거로는 서지만, 그 성질이 라벨에서 사라지면 안 된다.
IRON_FES_LIQUID_CEILINGS = (
    (10.0 * GPA, 1473.0, EUT_KIND_CEILING, "Li+ 2001 Table 1 LO77 — 액체 존재 · LO91 되돌림 런"),
    (14.0 * GPA, 1473.0, EUT_KIND_CEILING, "Li+ 2001 Table 1 LO92 — 액체 존재 · 되돌림 런"),
    (20.0 * GPA, 1273.0, EUT_KIND_CEILING, "Li+ 2001 Table 1 LO120 — 액체 존재 · 되돌림 런"),
)
#: 창 밖의 인쇄된 괄호 — 기록만 한다. 25 GPa 는 `iron_fes_eutectic_t_melt` 의 몫이고 이 튜플은
#: «논문이 여기서는 이렇게 좁혔다» 를 잃지 않기 위한 자리다.
IRON_FES_OUT_OF_WINDOW_BRACKETS = (
    (25.0 * GPA, 1373.0, 1423.0,
     "Li+ 2001 Table 1 — LO133(1223)·LO140(1373) 액체 없음, MO535(1423) 액체 있음. ⚠ 예전 상수가 "
     "쓰던 1473 K(LO95)는 **같은 표의 더 느슨한 천장**이다: 같은 25 GPa 에서 1423 K 에 이미 액체가 "
     "있으므로 그 압력의 천장은 1423 이고, 논문은 천장이 아니라 50 K 폭 괄호를 실제로 잰 것이다"),
)
#: ⚠ **그리고 창 안에는 «위쪽» 인쇄점이 하나도 없다** (감사 실측, 2026-09-11). 창은 `lo <= p < hi`
#:   이므로 **[10, 21)** 이고, 21 GPa 점은 그 배타적 상한 **위**다. 창 안 인쇄점은 하한 계열 셋뿐이며
#:   그 최고가 **1123 K** 다 — 즉 예전 상한 1473 K 는 창 안 최고보다 **350 K** 높았고, 괄호 폭이
#:   자료가 받쳐 주는 100 K(1023–1123)가 아니라 **450 K** 였다. 그 초과분이 전부 창 밖 두 점에서 왔다.
#: ⚠ **1423 K 를 상한으로 쓰는 것은 «공정선이 압력에 단조증가한다» 는 가정을 하나 쓴다** — 창 밖
#:   값을 창 안의 상한으로 읽는 것이기 때문이다. ⚠ **그리고 그 가정은 1348 을 쓸 때보다 멀리 간다**:
#:   21 GPa 는 창의 경계 바로 위(**4 GPa** 거리)지만 1423 은 **25 GPa** 의 값이라 [10, 21) 의 천장으로
#:   읽으려면 **8 GPa** 를 건너야 한다. 원전들의 그림이 방향은 지지하지만 선이 꺾이므로, **가정의
#:   크기까지 여기 적는다** — 작게 적으면 다음 사람이 그 거리를 모른다. 대안(창 안 최고 1123 K)은
#:   **하한 계열을 고르는 것**이라 더 나쁘다 — 그것이야말로 «어느 쪽도 고르지 않는다» 를 깨는 선택이다.
#: ⚠ **판정이 바뀌는 띠는 (1423, 1473] K, 폭 50 K 다**: 그 사이의 핵 경계 온도는 예전에 «걸침» 이었고
#:   이제 «액체» 다. 오늘 로스터에는 그 띠의 천체가 없다 — Fe–S 를 선언한 천체가 **하나도 없고**(둘 다
#:   `fe_prem`), 화성의 1909.95 / 2000 K 는 두 상한 모두 위여서 어느 후보에서도 `liquid` 다.
#: ⚠ **상단은 라벨 붙은 창 밖 점이다 — 25 GPa / 1423 K** (지휘석 결정 2026-09-11, 오너 검토 대기;
#:   되돌리기는 이 튜플 하나). 이유는 괄호가 하던 **두 일** 이다: 창 안 인쇄값을 감싸는 일과 **계보
#:   불일치를 싣는 일**. 상단을 창 안 최고(1123 K)로 좁히면 첫 일은 참이 되지만 둘째가 사라진다 —
#:   1348(21 GPa 계열)과 1123(창 안 20.6 GPa) 사이의 **약 225 K** 가 괄호 어디에도 안 남고, 그러면
#:   그 불일치에 **새 집**이 필요해진다. 좁히는 것은 «고르는 것» 은 아니지만 «보여 주기를 그치는 것»
#:   이다 (감사 지적, 2026-09-11).
#: ⚠ **1423 K 가 무엇인지 라벨이 말한다**: 창 **밖**(25 GPa)의 값이고, 실험 범위 상단(1473)이 아니라
#:   **Table 1 자신의 50 K 괄호(1373 < T_eut ≤ 1423)의 위끝**이다 — 즉 인쇄된 것 중 **더 좁은** 쪽이다.
#:   그것을 10–21 GPa 의 상한으로 읽는 것은 «이 구간에서 공정선이 25 GPa 값 아래에 있다» 는 가정
#:   하나를 쓰고, Li 자신의 자료(20 GPa ≤ 1273 → 25 GPa 1373–1423)가 그 방향을 지지한다. **가정임을
#:   여기 적어 둔다** — Fei 1997 의 1 bar–14 GPa 구간은 반대로 **내려가고**(988 → 860 °C = 1133 K),
#:   그 꺾임이 바로 이 창의 자료가 갈리는 이유다. **기울기 부호가 14–20 GPa 사이에서 바뀌고**, 그
#:   자리가 Fe₃S₂·Fe₃S 가 안정해지는 압력이다.
#: ⚠ **왜 더 좁히지 않았는가 — 1423 은 «1348 을 아직 담는, 가장 좁은 1차 등급 천장» 이다.**
#:   후보를 정렬하면 창 안 최고 1123 · Li 창 안 천장 1273 · **21 GPa 계열 1348(2차 인용)** ·
#:   Li 25 GPa **1423**(1차, 같은 표에 하한 1373) · 실험 범위 상단 1473 이다. 괄호의 둘째 일은
#:   상한이 **1348 위**일 때만 유지되므로 1123·1273 은 높은 계보를 **조용히 기각**하게 된다. 1348
#:   위의 1차 등급 인쇄값은 1423 하나뿐이고(1473 은 공정이 아니다), 그래서 창 안 공정선이 대략
#:   1133–1273 인데도 1423 을 둔다 — **그 넉넉함은 1348 을 담기 위한 것**이다 (감사석 정리,
#:   2026-09-11). 취향이 아니라 수에서 나오는 이유다.
IRON_FES_BRACKET_K = (1023.0, 1423.0)      # 하단 = 창 안 공정 최저 · 상단 = 라벨 붙은 창 밖 천장
IRON_FES_CEILING_PRIMARY_K = 1273.0        # Li+ 2001 20 GPa 천장 — 창 **안** 1차 등급, 함께 인쇄한다
IRON_FES_UPPER_ALTERNATIVE_K = 1348.0      # 21 GPa 계열(2차 인용) — 채택 안 한 후보, 기록으로 든다
#: ⚠ **7 GPa 의 «불일치» 는 없다 — 조건이 다른 두 수를 맞댄 것이었다** (감사 대조 2026-09-11, 이
#:   좌석이 원문에서 재확인). Fei+ 1997 (1997Sci...275.1621F, 보유, sha256 `cb3ea81deb87a6e0…`) 은
#:   p.1621 에서 공정선을 **선형**으로 인쇄한다 — 축자: «The eutectic T linearly decreased with
#:   increasing P, from **988°C at 1 bar** to **860°C at 14 GPa**». 즉 1261 K(988 °C)는 **1 bar 값**
#:   이고 7 GPa 의 값이 아니다. 그 선형선을 7 GPa 로 읽으면 924 °C = **1197 K**(논문이 «선형» 이라고
#:   적었으므로 보간은 논문이 허락한 산수, 그래도 **우리 환산**이다)이고, Li 의 7 GPa 천장
#:   **≤ 1223 K** 보다 26 K 아래다 — **두 논문은 어긋나지 않고, 천장이 제 일을 했다.**
#:   ⚠ **그래서 «Fei 와의 불일치» 는 없다** — 넘어진 자리는 하나였다: **수를 옮길 때 조건(압력)이
#:   함께 오지 않았다.**
#: ⚠ **그러나 7 GPa 의 «약 40 K 불일치» 자체는 있다 — 상대가 Fei 가 아니라 Buono & Walker 2015 다**
#:   (감사 발견, 2026-09-11). 그 논문(2015M&PS...50..547B, 초록만 보유, 결과 등급)은 «The Fe-FeS
#:   system maintains a eutectic temperature of **990 ± 10 °C to at least 8 GPa** if starting materials
#:   and pressure media are rigorously dehydrated. **Literature reports of pressure-induced freezing
#:   point depression … are not confirmed.**» 라고 인쇄한다 — 즉 7 GPa 에서 **1263 ± 10 K** 이고,
#:   Li 의 천장 **≤ 1223 K** 와 어긋난다. ⚠ **그리고 그 문장은 Fei 의 감소선 자체를 부정한다** —
#:   위 «기울기 부호가 바뀐다» 는 서술의 한쪽(내려감)에 대한 **1차 반대 출처**가 보유 집합에 있다는
#:   뜻이고, 조건절(«엄격히 탈수했다면»)이 아래 수소 오염 읽기와 같은 자리다. 판정은 안 바뀐다
#:   (7 GPa 는 창 밖, 화성은 어느 후보에서도 액체) — 바뀌는 것은 «왜 곡선이 아니라 괄호인가» 의
#:   근거가 한 겹 두꺼워지는 것이다.
#:   ⚠ *그 불일치를 담을 상수(`IRON_FES_7GPA_DISAGREEMENT_K`)를 185 작업 중에 **만들었다가 대조에서
#:   걷어냈다** — 출하된 적이 없으므로 움직인 기준선은 없다. «지웠다» 가 아니라 «태어나지 못했다» 가
#:   맞고, 그 구분을 적어 두는 이유는 다음 사람이 이력에서 사라진 상수를 찾지 않게 하기 위해서다.*
#: ⚠ 그리고 **Fei 의 선형선은 14 GPa 까지다** — 같은 논문이 Fe₃S₂ 생성으로 «단순 이원 공정계 →
#:   중간 화합물이 이질적으로 녹는 계» 로 바뀐다고 적으므로 20 GPa 로 외삽할 수 없다. 창의 상단
#:   절반에서 Fei 는 비교 상대가 못 되고, 그것 자체가 «곡선 대신 괄호» 를 받치는 근거다.
IRON_FES_FEI_LINE_C = ((0.0, 988.0), (14.0 * GPA, 860.0))   # 인쇄된 두 끝점 [°C] — 그 사이는 선형
...
def iron_fes_phase_verdict(t_k: float, p: float) -> str:
    ...
    if band is None:
        ...
        if single is None:
            # ⚠ **거절이 인쇄된 것을 들고 나간다** (브리프 185). 창 밖에 «논문이 여기서는 이렇게
            #   좁혔다» 가 있으면 그 수를 문장에 싣는다 — 안 그러면 그 상수가 «저장만» 이 되고,
            #   거절을 읽는 사람은 자료가 아예 없다고 읽는다. **판정은 바뀌지 않는다.**
            # 라벨 전문은 상수에 있다 — 거절문에는 **수와 자리**만 싣는다. 문장이 길면 다음 사람이
            # 거절을 안 읽는다.
            near = [f"{q / GPA:.0f} GPa 에 인쇄된 괄호 {lo_k:.0f}–{hi_k:.0f} K"
                    for q, lo_k, hi_k, _why in IRON_FES_OUT_OF_WINDOW_BRACKETS]
            extra = (" — 다만 " + " · ".join(near)) if near else ""
            fei_lo, fei_hi = IRON_FES_FEI_LINE_C
            # ⚠ 아래 끝은 **1 bar** 다 — 상수는 0 Pa 로 들지만 논문이 인쇄한 조건은 1 bar 이고,
            #   «0 bar» 로 찍으면 없는 조건을 말하게 된다.
            return ("cannot-say (창 밖이고 곡선도 없다"
                    f"{extra}. Fei+ 1997 의 선형 공정선은 1 bar {fei_lo[1]:.0f} °C → "
                    f"{fei_hi[0] / GPA:.0f} GPa {fei_hi[1]:.0f} °C 로 **14 GPa 까지**이고 "
                    "그 위는 계가 바뀌어 외삽할 수 없다)")
...
H2O = Material(
    ...
    t_over_reason=("얼음 기둥의 온도 {t_k:.0f} K 가 '{phase}' 적합의 상한"
                   "({t_max:.0f} K) 위다. 그 상한은 SeaFreeze v1.1.0 이 싣는 "
                   "French & Redmer 2015 표현의 온도축 매듭 구간이 끝나는 자리이지 "
                   "상 경계가 아니다 — 출처 자신의 격자는 2000 K 까지 간다. 그 위 "
                   "상태(초이온상)의 상태방정식은 French & Redmer 2016 "
                   "(2016PhRvE..93b2140F) 이 두 초이온상의 열역학 퍼텐셜로 구성해 "
                   "두었고, 이 레시피에 그것이 없다."),
