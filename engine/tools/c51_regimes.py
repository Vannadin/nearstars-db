...
EXPECTED = {
    # ⚠ 2026-09-24 re-freeze — radiogenic landing (mantle heat ×1.020 on the Earth default set) and C20 inputs
    #   moved with core_history. Old: (12.325, 6.09, +65.1, 2.450) · (7.104, 3.51, +84.2, 4.251) ·
    #   (8.923, 1.04, +49.4, 1.742) · (8.923, 0.94, +67.1, 1.928) · (1.784, 0.21, +102.7, 8.713) · (1.784, 0.19, +125.0, 9.641).
    ("Earth", "declared", 100.0): (12.325, 6.09, +67.3, 2.499),
    ("Earth", "C20", 100.0): (7.230, 3.57, +85.9, 4.260),
    ("Mars", "declared", 350.0): (8.923, 1.04, +51.7, 1.777),
    ("Mars", "declared", 500.0): (8.923, 0.94, +69.9, 1.966),
    ("Mars", "C20", 350.0): (1.824, 0.21, +104.7, 8.692),
    ("Mars", "C20", 500.0): (1.824, 0.19, +127.5, 9.618),
}

# ── the two bodies, from the interior solve rather than typed in ────────────
BODIES = {
    "Earth": {"mass_earth": 1.0, "cmf": 0.325, "radius_earth": 1.0,
              "t_p_declared": 1600.0, "t_p_c20": 1519.86, "delta_m": (100.0e3, 100.0e3)},
    "Mars": {"mass_earth": 0.1074, "cmf": 0.24, "radius_earth": 0.5320,
             # ⚠ 1380.65 는 `core_history` 기본 경로의 오늘 값이다 — 걸음 상한 4.0 Myr ·
             ...
             # ⚠ 2026-09-24: core_history 의 옛 오늘 값 복사라 방사성 착지와 함께 따라 옮김 —
             # 지구 1517.34 → 1519.86 · 화성 1377.9563 → 1380.65.
             "t_p_declared": 1600.0, "t_p_c20": 1380.65, "delta_m": (350.0e3, 500.0e3)},
}
