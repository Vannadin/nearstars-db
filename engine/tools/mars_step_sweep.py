...
def build(body: dict, t_pot: float, core_init: float) -> tuple[dict, float, float]:
    ...
    #   **안 맞고**, 틀린 값 `0.24` 와 `0.325` 는 **정확히 맞는다**.
    #   ⚠ **기전을 한 번 틀리게 적었다 — 「손실은 `1.0 - cmf` 에서 난다」는 틀렸다.** 두 질량의
    #   합은 **정확하고**(`m_core + m_mantle == m_kg`), 그래서 뺄셈 오차는 분모에서 **상쇄된다**
    #   (`m_core / (m_core + m_mantle) == m_core / m_kg`). 뺄셈만의 값 `0.23958333333333326` 은
    #   인쇄되는 어느 수에도 안 나타난다. 남는 잔차는 **곱했다 다시 나눈 왕복** 하나뿐이고,
    #   대수로 고쳐 쓸 수 없다.
