...
# 이름이 "핵이 없다"가 아니라 **이 노드의 물음이 성립하지 않는다**여야 한다. 옛 이름 CORELESS_CLASSES 는
# 바로 아래 자기 이유문과 정면으로 어긋나 있었다 — 서브넵튠은 철핵이 있다고 그 문장이 말한다 (C23, 2026-09-06).
NOT_THIS_NODES_QUESTION = ("giant", "gas_giant", "ice_giant", "sub_neptune",
                          "brown_dwarf", "star")
...
_NOT_ASKED_WHY = {
    ...
    "sub_neptune": "서브넵튠의 철핵은 가스 외피 아래 앉아 있고, 그 핵의 다이나모는 이제 "
                   "sub_neptune_dynamo 가 판정한다 (C23, 2026-09-06). 다만 그 판정의 첫 "
                   "게이트는 핵이 아니라 **맨틀 표면이 녹아 있는가**이고(Tang+ 2025), 그것은 "
                   "규산염 솔리더스가 답한다 — 철의 융해곡선이 아니다. 그래서 이 노드의 "
                   "물음은 여전히 이 클래스에 대해 성립하지 않는다. 예전 이유는 '받는 갈래가 "
                   "없다'였고, 그 고리는 끊겼다.",
...
def solve(core_pressure: float,
    ...
    if body_class in NOT_THIS_NODES_QUESTION:
        return out_of_domain(
            RECIPE, VERSION,
            f"'{body_class}' 에는 이 판정이 뜻이 없다. 이 노드가 묻는 것은 **금속 핵이 "
            f"액체인가** 이고, {_NOT_ASKED_WHY[body_class]}",
