...
def main() -> int:
    ...
    # ⚠ **도달성 가드이지 기준선이 아니다** (170 F ④, 자리 옮김 171 D). 이 파일의 사고 방식은
    #   «단정이 판정 뒤로 밀려 조용히 안 돌게 되는 것» 이었다 — 그러면 어떤 단정도 실패하지 않으므로
    #   초록이 늘 나온다. 그래서 **여기까지 실제로 실행된 `ok()` 의 수**를 센다. 블록 하나가 도달
    #   불가가 되면 수가 줄어 이 줄이 스스로 FAIL 한다. 단정을 **더할 때** 이 수를 함께 올린다.
