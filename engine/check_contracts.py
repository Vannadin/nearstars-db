...
#: ⚠ **(0, 0, 0) → (1, 1, 1), C62 (b) 2026-09-11 — 그리고 이번엔 수리가 아니라 등록된 귀결이다.**
#: 새 노드 `tidal_response` 의 `Needs` 에 `tidal_response` 블록이 있고, **오늘 로스터에서 그것을
#: 선언하는 천체가 없다**. 그게 결정 ① 이 등록한 바로 그 상태다 — 선언한 천체에서만 돌고, 오늘
#: 도는 로스터 천체는 0 이다. ⚠ *C62 (b) 의 J7 은 «클래스 ③/④ 기준선이 안 움직인다» 를 등록했고,
#: 그 절반이 틀렸다*: ④ 는 그대로지만 ③ 은 1 이 된다. 등록이 틀린 것이지 코드가 틀린 것이 아니고,
#: **두 등록이 서로 모순이었다** — 「아무도 선언 안 한다」와 「Needs 가 어느 표본에서든 공급된다」는
#: 같이 참일 수 없다. 첫 천체가 블록을 선언하는 날 이 칸은 저절로 빈다.
CLASS3_BASELINE = (1, 1, 1)           # (노드, 고유 키, (노드,키) 쌍)
...
#: ⚠ **행 자격을 못 얻고 사라지는 것은 이 두 수가 세지 않는다.** 후보 (노드, 키) 쌍은 **95** 이고
#: 그중 공급·미선언이 갈리는 것이 23 이다 (2026-09-10 측정) — 나머지 72 는 «전부 공급» 이거나
#: «전부 미선언» 이라 이 기록이 침묵한다. 후자는 클래스 ③ 이 보고, 전자는 아무도 안 본다.
...
def main() -> int:
    ...
    for node in sorted(registry.registered()):
        ...
        #    ⚠ **이 예외는 지금 노드 경로에서 도달 불가다** (C57): 사슬은 역산 솔버를 부르지 않고
        #    `test_interior`·`rocky_roster` 만 부른다. 설계는 남긴다 — 규약은 실재하고, 사슬이 언젠가
        #    그 분기를 갖게 되면 이 줄이 이미 맞는 자리에 있다. 다만 «걸린 적 있다» 는 말은 못 한다.
