...
"""Tidal heating from the fixed-Q law (`tidal-heating-methodology.md`), plus the two label sets §6 attaches to it.
    ...
    Ė = (21/2) · (k₂/Q) · G M_p² R⁵ n e² / a⁶,   n = √(G M_p / a³)   (doc :56, :58; m ≪ M_p)
    F = Ė / (4πR²)                                                 (doc :81–82)
...
- `tidal_heating` — `power` [W], `surface_flux` [W/m²], `io_power_ratio` [—], `heat_transport_regime` (the §6.1 outcome
  table, doc :246–251, strings verbatim; the 10⁻³–10⁻² decade has no row → "unclassified (between table rows)").
- `heat_transport_mode` — `mode` (the §6.2 table, doc :277–281: plate tectonics · stagnant lid · heat pipe), judged on the
...
Both tables are "guides, not sharp lines" (doc :244) and "there is no published W/m² boundary between the modes" (doc
:436–438); the labels carry that. Not emitted in this version: `radius_ceiling` and `plains_temperature` (the §6.3–6.5
Dante axis — a separate item), and `tidal_transport.derive_potential_temperature` is not consulted (its own docstring
labels it validation-failed, and Pandora is not a §6 lid-bearing case, doc :330–338).
...
VERSION = "1"
REFS = ("docs/reference/tidal-heating-methodology.md:56", ":81-82", ":246-251", ":277-281")

G = 6.67430e-11            # m³ kg⁻¹ s⁻², CODATA 2018
M_EARTH_KG = 5.9722e24     # IAU 2015 B3 nominal
R_EARTH_M = 6.371e6
# Io's tidal output as the document prints it: "~10¹⁴ W" (doc :105, :109, :481). The Dante rows of §6.5 (doc :449–454)
...
# 2.5 W/m² = Kankanamge & Moore 2019 (2019JGRE..124..114K, HELD; doc :281, :620–623). Neither is used in a verdict here.
IO_FLUX_VEEDER_W_M2 = 2.0
IO_FLUX_KM2019_W_M2 = 2.5

# §6.1 outcome table, doc :246–251 — strings verbatim
...
# §6.2 transport-mode table, doc :277–281
...
GUIDES = ("doc :244 'these are guides, not sharp lines'; doc :436–438 'there is no published W/m² boundary between the modes' "
...
def tidal_power(k2_over_q: float, perturber_kg: float, radius_m: float, a_m: float, e: float) -> tuple[float, float, float]:
    """(Ė [W], F [W/m²], n [rad/s]) — doc :56, :58, :81–82."""
...
def outcome_regime(flux_w_m2: float) -> str:
    """§6.1 table, doc :246–251. The decade 10⁻³–10⁻² has no row."""
    if flux_w_m2 >= 1.0:
...
def transport_mode(total_flux_w_m2: float) -> str:
    """§6.2 table, doc :277–281, read on the total surface flux. Plate tectonics sits at ~0.09 W/m² (Earth 92.1 mW/m²);
...
def solve(mass_earth: float, radius_earth: float | None, semi_major_axis_m: float | None,
    ...
    if missing:
        return out_of_domain(RECIPE, VERSION, f"cannot-say (no orbit): {', '.join(missing)} 미선언 — Ė ∝ M_p² e²/a⁶ (doc :56)",
                             inputs, REFS)
    if k2_over_q is None:
        return out_of_domain(RECIPE, VERSION, "cannot-say (no k2_over_q): 조석 소산 k₂/Q 미선언 — 클래스 밴드는 2–3 자리수 폭이라 "
                             "(doc :194–202) 값을 고르지 않는다", inputs, REFS)
    if radius_earth is None:
        return out_of_domain(RECIPE, VERSION, "cannot-say (no radius): Ė ∝ R⁵, F ∝ R³ (doc :56, :440)", inputs, REFS)
    ...
    notes = (
        f"fixed-Q law doc :56 with n = √(G M_p/a³) (m ≪ M_p, doc :58): k₂/Q {k2_over_q} · e {eccentricity_forced} · "
        ...
        f"io_power_ratio = Ė / {IO_POWER_W:.1e} W, the document's printed '~10¹⁴ W' (doc :105, :109, :481). The §6.5 Dante "
        "rows (doc :449–454) invert to 1.016e14 W (2.44 W/m² at R 1822 km), 1.6 % from this constant — a convention difference.",
        f"heat_transport_regime is the §6.1 outcome table (doc :246–251) read on the TIDAL flux alone; {GUIDES}.",
        ...
        "§6 lid-bearing case, doc :330–338).",
    )
...
def solve_mode(surface_flux: float | None, radiogenic_power: float | None, radius_earth: float | None) -> Result:
    ...
    refs = ("docs/reference/tidal-heating-methodology.md:273-281",)
    if radius_earth is None:
    ...
    notes = (
        f"§6.2 table (doc :277–281) read on the TOTAL surface flux {total:.4g} W/m² = {' + '.join(parts)} W/m²; "
        ...
        "doc :330–338 names Pandora a global-fluid-layer case, so its selector is the next item.",
    )
