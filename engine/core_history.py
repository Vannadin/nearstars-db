...
def rates(t_c: float, t_m: float, p: dict, t_gyr_from_present: float) -> dict:
    ...
    if p.get("loss_law") == "foley":
        ...
        delta = p["delta_m"]
        r_lid = p["r_p"] - delta
        # ⚠ **표면온도를 넘기지만 eq. (3) 은 그 값에 안 움직인다 — 측정된 사실이다** (2026-09-20).
        # `t_s` 만 216 ↔ 293 K 로 바꾸면 `f_man_w_m2` 가 **열여섯 자리까지 같은 수**를 낸다
        # (T_p·d·g 를 바꿔도 같다). 앞의 `(T_p − T_s)` 가 `θ^(−4/3) Ra_i^(1/3)` 안의 ΔT 와
        # **상쇄**되기 때문이다. 그러니 「ΔT 가 두 번 들어가 크게 움직인다」는 읽기는 **틀렸다**.
        # ⚠ **그래도 넘긴다.** 상쇄는 **이 적합의 성질**이지 법칙의 약속이 아니고, 넘기지 않으면
        # 다음 사람이 `d_m`·`g` 만 바디 것이고 `T_s` 만 모듈 기본값인 **비대칭**을 다시 만난다.
        # 값에 안 움직인다는 것과 인수를 안 준다는 것은 다른 말이다. Nimmo 쪽은 실제로 움직인다
        # (같은 두 값에서 `q_m` 3.609e12 ↔ 3.300e12 W).
