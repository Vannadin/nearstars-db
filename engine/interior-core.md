...
⚠ **Committed before the code.** The text below is P35 (parallel seat, 2026-09-11) moved into the ledger,
sha256 `bb67e2ced144f8eb…`, **29375 B**, still living at
...
moving it breaks both at once* (directing seat, 2026-09-11).

⚠ **Three edits were made in the move, and they are all the same edit.** The draft cited three code
sites by line number, and this tree forbids that form, so each became a phrase anchor at the same site (`COMPOSITIONS: dict[str, tuple[float, float,
float, str]] = {`, the `if stack[layer][1].name == "h2o":` branch that assigns `p_ice_base`, and
`def check(g: dict) -> int:`). *Nothing else was changed.* The headings are one level deeper than in the
draft, to nest under this section. The three forms as the draft wrote them on **2026-09-11**, so the move
is reversible:

> note (2026-09-11): `interior.py:82` · `interior.py:734` · `chain.py:52`


⚠ **Twelve amendments, and the useful fact is who found what.** The draft went from its first form to
this one through twelve corrections in about ninety minutes — the audit seat against the PDFs (Wahr's
line pointers and the Table 4 trap, the Moon's Q lineage, Io's missing number), the audit seat against
the engine (there is no ρ(r) on a radius grid to integrate; `Structure` has no `r_ice_base`; the preset
label needed a node prefix), and this seat against the engine (the mantle map double-covered ocean
bodies; `cmf`/`imf` live in `Result.inputs` where `state.get` cannot see them; `cmf` has a preset
precedent inside `solve` itself). ⚠ **Nine of the twelve are «the draft asked the engine for something
it does not have», and none of them would have survived first contact with the code** — which is what a
pre-registration is for. *The cost of finding them here was messages; the cost of finding them after the
build is a commit, a gate and a re-measurement.*
...
| # | decision |
|---|---|
| scope | **emitter only** — the node emits k₂, h₂, l₂, Q; it does **not** replace any declared `k2_over_q` (Pandora's `k2_over_q: 0.0016`, board-fitted) and the C39 seam `k2q_class_table → tidal_heating via k2_over_q` (chain.yaml :555) stays as it is |
| (a) liquid layer | **membrane/fluid limit + label** — Beuthe 2015 eq. (27) `k°_n + 1 = h°_n` (layout p. line 525) with μ → 0 in the fluid layer; every output carries «fluid layer in the membrane limit (Beuthe 2015 eq. 27)». Saito 1974's fluid-layer propagation (P24 §4, held, equations image-only) is the **registered successor candidate**, not built here |
...
| where | today | moves? |
|---|---|---|
| `engine/chain.yaml` :227 `tidal_response` | `status: missing`, `outputs: [k2_over_q, layer_q]`, note «k2q_class_table이 이 자리를 대신» | status → built; outputs become `[k2, h2, l2, q, k2_over_q_emitted, layer_q]` — ⚠ the emitted `k2_over_q_emitted` is a **new name**, so the existing edge :555 (`k2q_class_table → tidal_heating via k2_over_q`) is untouched by construction |
| `engine/tidal_heating.py` :340 (`if k2_over_q is None: cannot-say`) | reads the **declared** `k2_over_q` only | **untouched** — non-consumer of the new node in this brief |
...
| fluid-layer limit | Beuthe 2015 eq. **(27)** `k°_n + 1 = h°_n` (layout line 525), Appendix C membrane cases (h₂ 1.27 Model L / 1.35 Models M, D — P28 T1) | decision (a) label and anchor T1 |
...
| J5 | Beuthe membrane cases | h₂ 1.27 / 1.35 (P28 T1) — ⚠ these are values of the **membrane approximation eq. C.3**, so reproducing them verifies our membrane formula, **not the propagator**; the propagator's nets are J1 and J3 | held (result) | reproduce to the printed 2 decimals (membrane formula only) |
...
| E | whether Saito 1974 fluid-layer propagation becomes the next C-item after this build | yes (needs equation transcription from page images); defer |
...
4. ⚠ **No fluid-layer treatment beyond the membrane limit** without its own registration (Saito route = decision E).
