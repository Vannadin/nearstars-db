...
| **C18** | `body_class → dynamo_rocky via sub_neptune` (`:623`) | a sub-Neptune integrates (C1) and has no dynamo path. **Closed 2026-09-04 as a named refusal** — the three refusals already stand at their places and are the closure: `core_state` (*"서브넵튠의 철핵은 가스 외피 아래 앉아 있지만 그 핵을 받는 다이나모 갈래가 아직 없다 — dynamo_giant 는 질량으로 배제하고 dynamo_rocky 는 이 클래스를 받지 않는다"*), `dynamo_giant` (below `GIANT_M_MIN`: Reiners & Christensen *"explicitly excluded"* sub-Saturns for helium-rain stratification, and the surface-field attenuation is *"hard to quantify"*), `dynamo_rocky` (`sub_neptune` is not in `ROCKY_CLASSES`). **What is missing, named**: a published scaling for a dynamo in an iron core sitting under an H/He envelope — no held paper prints one (Reiners & Christensen stop at Saturn, OC06/RM22 are rocky, Gaidos+ 2010 is rocky). **Reopens if** such a source is held, or the owner elects to run the rocky ladder on a sub-Neptune's core under a declared label (a Phase 4 choice). The edge stays `status: gap` — a real coupling with no supplier — and its note says closed. |
