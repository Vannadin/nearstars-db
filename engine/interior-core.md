...
⚠ **And the mixture's adiabatic gradient is large: ∇_ad ≈ 0.33** against `fe_prem`'s 0.055. It is not a
mistake — it is Huang's own derivative doing its work: the light elements soften the alloy so much that
K_T falls from **156 GPa** (pure Fe) to **93.8** (13/1/0.5) and **49.1** (19/4/0.5), and ∇_ad ≈ γP/K_S rises
accordingly. ⚠ *The derived γ is 1.97 for the mixture where the paper prints 2.74 for pure Fe* — because
α and C_V carry no printed composition derivative and are held at the anchor's values while ρ and K_T move.
**That zero derivative is this engine's assumption and the material's label says so.**
...
**No core mass fraction puts the radius inside 3 % and `nmoi` inside 1 % at once.** Where these materials
solve at all, the radius is never better than **8.6 %** off. The moment of inertia is easy — 0.11 % at the
right cmf — and the radius is not, and the range where the radius could improve is above the cut.
