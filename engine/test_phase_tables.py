...
# 합법이다 — 값을 지어 채우지 않는다.
# ⚠ **27 → 29, P33 B (2026-09-13).** 저압 창이 열리면서 두 이성분에 상이 하나씩 더 붙었다 —
#   `fe_s_balog_w0.1300` 과 `fe_s_balog_w0.1900`, 19 GPa **아래**를 답하는 모형 상이다. 이 수가
#   또 제 일을 했다: 게이트가 아니라 이 시험이 먼저 멈추고 **두 이름을 찍었다**. 그 두 상의 표
#   칸은 비어 있고, 그것은 «발표된 값이 없다» 로 합법이다 — 값을 지어 채우지 않는다.
# ⚠ **19 → 27, 브리프 183 (2026-09-10).** 오너 상자의 여덟 모서리(S 13·19 × O 1·4 × C 0.5·1.4 wt%)가
#   재질로 등재되면서 상이 여덟 늘었다. 이번에도 이 수가 제 일을 했다 — 게이트가 멈추고 새 이름
#   여덟을 그대로 찍었다. 여덟의 표 칸은 비어 있고 그것이 합법이다: 다원계 액체 핵의 전도도·조석
#   상수는 인쇄된 것이 없다. **값을 지어 채우지 않는다.**
row(len(emitted) == 29, f"방출 상 {len(emitted)}개: {' · '.join(pt.PHASE_KEYS)}")
