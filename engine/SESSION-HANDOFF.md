...
*Written by the parallel seat for the work seat to fold into `engine/SESSION-HANDOFF.md` with 198. The previous block («written 2026-09-12 ~09:00») becomes «superseded by the block above». ⚠ One placeholder: **198's sha**. ⚠ This draft is **frozen at hand-off** — later edits go to the folding seat first, with the new hash. Measurements are attributed where they were made: the 197 timings and the proof runs are the work seat's, the `@34/@43` and `15 / 35` figures are tool output reproduced by the audit, the closure count and the C84 phrase counts are the parallel seat's.*
