...
- **Gate**: `scripts/check.sh --from <sha> --targeted` per brief (169 G mapped `*_anchor.json` and `tools/*.py`, so the next targeted run should be real); full only on the push tip; never two at once; scratches are the gate's to delete.
