...
## Resume pointer — latest (written 2026-09-12 ~23:00, parallel seat, covering since the 09:00 block)

- **Seats** c3 directing · 21 work · 70 parallel · 45 audit. Brief counter at **198**; ledger: **C80 closed**, **C82 · C83 · C84 · C85** candidates.
- **Remote `engine/prototype`** = **`b4063a01`** (verified with `git ls-remote`: `b4063a013439…`), **6 commits on the branch today** — `3716708f` · `1554fdf7` · `fe26aa7d` · `40989ff8` · `940335bb` · `b4063a01`. ⚠ Re-verify before citing; count a day with **explicit times** (`--since=…T00:00:00 --until=…T00:00:00`), because a bare date resolves to *that date at the current time* and silently drops the day.
- **Seven owner decisions landed, plus M1.** **M1** (09:00) — «오차가 충분히 작다면 속도를 신경쓰는 게 맞다»: secant inverter, and «충분히 작다» = **shipped precision moves not one digit** (`run.py`'s emitted values and what phase4/boards read). ⚠ The anchor `.values.txt` prints full `repr` and is **not** the threshold — it becomes a new baseline kept beside the old. Then (09:5x, relayed): **M2 = (a)** widen the ρ bracket with an **extrapolation label**; **③ = (c)** split the name, both nodes emit; **① = (가)** keep today's state; **④** as built; **⑤** keep the shooting budget; **⑥-2 = (2)** exclude the 11 author-flagged species by default, declaration includes; **⑦** fill low-pressure Fe–S at **model grade** — P33 becomes a brief.
- **197 landed — `b4063a01`.** Secant inverter, tolerance 1e-10; `free_energy` **833 250 → 220 506** (−73.5 %). **Ratios by measure, same condition (both system python, same day, sequential):** CPU **2.17×** (3035.75 → 1397.02 s), instructions **2.34×** (84.23e12 → 36.08e12), **gate-to-gate 2.23×** (3029 s at pool 2 → **1360 s at pool 8**). ⚠ **The standalone wall of 1914.27 s is not the basis of any ratio** — it carried contention (CPU 1397 s, **517 s off-CPU**), and the same test ran **faster inside the gate** (1360 s), which is what exposed it. ⚠ **The registered aim ≤ 1039.7 s is missed on every measure** — standalone wall 1.84×, gate 1.31×, CPU 1.34× — **recorded as missed**.
- **Values: the shown limit and the measured band, printed together.** M1 was decided on «≤ 1e-9»; the measured worst over **all 8 009 requested states** is **1.382e-9**, split **in-grid (≥ 295 K) 1.018e-9 · below-grid 1.382e-9**. ⚠ **The shown limit is exceeded inside the fit grid too**, by **1.8 %** — not only outside it. The owner's own threshold (**shipped digits move 0**) passed: controls 12 rows + 3 anchors, **printed-digit moves 0 · repr moves 0 · applicable flips 0**. ⚠ **Because the number changed, the owner may re-open M1.**
- **The strongest single line on values**: the test's printed output is **301 lines byte-identical** (`fd826ed6`) across the old and new trees, and that is the **same hash 190 B registered** at `f681afbd ↔ 587842b3` — **from before 190 through 197, this test's printed output has not moved one byte.**
- ⚠ **The earlier HOLD is withdrawn.** The numbers that stopped 197 overnight were measured from **non-registered inputs** — a test that reads hand-entered values measures something other than the registered body, and a stop built on that has no basis.
- **C85 — candidate, distance only.** A non-roster body sits on the shooter's knife-edge: a **1e-9** perturbation flips convergence (amplification ≈10⁶; in-tree precedent, the reverted 27 % warm-start). ⚠ Not «the value is wrong» — «near there, *converged* hangs on a very small number». **Work starts only if a roster body actually reaches that threshold**; until then the only task is measuring how far ours sit from it.
- **198 is written and frozen at `f7afc85886908ac6`** (89 lines) — `ThermalSet` gains `t_min` (default unbounded), `covers_t` tests both edges, below-floor **grades** as above-ceiling does. ⚠ **The early return is the trap**: `if self.t_max <= 0.0: return True` never looks at `t`, so a floor added only to the last line leaves a set with `t_min` and no `t_max` unbounded — **the liquid-Fe pair**, the likeliest floor customers. Named test: «a set with `t_min` and no `t_max` grades a T below the floor». Declaring is **198 B**, separate. **This commit.**
- **Queue after 198**: **198 B** (declare the floors) → **190 C** (M2 (a), the labelled extrapolation) → **C68 B** (③ (c), split the name) → **P33 as a brief** (⑦, model-grade Fe–S) → **196**. ⚠ 196's first line stands: Huang 2018's compositions (11.8 · 10 wt % S) are not ours (13 · 19 wt %), and 13 is **outside** the printed pair.
- **Artifacts, all current on the work account**: the daily board (`5330923e`), the core-items page (**84 rows**, C1–C85 pending the next regeneration), the chain explorer (**51 nodes · 210 edges**). ⚠ **Their generators of record are now `board/core_items_0912.py` and `chain_overlay_0912.py`**, both of which **refuse without an explicit sha** — the originals read `HEAD` and are kept only as history.
- **Owner steer, kept in the decisions page header**: this is a **worldbuilding tool** — **speed is a priority, not a stop line**; values and evidence are not traded for it, and slowdowns are **recorded as defects**. The slow part is our verification loop, not the owner-facing solve (4–6 s per body). ⚠ **Separate from 규칙 0번** (roster relevance is not a work criterion): evidence work is not narrowed by scope.
- **Rules bought today, each by its instance.** ① **Anchors and control bodies are read from the test file's own definitions** — hand-entered inputs made a HOLD out of nothing. ② **Print user beside wall, and discard a wall that greatly exceeds user** — a suspended process reported 19 949 s. ③ **Compare like with like** — a «within 1 %» claim compared user against real; user against user it was 0.05 %. ④ **Record the requests, don't sample the grid** — a sampled band said 8.83e-10 where all 8 009 requested states said 1.382e-9. ⑤ **Count a day with explicit times.** ⑥ **Cite the artifact, not the index** — a hash belongs only on something that promises not to change. ⑦ **A specification sentence is read as an instruction**: if a reader can build it two ways, it is not finished.
- ⚠ **Tree state at the time of writing:** verify with `git status --porcelain` and `git ls-remote origin engine/prototype` before quoting a tip or a line. **Cite by anchor, never by line number.**
## Resume pointer — 2026-09-12 ~09:00 (superseded by the block above)

- **Seats** c3 directing · 21 work · 70 parallel · 45 audit. Brief counter at **196**; ledger items run to **C79** with **C80 closed by 195**, and **C82 · C83 · C84** proposed overnight — rows not yet written.
- **Remote `engine/prototype`** = **`fe26aa7d`** at the last verified read (`git ls-remote`, `fe26aa7d2f68…`), plus **`40989ff8`** (193) and **this commit** (194), both local and unpushed — re-read at fold time, `git ls-remote origin engine/prototype` = `fe26aa7d2f687ad61cef7320abc3f24b4ad727b1` with local `HEAD` = `40989ff8` (2026-09-12 06:35). ⚠ **Confirm the tip yourself before citing it** — three pushes happened inside this block.
- **What landed, in order.**
  1. **`3716708f` — 190 B**, the value-preserving levers. `test_interior.py` alone went **5276.14 s → 3064.27 s** (−41.9 %) with output unchanged. ⚠ **The registered wall-clock target was missed and is recorded as a miss**: «≤ 1.5 × 693.11 s = 1039.7 s» against 3064.27 s, **2.95×** over. Against the pre-190 baseline the state is **wall 4.42×** and **instructions 4.69×** (84.28e12 vs 17.96e12) — name the measure whenever either number is quoted. The **counter criterion passed**: `thermal_at_hse` 19 024 · `density_at` 6 736 unchanged, `free_energy` **1 225 952 → 833 250**, per inversion **182 → 123.70**. Gate 57 min 38 s, FAIL 0.
  2. **`1554fdf7` — 191**, BurnMan installed into **`engine/.venv-burnman`** (Python 3.9) with a **29-line** `engine/requirements-burnman.txt`; `engine/.venv` and `engine/requirements.txt` untouched. The pin file ends in **`setuptools==58.0.4`** — «pin what you checked» — and its header records that the closure's own setuptools is **82.0.1**, required unversioned by `osqp`, **fetched and not installed** because the venv's bundled 58.0.4 already satisfied it. ⚠ **Two counts that must not be merged**: the pin file is **29 lines**, the archive record **30 entries**.
  3. **`fe26aa7d` — 195**, the pool barrier names a vanished worker. **C80 is closed.** Proof splits two ways: a **killed** worker is named **at the next poll**, the 60 s ceiling never reached (2 of 3 steps reaped; no `pool_barrier_deadline` line — the kill log's own last line reads `배리어가 돌아오기까지 0 s · fail=1` (`SECONDS` is integral, so under a second), against `5 s` in the stall proof), a **live but silent** worker waits out the ceiling (5 s in the proof) and the line names who is still running. ⚠ **The control did not complete** — the same injection could not be aimed at the old code, which has no `.pid` and an unexpanded `$_base` in the subshell's command line, so it shows «the old tree has no name to point a kill at», **not** how the old code fails; the old hang itself was not reproduced. Combined 191+195 gate **56 min 50 s, FAIL 0**, longest step 2993 s.
  4. **`40989ff8` — 193**, duplicate-output reporting, in `engine/check_contracts.py` (+62) and `engine/interior-core.md`.
  5. **194** — C73 lockstep, **this commit**: both diffs printed against a baseline pinned as a **set** (39 + 1), no edge drawn, nothing judged.
- **191's check 4 ran, and it retires a standing claim.** `engine/.venv/bin/python engine/test_interior.py`, alone, at `fe26aa7d`: **rc 0 · 2992.93 s · 0 `SKIP` lines · «모두 통과»**, output `191-seafreeze-check4.txt` (`5a3dbeb3e878f460…`, 326 lines). All **four** SeaFreeze-gated sections executed and passed — Grüneisen identity **0.0000 %** on `ice_ih` · `ice_iii` · `ice_v` · `ice_vi`; constant drift 3e-08 to 7e-08; `ice_ih` **8.3e-12** against IAPWS-06; liquid water worst ρ 8.5e-05 · c_P 7.9e-04; `ice_x` worst **1.475 %** against its own source, seam **−2.26 %**. So «SeaFreeze cannot be exercised / importable ≠ reproduced» is **withdrawn** — appended beside the registered text in P36, 190 §7 (Amendment 9) and owner item ⑧, never as an edit. ⚠ Three boundaries travel with it: this is **our arithmetic against SeaFreeze's**, not a check of French & Redmer 2015; **`ice_vii` was not compared** (continuity and seam only); the **−0.37 %** against the published ice VI value **is the temperature span**, by the output's own label.
- **Findings opened overnight, none rowed yet.**
  - **C82** — a registered pressure window written wider than the real domain; the inversion saturates silently at the edge, and the domain is a temperature-dependent band, so one pressure axis overstates it.
  - **C83** — several documented refusals sit outside the audit's net, so nobody checks that they fire.
  - **C84** — **`ThermalSet` has no `t_min` field.** Of six sets **four declare `t_max`** and the two liquid-iron sets declare no temperature edge at all. ⚠ **A lower edge does exist, but only where an evaluator enforces its own** — `water_hot.T_MIN` and `ammonia_table.T_MIN_K`, with real refusals (`eos.py` at the water-hot and ammonia-table guards) — and **`ice_fr2015` has none**, which is why C82 is reachable. ⚠ Do **not** read this as «no lower temperature edge exists anywhere»: `t_min` occurs **0** times, `T_MIN` **10** times, and only the second count describes the engine. ⚠ **M2 waits on this.**
- **The morning list.** **M1** — adopt Newton inversion for more speed and accept density moving by **≤ 1e-9 relative**, or keep bit-identical values; ⚠ **the registered 1039.7 s target is missed either way** (staying: wall 4.42× / instructions 4.69×; adopting: an estimated wall 1.6×, **linear estimate, not measured**). **M2** — widen the ice representation's upper density edge on the authors' «extrapolates well», refuse outside it, or keep today's silent clamp. Standing **①–⑦** unchanged, with ⑥ and ⑧ decided; ⑥ now also carries «installed, and only the licence answer and the install are closed — P34 A–I remain open». **③ has a measured datum**: both producers of `has_inner_core_solved` answer on **Earth and Mars** today and the winner is **execution order** — `core_energy_balance @34` before `core_entropy_production @43` in `graph.order`, declared nowhere. Tool output, reproduced by the audit, alongside `계약 대조 14건 … (레시피 15 / 계산 노드 35)` and `실행 단위 33개를 펼친 51 노드`.
- **Drafts and their hashes.** **190** `8f47c59f2b873ae7` (Amendment 9 = check 4) · **191** `1076df92cce06bb3` (Amendments 5–8: the `.gitignore` gap, the struck §2/§3/§5 text, the scanner breakage, check 4) · **192** `8c009a35e31bd96a` with script `0e33f2eabd41342f` and output `9aca6f0d6f7423a3` · **193** `b0a8829c7eefed6e` · **194** `767c5bd98b8b0127` · **195** `8bac89e9734e8213` · **196** `f19a5dc4325ba4f2`. ⚠ 196's first line still stands: Huang 2018's compositions (11.8 · 10 wt % S) are not ours (13 · 19 wt %), and **13 is outside the printed pair**, so any use is extrapolation labelled `composition-substitute`.
- **Artifact folders, and which one a ledger row cites.** `gate-logs/` (its own `INDEX.md`, **cited without a hash** — see below) · `c80-evidence/` — canonical for the two `587842b3` logs and the audit's `pool/` artifacts · `c80-proof/` (INDEX `458ff38749b4f4e7`, five files, `shasum -c` 5 OK) — **195's rows cite this one**. ⚠ The three hold different things; a citation to the wrong folder points at a duplicate or at nothing.
- ⚠ **Cite the artifact, not the index.** An index is **mutable by design** — it grows whenever a file is added — so a hash pinned to one goes stale on the next improvement, and a reader then cannot tell an improvement from a tamper. This happened today: adding the two `.time` files and one `.out` to `gate-logs/INDEX.md` moved it from `a04161589530f0e2` to its present contents and broke **three** citations — one of them **inside the pushed tree** (C79's ledger row in `engine/interior-core.md`, committed at `3716708f`), which therefore rides the next code commit rather than being edited. **A hash is a promise of immutability, so it belongs only on an artifact that promises it back** (audit's wording) — and the three citations that broke today all pointed at something **designed to change**: a skip set that must grow, an index that must grow, a checker's scan surface that must widen. That test is checkable before the citation is written, which «improvements break citations» is not. `c80-proof/` carries `SHA256SUMS.txt` for exactly this reason. Name an index without a hash; pin the file you actually mean.
- **Rules added overnight, each with the instance that bought it.**
  1. **An assertion is evaluated against the failure it is written for** — «ΔE_th = 0 on every 7000 K row» passed on a broken parse that happened to keep exactly those rows.
  2. **Read coverage from where the fit is defined**, not from a stability region or a simulation box — 190 §2 and 192 made the same substitution.
  3. **«Absent» is per interpreter, never per machine** — SeaFreeze was called uninstalled on the strength of system python.
  4. **A second evaluation is reproduction; an import is not** — «importable» stood in for «reproduced» until check 4 ran.
  5. **Name the measure inside the claim** — «4.7×» was the instruction ratio quoted where the wall ratio (4.42×) belonged.
  6. **Hash with the read time** — three amendments were written against a draft that had already moved.
  7. **A timing run is a read of the machine** — while any duration is measured nothing else runs, and **the timing seat, not the permitting seat, says go**.
  8. **A dirty worktree relabels the citation** — non-empty `git status --porcelain` ⇒ «worktree @<tip> + uncommitted».
  9. **Print the list; derive the count from it** — a count and its list disagreed twice inside one document.
  10. **`.gitignore` speaks to git, not to scanners** — the new venv made 18 citations ambiguous because a skip set matched the exact component `.venv`.
  11. **A control has to be inside the same run as the thing it tests** — an ignore-rule test could not separate the new pattern from the old one, because `core.excludesFile` **adds to** the repo's `.gitignore`.
  12. **Cross-session clearances go to the directing seat** — no seat borrows another's permission, and a peer's «go» is not the owner's.
- ⚠ **Tree state at the time of writing:** confirm with `git status --porcelain` and with `git ls-remote origin engine/prototype` before quoting either a tip or a line of code; 194 is in flight. **Cite by anchor, never by line number** — `.gitignore`'s venv pattern is line **54** on `engine/prototype` and **52** on `main`, which is exactly the failure mode anchors avoid.

*Written by the parallel seat for the work seat to fold into `engine/SESSION-HANDOFF.md`; folded 2026-09-12 with both placeholders filled — 194's sha is this block's own commit and the remote tip was re-read at fold time — so the note that asked for them is gone. Measurements are attributed in the drafts they came from: the 190 B timings and the 195 proofs are the work seat's, the `@34/@43` and `15 / 35` figures are tool output reproduced by the audit, the closure count is the parallel seat's.**194's sha** and the remote tip as it stands then. Measurements are attributed in the drafts they came from — the 190 B timings and the 195 proofs are the work seat's, the `@34/@43` and `15 / 35` figures are tool output reproduced by the audit, the closure count is the parallel seat's.*

## Resume pointer — 2026-09-11 ~23:30 (superseded by the block above)

- **Seats** c3 directing · 21 work · 70 parallel · 45 audit. Brief counter at **195**; ledger items run to **C79**, with **C80 proposed and not yet rowed**.
- **Remote `engine/prototype`** = **`587842b3`** (verified with `git ls-remote`: `587842b3457805d020c1d27a5bc61f9d45dc8596`), **20 commits on the branch today**. **190 landed** — ice VII and X take their thermal answers from French & Redmer 2015's printed free energy, on the authors' recommended **HSE** parametrization of Eq. (9), where today's constants came from **PBE**; that is a deliberate column change of about 1 % in P at ρ 2.2566 g/cm³ · 1000 K, and the ledger carries it as **C79**.
- ⚠ **A measurement is in flight and its subject is undecided.** The gate hung once during 190's work, and `test_interior.py` took **68 minutes** in the run that followed. Whether 190 bought that time is **being measured now** — the work seat is running the same test on the trees either side of 190 (up to ~90 min), with the audit's three-way timing after it. **Until those finish, no cause is stated**, and **C80 is a candidate rowed on the hang itself**, not on an explanation of it.
- **192 ran and printed.** Our liquid-iron values were compared with two published sources at Ichikawa's own fitted states: ρ is **above both at 337.7 GPa · 7000 K and below both at 142.9 GPa · 5000 K** — no single-signed offset — while K_S is **below both at both states** (−12.1 % and −6.2 % against Ichikawa). The 60 GPa state is **withheld**, its nearest comparand being 90 GPa away. **The table chooses no source**; owner decision ① now has numbers in front of it and the directing seat's recommendation is (가).
- **Drafts and their hashes.** **191** `34becabb31dd60f0` — BurnMan install, now four amendments deep: the closure is **29 archives, 100.9 MB, all wheels, extras unresolved**, BurnMan is the **only GPL member**, and ⚠ **installing it would downgrade `mpmath` 1.4.1 → 1.3.0** through sympy, which the venv pins for the Fermi oracle. Decision: BurnMan gets **its own venv** (`engine/.venv-burnman`), `engine/requirements.txt` untouched, pins in a second file. **192** `8c009a35` (draft) with its run — script `0e33f2eabd41342f`, output `9aca6f0d6f7423a3`, ledger addendum `c0cd0a3f3f73471d`. **193** `59aef92c` — C68 merge rule. **194** `f106e1d1` — C73 lockstep, recounted to **39 forward and 1 reverse**. **195** is the work seat's and is not in this folder.
- **Held but not run: the Fermi-oracle check.** A throwaway `/tmp` venv with `mpmath==1.3.0` — the version the resolver actually picks, since mpmath's releases are 1.0.0 · 1.1.0 · 1.2.1 · **1.3.0** · 1.4.0 · 1.4.1 — running `test_fermi.py` from a clean clone of `587842b3`, with the pinned side produced by **reading** `engine/.venv`. Both value sets print with the test's own tolerance beside them. **Waiting on the machine**: work seat's measurement, then the audit, then this.
- **Paper cache: 311 of 311 primary files carry provenance.** New tonight: the ten owner downloads closing B42–B51, and **BurnMan 2.1.0's sdist as a reading copy** (`84011ec328227123…`). ⚠ **Neither BurnMan's sdist nor its wheel contains any licence text** — `dist-info` holds only METADATA, WHEEL, RECORD — and its metadata makes **three** statements: `License: GPL`, `GPLv2+`, and `License :: Other/Proprietary License`. The NOTICE row rests on the **repository's** `license.txt`, a different artifact from the one an install unpacks. That sentence is with the owner; ⑥ stands as decided.
- **One request is open and gated**: Huang 2018's Supporting Information, HTTP 403 from the publisher for both the article page and the SI link.
- **Rules added since 20:00**, each bought by a specific mistake:
  1. **A column the script parses is a column the table must judge** — the parser held K_T and K_S for its own row count while the cell beside them said «not printed».
  2. **A dependency table complete about the wrong set is indistinguishable from complete** — resolving the closure is a registered step, not a caution; `mpmath` was never in the direct table.
  3. **Single-source rows are labelled by source count at every state**; «withheld» is reserved for rows that have a band and states too far apart.
  4. **A pin that outlives its reason is worse than no pin**, because it looks deliberate — hence the check that fires when a dependency's band moves, not only when resolution fails.
- ⚠ **Tree state at the time of writing:** clean at `587842b3` unless the work seat's measurement has left something; anyone reading engine files before the next commit should confirm with `git status --porcelain` and label the read accordingly.

*Written by the parallel seat for the work seat to fold into `engine/SESSION-HANDOFF.md`. The previous block («written 2026-09-11 ~20:00») becomes «superseded by the block above». ⚠ The C80 evidence hashes (hang log, timing runs) are the work seat's and the audit's — they are not in this folder and must be filled in at fold time.*

## Resume pointer — 2026-09-11 ~20:00 (superseded by the block above)
...
- **189 is staged as two commits** (work seat) with sha placeholders here: commit 1 = `5c5b48d3`, commit 2 = this commit. Its registered definition is the part to read before touching any solver: **`converged` is three-state per site — `True` · `False` · `None`**, where `None` means «no criterion branch; the loop runs its full count by design» (four bisections are in that state — `engine/core_state.py@«기준 가지가 없다 — 정해진 횟수를 다 돌고 중점을 돌려준다»`, `engine/mantle_flux.py@«기준 가지가 없다 — 정해진 횟수를 다 돌고 중점을 돌려준다»`, `engine/ammonia_table.py@«기준 가지가 없다 — 예순 번을 다 돌고 중점을 돌려준다»`, and **`stagnant_lid.py` — retired, target gone**: the old `:360` landed on a blank line **already at `f681afbd`, the commit that wrote this line**, so which site it meant cannot be recovered; the file carries three such bisections (`nu_asymptotic_bisect`, `ra_local_bisect`, `nu_table2_bisect`), which is also why «four bisections» undercounts — and adding a criterion is forbidden because an early `break` returns a different midpoint). At a node, `converged` is the AND over sites that have a criterion branch **and ran**; `unconverged_solvers` lists the False sites; `bracket_invalid` lists sites whose entry sign check failed. ⚠ **If every site in a node is `None`, the node is `None`, not `True`** — «nothing to ask» and «everything passed» must not print the same. Files by hash: draft `drafts/189-c71-unconverged-delivered-prereg.md` **`24a5cf77050e8624`**, loop census `c71-loop-census-7ae0ed27.txt` **`19599ae6a5fdcb1e`**, criterion branches `c71-criterion-branches-7ae0ed27.txt` **`200c72b6adbe322d`**. The C76 call-surface baseline is `c76-callsurface-7ae0ed27.txt` **`60c75ee934d1a85c`**.
