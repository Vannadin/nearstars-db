...
- **Gate**: `scripts/check.sh --from <sha> --targeted` per brief (169 G mapped `*_anchor.json` → its reading test; its blanket `tools/*.py` → “the tool itself” rule is being rolled back to “only the four tools check.sh calls” — the other eleven include writers (`make_water_table.py`, `refresh_board_rows.py`) that a targeted run would execute; until that lands, a commit touching a `make_*` tool must not run non-isolated targeted); full only on the push tip; never two at once; scratches are the gate's to delete.
