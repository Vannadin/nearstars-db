...
**C52 closed as a count (`3bd3de8a`)**: functions in `mantle_budget` sharing Earth defaults — 5 by signature, 3 by radius pair (what grep saw), 6 touching any of the four (one reads `D_MANTLE_M` in its body), 10 places in total resting on the shared constant; three stale *labels* repaired (labels written after the first move 1.5 → 0.14 were wrong after the second → 0.088) — repairs point at the constant's declaration, never retype the number.
